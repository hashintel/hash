...
export const parseSessionLogArchive = (input: unknown): SessionLogArchive => {
  ...
  for (const session of archive.sessions) {
    ...
    for (const entry of session.entries) {
      if (entry.versions.some((version, index) => version.version !== index + 1)) {
        throw new TypeError(`Archived entry ${entry.ordinal} has non-contiguous versions.`);
      }
      for (const version of entry.versions) {
        if (!isJsonValue(version.materialized)) {
          throw new TypeError(`Archived entry ${entry.ordinal} is not JSON-compatible.`);
        }
      }
    }
    for (const read of session.reads) {
      if (!read.settlements.every(isJsonValue)) {
        throw new TypeError(`Session log ${session.sessionId} has a non-JSON settlement.`);
      }
      ...
      if (readOrdinals.some((ordinal, index) => index > 0 && ordinal <= readOrdinals[index - 1]!)) {
        throw new TypeError(
    ...
    if (new Set(readIdentities).size !== readIdentities.length) {
      throw new TypeError(`Session log ${session.sessionId} repeats a materialized read.`);
    }
  }
  return archive;
};
...
const sameVersion = (
  ...
  canonicalString(archived.materialized) === canonicalString(incoming.materialized);
...
export const archiveSessionLogRead = (
  ...
  if (
    ...
    read.entries.some(
      (entry) => entry.substrateEntryId.length === 0 || !isJsonValue(entry.materialized),
    ) ||
  ...
  let session = cloned.sessions.find((candidate) => candidate.sessionId === read.sessionId);
  if (!session) {
  ...
  for (const incoming of read.entries) {
    ...
    if (!archived) {
      archived = {
        ...
        ...(read.incarnation === undefined ? {} : { substrateIncarnation: read.incarnation }),
        versions: [],
      };
      entries.push(archived);
    }
    const versions = archived.versions as ArchivedSessionEntryVersion[];
    let version = versions.find((candidate) => sameVersion(candidate, incoming));
    if (!version) {
      version = {
  ...
  const archivedRead: ArchivedSessionRead = {
    ...
    ...(read.incarnation === undefined ? {} : { incarnation: read.incarnation }),
    entries: readEntries,
    settlements: structuredClone(read.settlements),
  };
  const archivedReadIdentity = canonicalString(archivedRead as unknown as JsonValue);
  if (
    !session.reads.some(
      (candidate) => canonicalString(candidate as unknown as JsonValue) === archivedReadIdentity,
    )
  ) {
...
const currentVersion = (entry: ArchivedSessionEntry): ArchivedSessionEntryVersion =>
  entry.versions.at(-1)!;
...
export const readArchivedEntryRange = (
  ...
  const entries = session?.entries.filter(
    (entry) => entry.ordinal >= pointer.entryStart && entry.ordinal <= pointer.entryEnd,
  );
