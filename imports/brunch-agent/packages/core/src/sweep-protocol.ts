...
export const advanceSweepHighWater = (state: SweepState, throughUserEntryId: string): SweepState =>
  parseSweepState({
    ...state,
...
export const computeUnaccountedAskAdvisories = (
  ...
  const accountedAffordanceIds = new Set(
    tail.flatMap((entry) =>
      entry.replyToAffordanceId === undefined || !capturedUserEntryIds.has(entry.id)
        ? []
