...
describe('settlement and sweep protocol', () => {
  test('keeps the high-water and loop guard in one parse-checked sweep state', () => {
    ...
    expect(initial).toEqual({ sweptThroughUserEntryId: null, lastCheckedUserEntryId: null });
  ...
  test('computes the unswept range through the latest true-user entry only', () => {
    expect(unsweptTail(entries, createInitialSweepState())).toEqual(entries.slice(0, 2));

    const swept = advanceSweepHighWater(createInitialSweepState(), 'user-reply');
  ...
  test('nudges once per latest user entry, declines legally, and suppresses suspended asks', () => {
    ...
    if (first.action !== 'nudge') throw new Error('expected a settlement nudge');
    ...
    expect(
      decideSettlementTrigger({
        entries: [...entries, { id: 'agent-declines', kind: 'assistant', text: 'Not yet.' }],
  ...
  test('reports asks without an affordance-bound capture as advisory-only facts', () => {
    ...
    expect(computeUnaccountedAskAdvisories(entries.slice(0, 2), new Set())).toHaveLength(1);
    expect(computeUnaccountedAskAdvisories(entries.slice(0, 2), new Set(['user-reply']))).toEqual(
      [],
    );
  });
  ...
  test('gives a refused settled range one same-response repair continuation', () => {
    ...
    const refused: readonly SweepSessionEntry[] = [
      ...entries,
      { id: 'refusal', kind: 'assistant', text: '', sweepResult: { status: 'refused', refusal } },
    ];
