...
): SessionLogRead['entries'][number] => ({ substrateEntryId, kind, text, materialized });
...
describe('session-log archive', () => {
  test('identity-merges evolving entries, versions changed materializations, and skips exact duplicates', () => {
      ...
      read(
        ...
        [
          ...
          entry('message-assistant', 'assistant', 'Let me confirm that.', {
            parts: [{ type: 'text', text: 'Let me confirm that.', state: 'done' }],
          }),
        ],
    ...
    expect(
      session.entries.map(({ ordinal, substrateEntryId }) => ({ ordinal, substrateEntryId })),
    ).toEqual([
  ...
  test('resolves true-user and harness-classified affordance quotes to archive ordinals', () => {
    ...
    expect(resolveEvidenceQuotes(archive, 'session-1', [{ excerpt: 'June works.' }])).toEqual({
      ok: true,
      evidence: [
        {
          excerpt: 'June works.',
  ...
  test('distinguishes no match from an injected non-user match and provides repair guidance', () => {
    const archive = archiveSessionLogRead(
      createEmptySessionLogArchive(),
      read('1', [entry('message-injected', 'non-user', 'Begin the interview.')]),
    );

    expect(resolveEvidenceQuotes(archive, 'session-1', [{ excerpt: 'missing' }])).toEqual({
      ok: false,
      refusal: {
    ...
    expect(
      resolveEvidenceQuotes(archive, 'session-1', [{ excerpt: 'Begin the interview.' }]),
    ).toEqual({
      ok: false,
      refusal: {
  ...
  test('retrieves every entry in a stored pointer range without consulting the substrate', () => {
    const archive = archiveSessionLogRead(
      createEmptySessionLogArchive(),
      read('2', [entry('message-1', 'user', 'first'), entry('message-2', 'assistant', 'second')]),
    );
