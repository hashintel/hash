...
const apply = (
  ...
  expect(parseCaptureStoreSnapshot(JSON.parse(JSON.stringify(result.snapshot)))).toEqual(
    result.snapshot,
  );
  return result;
};
...
describe('capture-store contract', () => {
  ...
  test('open-issue refuses every issue the persisted contract would reject', () => {
    ...
    for (const [reason, overrides] of [
      ...
      ['a plugin origin whose namespace is empty', { origin: { type: 'plugin', namespace: '' } }],
      ['no references at all', { references: [] }],
      ['the same capture referenced twice', { references: [captureId, captureId] }],
      ['a reference to a capture that does not exist', { references: ['capture-missing'] }],
  ...
  test('a conflicting issue opens only over two or more distinct active captures', () => {
    ...
    const [marchId, juneId, septemberId] = created.snapshot.captures.map((capture) => capture.id);
    // March is superseded by a correction; September is retracted.
    const corrected = apply(created.snapshot, {
      type: 'apply-sweep',
      proposals: [
        valueProposal('April', userEvidence('Actually April', 4), { supersedes: marchId! }),
      ],
    });
    ...
    expect(deriveIssueStatus(resolved.snapshot, issue.value.issueId)).toBe('closed');
    ...
    if (!('issueId' in ambiguous.value)) throw new Error('Opening an issue did not return its id.');
  ...
  test('an unresolved conflict pins its captures against supersession and retraction', () => {
    ...
    const [marchId, juneId, venueId] = captures.snapshot.captures.map((capture) => capture.id);
    const issue = apply(captures.snapshot, {
      type: 'open-issue',
      issueType: 'conflicting',
      origin: { type: 'harness' },
    ...
    for (const [attempt, command] of [
      ...
      [
        'superseding the June side',
        {
          type: 'apply-sweep',
          proposals: [
            valueProposal('July', userEvidence('Actually July', 5), { supersedes: juneId! }),
          ],
        },
      ],
      [
        'retracting the March side',
        { type: 'retract-capture', captureId: marchId!, evidence: [userEvidence('Forget it', 6)] },
      ],
      [
        'retracting the June side',
        { type: 'retract-capture', captureId: juneId!, evidence: [userEvidence('Forget it', 7)] },
      ],
    ] as const) {
  ...
  test('a persisted conflicting issue of one capture is not readable', () => {
    ...
    const [marchId, juneId] = created.snapshot.captures.map((capture) => capture.id);
  ...
  test('a command stores its own copy of the evidence and ids the caller passed', () => {
    ...
    const retraction = retracted.snapshot.events.find((event) => event.type === 'retraction');
  ...
  test('a caller-supplied evidence range is refused at every evidence command surface', () => {
    ...
    const [marchId, juneId, septemberId] = captures.snapshot.captures.map((capture) => capture.id);
    const issue = apply(captures.snapshot, {
      type: 'open-issue',
      issueType: 'conflicting',
      origin: { type: 'harness' },
  ...
  test('persisted snapshots refuse a reversed evidence range in a capture or an event', () => {
    ...
    const withReversedRange = (family: 'captures' | 'events'): unknown => {
      const clone = structuredClone(retracted) as unknown as Record<string, EvidenceBearing[]>;
    ...
    expect(() => parseCaptureStoreSnapshot(withReversedRange('captures'))).toThrow(/range/i);
    expect(() => parseCaptureStoreSnapshot(withReversedRange('events'))).toThrow(/range/i);
  });
  ...
  test('every command type round-trips its accepted result through persisted parsing', () => {
    ...
    const [marchId, juneId, venueId] = snapshot.captures.map((capture) => capture.id);
    ...
    if (!('issueId' in conflict.value)) throw new Error('Opening an issue did not return its id.');
    ...
    expect(parseCaptureStoreSnapshot(JSON.parse(JSON.stringify(snapshot)))).toEqual(snapshot);
    expect({
