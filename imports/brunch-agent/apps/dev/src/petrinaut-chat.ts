...
import { createFlueReplyProjector, projectFlueHistoryForSweep } from '@brunch/binding-flue';
...
const targetDocumentIdFor = (conversationId: string): string => `petrinaut-local:${conversationId}`;
...
export const petrinautChatHandler = createAiSdkChatHandler({
  ...
  askReply: {
    ...
    async admit(input) {
      ...
      return decideAskReplyAdmission(pendingAskAffordanceId(entries), input.ask.toolCallId);
    },
