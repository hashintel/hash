...
faux.setResponses([
  fauxAssistantMessage([
    fauxThinking('I should establish the process outcome before proposing structure.'),
    fauxText('What outcome should this process reliably produce?'),
  ]),
  fauxAssistantMessage([
    fauxThinking('The settlement check does not add user evidence.'),
    fauxText('We can start with the outcome and then work backward through the process.'),
  ]),
]);
...
try {
  ...
  const fixturePath = fileURLToPath(
    new URL('../../../test/fixtures/transport-aisdk/panel-initial.post.json', import.meta.url),
  );
  ...
  const partIds = chunks
    .filter((chunk) => chunk.type === 'reasoning-start' || chunk.type === 'text-start')
    .map((chunk) => chunk.id);
