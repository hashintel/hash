...
faux.setResponses([
  ...
  fauxAssistantMessage([fauxText('The settled prefix is captured; nothing further to sweep.')]),
]);

const flue = await start({ agents: [GherkinElicitor], providers: [faux.provider] });
...
try {
  ...
  const postChat = async (requestId: string, body: unknown): Promise<Response> =>
    app.fetch(
  ...
  const askCall = initialChunks.find((chunk) => chunk.type === 'tool-input-available');
  ...
  console.log(
    `PETRINAUT_ASK_RESULT ${JSON.stringify({
      ...
      askCall,
      initialToolOutputs: initialChunks.filter((chunk) => chunk.type === 'tool-output-available'),
