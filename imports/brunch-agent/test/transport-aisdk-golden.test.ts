...
const readSseChunks = (name: string): StreamChunk[] => {
  const frames = readFileSync(join(FIXTURES, name), 'utf8').trim().split('\n\n');
...
describe('FE-1435 real-panel wire transcript', () => {
  ...
  test('validates one batched follow-up with both client outputs and diagnostics decoration', () => {
    ...
    const clientToolOutputs = assistant.parts.filter(
      (part) => part.type === 'tool-addPlace' || part.type === 'tool-addTransition',
    );
    ...
    expect(clientToolOutputs.map((part) => part.output?.applied)).toEqual([true, true]);

    const serverTool = assistant.parts.find((part) => part.type === 'tool-serverProbe');
    ...
    expect(diagnostics.parts[0]?.text).toStartWith(
  ...
  test('validates streamed reasoning, text, server tool, and client calls', () => {
    ...
    expect(
      chunks
        .filter((chunk) => chunk.type === 'tool-input-available' && chunk.providerExecuted !== true)
