...
const responseChunks = async (response: Response): Promise<readonly StreamChunk[]> =>
  (await response.text())
    .trim()
    .split('\n\n')
    .slice(0, -1)
...
const post = (body: unknown): Request =>
  new Request('http://brunch.test/api/petrinaut/chat', {
    method: 'POST',
    headers: { 'content-type': 'application/json', 'x-request-id': 'request-fe1449' },
...
const askSuspensionEvents: readonly HarnessReplyEvent[] = [
  ...
  },
  {
    ...
    output: {
      ...
      payload: { question: 'What outcome should this process reliably produce?' },
    },
    execution: 'server',
  },
...
const resumedTurn: AskReplyHandler['run'] = async (_input, emit) => {
  ...
  emit({ type: 'response-finish', terminalState: 'completed', finishReason: 'stop' });
};
...
describe('FE-1449 ask suspension on the wire', () => {
  test('translates the ask as an awaiting client tool and withholds the affordance output', async () => {
    ...
    expect(chunks.find((chunk) => chunk.type === 'tool-input-available')).toEqual({
...
describe('FE-1449 ask return POST', () => {
  ...
  test('refuses stale, duplicate, and forged submissions before any dispatch', async () => {
    for (const [reason, error] of [
      ...
      expect({ reason, status: response.status, body: await response.json() }).toEqual({
        reason,
        status: 409,
        body: { error },
      });
  ...
  test('keeps refusing machine-only follow-ups: mutation outputs and diagnostics stay non-user', async () => {
    ...
    const response = await handler(
      post(readFileSync(join(FIXTURES, 'panel-tool-results.post.json'), 'utf8')),
    );

    expect(response.status).toBe(422);
    expect(await response.json()).toEqual({ error: 'tool_result_follow_up_not_supported' });
    expect(resumed).toBe(false);
  });
  ...
  test('refuses a malformed or ambiguous ask submission as invalid', async () => {
    ...
    const withParts = (parts: readonly unknown[]): unknown => ({
      ...
      messages: [
        ...
        { ...askReturnPost.messages[1], parts: [{ type: 'step-start' }, ...parts] },
      ],
    });
    ...
    for (const body of [
      ...
      expect(await response.json()).toEqual({ error: 'invalid_ask_submission' });
      expect(resumed).toBe(false);
    }
  });
