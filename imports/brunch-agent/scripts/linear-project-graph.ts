...
const cycleMembers = (
  ...
  const reachesStart = (start: string, current: string, visited: Set<string>): boolean => {
  ...
  for (const issueId of issueIds) {
    if (reachesStart(issueId, issueId, new Set([issueId]))) cycleIds.add(issueId);
  }
  return cycleIds;
};

export function renderProjectGraph(graph: ProjectGraph): string {
  const issuesById = new Map(graph.issues.map((issue) => [issue.identifier, issue]));
  const hardEdges = graph.hardEdges
    ...
    .sort(
      (left, right) =>
        compareIdentifiers(left.from, right.from) || compareIdentifiers(left.to, right.to),
    );
  ...
  const cycles = [...cycleMembers(unlayeredIds, outgoing)].sort(compareIdentifiers);
  ...
  const issueLines = [...graph.issues]
    .sort((left, right) => {
      const leftLayer = layerById.get(left.identifier) ?? Number.POSITIVE_INFINITY;
      const rightLayer = layerById.get(right.identifier) ?? Number.POSITIVE_INFINITY;
      return leftLayer - rightLayer || compareIdentifiers(left.identifier, right.identifier);
    })
    .map((issue) => {
...
const isLinearIssueRef = (value: unknown): value is LinearIssueRef =>
  ...
  (value.project === null || (isRecord(value.project) && typeof value.project.name === 'string'));

const readProjectIssuePage = (value: unknown): ProjectIssuePage => {
  if (!isRecord(value)) throw new Error('Linear returned a non-object response.');
  if (Array.isArray(value.errors) && value.errors.length > 0) {
    const messages = value.errors
      .map((error) => (isRecord(error) && typeof error.message === 'string' ? error.message : null))
      .filter((message): message is string => message !== null);
    throw new Error(`Linear GraphQL refused the project query: ${messages.join('; ')}`);
  }
  const data = value.data;
  const projects = isRecord(data) && isRecord(data.projects) ? data.projects.nodes : undefined;
  ...
  for (const issue of issues) {
    if (
      ...
      (issue.parent !== null &&
        (!isRecord(issue.parent) || typeof issue.parent.identifier !== 'string'))
    ) {
    ...
    for (const relation of issue.relations.nodes) {
      if (
        ...
        (relation.relatedIssue !== null && !isLinearIssueRef(relation.relatedIssue)) ||
        (relation.type === 'blocks' && !isLinearIssueRef(relation.relatedIssue))
      ) {
        throw new Error(`Linear returned an unexpected relation shape for ${issue.identifier}.`);
      }
    }
    ...
    if (issue.relations.pageInfo.hasNextPage || issue.inverseRelations.pageInfo.hasNextPage) {
      throw new Error(
...
const queryProjectPage = (projectName: string, after: string | null): ProjectIssuePage => {
  ...
  if (result.exitCode !== 0) {
    throw new Error(`linear api failed: ${result.stderr.toString().trim()}`);
  }
...
export const fetchProjectGraph = (projectName: string, includeClosed = false): ProjectGraph => {
  ...
  for (const issue of projectIssues.values()) {
    ...
    for (const relation of issue.relations.nodes) {
      ...
      if (relation.type !== 'blocks' || !relatedIssue || !selected(relatedIssue)) continue;
      issues.set(
        relatedIssue.identifier,
        issues.get(relatedIssue.identifier) ?? asProjectIssue(relatedIssue, resolvedProjectName),
      );
    ...
    for (const relation of issue.inverseRelations.nodes) {
      ...
      if (relation.type !== 'blocks' || !sourceIssue || !selected(sourceIssue)) continue;
      issues.set(
        sourceIssue.identifier,
        issues.get(sourceIssue.identifier) ?? asProjectIssue(sourceIssue, resolvedProjectName),
      );
...
const usage = `Usage: bun run linear:graph [--project <name>] [--all]
...
if (import.meta.main) {
  try {
    ...
    if (options.help) {
      ...
      console.log(
        renderProjectGraph(fetchProjectGraph(options.projectName, options.includeClosed)),
      );
    }
  } catch (error) {
    console.error(`linear:graph: ${error instanceof Error ? error.message : String(error)}`);
    console.error('Run `bun run linear:graph --help` for usage.');
    process.exitCode = 1;
  }
}
