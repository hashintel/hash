...
| Document | Status | Linear | Digest |
| --- | --- | --- | --- |
| [CONVERGENCE](planning/_shared/CONVERGENCE.md) | active | FE-1401 (created); maintained per legibility.md | The convergence record between the two root maps (FE-1383 build, FE-1357 demo/plugin-spec): per-obligation status ledger, vocabulary-drift table, the inter-map seam record, and the living cross-map sequencing strategy. Rows and sequencing update as branches land |
| [flue-architecture-cheatsheet](planning/_shared/flue-architecture-cheatsheet.md) | active | commented on FE-1383; feeds docs/agents/flue-routing.md | Architect's consolidation of all 21 Flue guide pages: affordances, recommended patterns, do-not-rebuild warnings per concern; three-lane boundary summary (consume / translate-in-binding / own); five ranked divergence risks; reconciled against the flue-vs-tilde analysis and FE-1391's installed-2.0.3 source read (public history shape, host-owned URL, compaction preservation) |
| [topology](planning/_shared/topology.md) | active | ratified → ADR-0002; N1 discharged by FE-1422; local N5 implemented by FE-1391 | Pseudo-style verification of the package/app tree against the three-lane model and spec §12.2: ask/session-log protocols in core, Flue hook/history/storage wiring in the binding, and normative placement rules N1–N6 for the sweep executor, packs/cards, demo shell, experiments, and deploy-target storage implementations |
...
| [flue-entry-projection-source-read-2026-08-18](planning/legibility-sweep/flue-entry-projection-source-read-2026-08-18.md) | active | FE-1391 source gate + implementation follow-through; reshapes FE-1386 | Installed `@flue/runtime`/`@flue/sdk` 2.0.3 evidence for history, URL ownership, message discriminators, bounds, and compaction/state preservation, followed by the FE-1391 implementation disposition; FE-1386 remains one behavioral pin |
| [remediation-plan-2026-08-17](planning/legibility-sweep/remediation-plan-2026-08-17.md) | active | A1 discharged by FE-1422; A3/B1 discharged by FE-1391; B2 source-settled | Two ledgers from the sweep's consolidated read: A — eleven wrong-or-fragile items with owners and plan corrections; B — seven information gaps routed by cheapest honest resolution. FE-1391 removed caller-supplied ranges and implemented the public history/archive contract; FE-1386 retains the compatibility pin |
