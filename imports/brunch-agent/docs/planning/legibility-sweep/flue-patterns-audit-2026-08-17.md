...
split most notably), and in one place canon *downgrades* a known finding's severity (the faux
...
3. **Eval harness shape.** Canon (evals guide): Vitest, separate `vitest.evals.config.ts`,
   ...
   test" (their words); the child process *honors* the one-runtime-one-process rule by giving
...
4. **Hybrid in-process + HTTP coverage.** Canon presents `start()` (in-process) and
...
5. **`db.ts` path anchored to the module, env-overridable.** Canon's example is a bare relative
...
6. **Flat agent file rather than per-agent folder.** Canon's multi-agent layout nests
...
7. **`'use agent'` directive discipline.** First statement before imports, exported capitalized
   ...
   `agentName` → function name), and the identity regex. The pinning is *more* load-bearing
...
10. **Tool contract.** Inline `useTool` with valibot `input`/`output`, `run` returning
...
11. **Signals.** `ctx.append({ kind: 'signal', type, tagName, body, attributes })` matches the
...
12. **Instruction assembly.** Agent returns the binding's string; canon composes "your returned
    ...
    change).
13. **`initialData`.** Valibot schema validated once at first contact, immutable — canon's
...
14. **`db.ts` shape.** Default-exports `sqlite(path)` from `@flue/runtime/node` — canon's
...
15. **Version hygiene.** All `@flue/*` packages pinned to one version line (`^2.0.3`);
    `pi-ai` (faux provider) a devDependency only.

## Docs-are-silent — verified empirically or accepted

16. **`start()` does not auto-discover `db.ts` (probed).** The docs say `db.ts` is "discovered
...
17. **Monorepo/workspace composition.** Canon has no monorepo story (its own admission). The
...
- **`db.ts` as build-time convention file kept apart from `db-path.ts`** — consistent with
  ...
  Keep.
- **No-db in-memory eval trick** — canon confirms in-memory default *for the no-`db.ts` case*;
...
1. **`useAgentStart` firing semantics.** ~~Undocumented~~ **Resolved 2026-08-17**: the
   agent-hooks *guide* (a page this audit did not fetch — it read the agent-api reference
...
> part of the repo that *feels* most improvised — is composed entirely of documented parts and
