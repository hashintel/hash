...
6. **That assistant is entirely browser-resident** and cannot be otherwise: tools
   ...
   no headless net-*generation* path (though the CLI is a headless *simulation*
...
- In HASH: the editor runs in a **sandboxed null-origin iframe** at
  ...
  **Why the sandbox exists, and why it constrains everything** (§3.5, and decisive
  for §6a): the editor compiles and runs *user-authored code* — place visualizers
  ...
  **without** `allow-same-origin`, yielding an opaque origin that cannot read
  ...
  no network reach. That is precisely why persistence *and* the AI stream must
...
Brunch Actual-mode integration is *not in any published version*. Consuming
...
The tool *schemas* are declared server-side but **every tool executes in the
...
```ts
import { createJsonDocHandle, type PetrinautDocHandle, type SDCPN }
  from "@hashintel/petrinaut-core";
import { DefaultChatTransport, Petrinaut, type PetrinautAiChatTransport,
         type PetrinautAiMessage, WalkthroughProvider }
...
Background compute *does* exist, but for simulation, not AI:
...
delegates *all* Petrinaut semantics to `@hashintel/petrinaut-cli` over a **stdio
subprocess** — `subprocess.Popen(("petrinaut", ...))` in
`../hash/apps/petrinaut-opt/src/petrinaut_client.py:11,60-119`. So a headless,
Node-based way to *load and simulate* a net does exist and is in production use.
What does not exist headlessly is net *generation* by the assistant — that remains
browser-only. The CLI is `private: true` and unpublished, so consuming it means
vendoring or building from the monorepo.

Note for §6a: the monorepo *does* have serious agent infrastructure —
...
name but is unrelated: it migrates *positional scenario rows* when a user edits a
...
1. **Unknown keys on an individual place/transition/colour in a *file* are
...
`variableName` (lower_snake_case), `type`, `defaultValue` (a *string*, refined by
...
At the *runtime* level (constructing an `SDCPN` in code rather than parsing a
...
hits. `SDCPN` describes *structure only*. Initial state is either:
...
- **a scenario** — `scenario.initialState`, a discriminated union
  ...
  - `{ type: "per_place", content }` — **keyed by place ID**; uncoloured values are
    *string expressions* with `parameters` and `scenario` in scope (e.g.
...
export const PETRINAUT_EXTENSION_NAMES = [
  "colors", "stochasticity", "dynamics", "parameters", "subnets",
] as const;
export const DEFAULT_PETRINAUT_EXTENSIONS = {
  colors: true, stochasticity: true, dynamics: true,
  parameters: true, subnets: true,
};
```
...
A host can *disable* extensions per document via
...
note most are in the *legacy* (unversioned) shape, so pick the right model:
...
*Form 1 — the JSON file.* A generator must emit an object satisfying
...
hamburger → **Import** action exists on demo.petrinaut.org but is *hidden in
HASH*, which passes `hideNetManagementControls="except-title"`. So this option has
...
*Form 2 — the live SSE stream (already built, for brunch specifically).* See §7.1.
...
- The artifact carries **almost no sanctioned provenance** —
  ...
  itself captures none anywhere (§3.6). But the import path is *permissive*
...
- Petrinaut's *editing* affordances are unavailable to the elicitor: once the
...
{ "transitionId": "start_implementation",
  "input": { "queued": 1 }, "output": { "implementing": 1 },
...
Note the split: Actual mode is about *executions* (a running net), whereas the
September demo needs *elicitation of a model* (a net definition). The existing
...
- `ARCHITECTURE.md` carries an explicit note that it deliberately does *not* live
...
- **Two latent bugs found in passing**, both consequences of the opaque-origin
  ...
  1. Petrinaut persists its own visual/user settings to `localStorage` key
     `"petrinaut:user-settings"` *inside the library*
