...
4. Three kernel mechanisms turn out to fit voice by accident: §5.1 already licenses tap-less uis to
   ...
   mid-flight abort, and §7.3's one-live-affordance rule is *strengthened* by a serial audio channel.
...
6. "The agent interrupts to clarify" has **no seam at all**: capability 7 is an end-of-agent-turn
   hook (§8.1, §10), and nothing in the ten capabilities evaluates anything *during* the user's turn.
...
| C3 | Endpointing becomes a ui responsibility the charter withholds | **High** (conceptual) | Name a *turn shell* with a one-way valve; push-to-talk collapses it |
...
The residual risk is therefore not corruption but *conversational* incoherence: an aborted turn
...
In a text ui the Send button is the endpointer, and the *user* owns it — deciding that an utterance
...
Two consequences for us. Our interview domain is the *worst case* for acoustic endpointing: a domain
...
evaluates anything during the user's turn**, and by §7.4's design the turn is *suspended* while the
...
as the current mitigation — and unlike latency it reads as *rudeness*: one badly-timed interrupt in
...
Note also what the managed voice platforms do *not* give you here. Their interruption support is
...
**The rendering one is a small genuine gap.** §7.2's floor is a *markdown* floor, and markdown reads
...
resolution gets *easier*, not harder), while the invariant's plain-English meaning quietly becomes
...
- **Bias the ASR with the plugin's vocabulary.** This is a solved, cheap, well-supported feature and
  ...
  **step-lexicon**"; a target's lexicon *is* the biasing list. This introduces a new read-only
  ...
  distinction, since a forced insertion is *worse* than a misrecognition when the output becomes
...
This collides with the *recommended architecture* rather than with voice itself, and it is the one
...
1. **The return channel is text, full stop.** The JS SDK's only outbound path is
   ...
   parallel channel to the browser *anyway* — at which point the provider transports half the
...
2. **The provider's history model cannot represent ours.** `onTranscript(transcript:
   ...
   signals, which the kernel needs *mechanically* ("a capture citing an injected entry is refused at
...
3. **Our turn is not one text response.** `terminate: true` mixed batches (§7.7: "mixed batches
   ...
   turn mean model work continues *after* the user-facing answer. Both integrations are
...
swapping *TTS only*, are narrowing rather than expanding. Gemini Live also remains "in Preview."
...
render or tool execution. By §7.4's design the turn is *suspended* while the user speaks: there is no
...
re-renders the agent" — so a data part can carry the running list *while an agent turn is running*
...
boundaries. For a 5–20s conversational exchange that reads as live; for the demo's *long audio
description of a system*, the panel sits empty for minutes and then fills at once. Which is exactly
...
running entity list *is* that scratchpad made visible. Therefore:
...
do that a NER pipeline cannot: an entity moving from *noticed* to *captured with an epistemic status
and a quoted span*, on screen, when the sweep lands. That transition is the demo's best single beat
...
1. **Measure the loop.** Instrument end-to-end wall clock for one ask cycle: user dispatch → model →
   ...
   cannot get it under that, T0 (silent, text-out) becomes the *right* answer rather than the safe one.
