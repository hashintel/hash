...
definition ([`ir-design.md`](./ir-design.md), Layer A): speculative payload type systems drafted
...
literal Given/When/Then — then statement granularity *coincides* with artifact granularity, which
is coincidence, not violation; the payload holds what was said either way. The property's force
is that the interviewer never *requires* the artifact's decomposition mid-conversation.
...
report is almost empty at the thin end: the *mechanism* holds but earns little — its value grows
...
above. (3) is the interesting one: the plugin is *named after its projection target*, and its
...
the C5 adjudication (spec §5, Appendix A). What it *does* expose is a wording gap in property 2:
"the granularity the user stated it" has no user here; the mined capture's granularity is set by
the documented transformation. The property generalizes from *statement* granularity to
...
domains), *not* harness-ward: the harness has no domain notion of "manual" or "shop floor".
...
ownership) routes only the *technique* to the generic quiver — contrastive choice-point pressure
("when two X compete for one Y, who wins, by what rule?") operates on harness vocabulary. The
*kind* stays in each plugin's catalog; if the process family grows, the seam is a shared
...
| 3 | Projection-independence | **survives, amended** | Restated operatively, because its bite is proportional to domain–format distance and Gherkin has almost none: kinds are defined in domain vocabulary *and the IR legitimately holds kinds no current projection consumes, with the typed loss report keeping that honest*. The second clause is the enforceable content; the first degenerates gracefully where the target format is the domain. |
...
**Sublimation findings.** The standing expectation held, with better resolution on *where*
