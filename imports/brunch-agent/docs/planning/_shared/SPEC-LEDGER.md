...
| Package topology | §12.2 | **partial** | `core` (+`testing` subpath), `binding-flue`, `transport-aisdk`, `plugin-gherkin`, `apps/dev` exist; `plugin-assurance` pending. FE-1437 records their native HASH destinations and preserves the existing dependency boundaries through the import. Topology pin derives from §12.2 itself (FE-1400 `ef00201`); FE-1436 adds the transport-only dependency gate. |
...
| Imported app's four roles (remote server, dev loop, target gallery, probe surface) | §12.5 | **partial** | remote server + dev loop exist; gallery and probe surface remain owned by FE-1385. FE-1437 re-charters `apps/dev` as HASH's `apps/brunch-agent`; no dedicated demo shell is created. |
