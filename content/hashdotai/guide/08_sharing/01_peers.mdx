...
In addition to crosswalking your representation of an entity with another, you can also set up [AI agents](/guide/agents) in HASH to monitor an external webpage for changes, and suggest or make updates to your own entity in response. Full details of these updates, including their provenance (e.g. source/origin and datetime) are captured as part of the [history](/guide/webs/history) available on every entity in HASH.
