...
PUBLIC_HOST=$(aws ec2 describe-instances --region us-east-1 --filters "Name=tag:Name,Values=h-hash-$WORKSPACE-$REGION-bastion" \
...
ssh -i "$CURR_DIR/h-hash-$WORKSPACE-$REGION-bastionkey.pem" "ec2-user@$PUBLIC_HOST" $@
