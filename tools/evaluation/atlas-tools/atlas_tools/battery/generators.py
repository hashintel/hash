"""Planted-shape dataset generators, one typed pydantic model per shape.

Each generator pairs a per-shape ``*Params`` model (validated, ``extra="forbid"``) with a
``*Generator`` class carrying the ``shape`` discriminator and the node count ``n``. ``run(seed)``
is a pure function of (model, seed) producing float32 embeddings, int64 edge pairs, int64 labels
(-1 = unlabeled), and a ground-truth descriptor. All randomness flows through
``np.random.default_rng(seed)``; identical (config, seed) yields identical bytes. The embedding
dimension is configurable (default 64); nothing hardcodes it.

:data:`Generator` is the discriminated union over all shapes, and the union itself is the
dispatch mechanism; there is no registry. Suite YAML entries (``{shape, n, params}``) validate
directly into generator instances via :data:`generator_adapter`; call ``.run(seed)`` on the
result.

Shapes:

- ``bipartite_star``: documents with about ``items_per_doc`` items each; items have degree 1 (an
  optional ``multi_parent_frac`` gives that fraction of items a second parent). Documents are
  drawn around ``n_topics`` topic centers and items around their parent document; labels are
  topic ids.
- ``clique_communities``: communities with dense intra-community random edges; embedding
  clusters aligned with communities; labels are community ids.
- ``chains``: document-flow chains (order, item, movement, delivery) of configurable depth;
  edges link consecutive chain members; embeddings cluster by type rather than by chain; labels
  are type indices.
- ``lattice_product``: two categorical factors; embeddings near the sum of the two factor
  vectors (ring or sheet structure); edges connect nodes sharing a factor level; labels are
  (factor_a, factor_b) cell ids.
- ``noise_edges``: clustered embeddings plus uniformly random edges that carry no structure;
  labels are embedding cluster ids.
- ``isolates``: clustered embeddings where ``isolate_frac`` of nodes have zero edges and the
  rest get intra-cluster edges; labels are cluster ids.
- ``mixed``: a weighted combination of other shapes concatenated into one dataset, with node and
  label offsets recorded in the truth descriptor.
"""
...
def _cluster_embeddings(
    ...
    """Draw balanced gaussian clusters; return (embeddings f32, labels int64)."""
...
def _canonical_edges(edges: np.ndarray) -> np.ndarray:
    """Canonicalize undirected edges: sort each pair, drop self-loops, deduplicate rows."""
...
def _intra_group_edges(
    ...
    """Draw ``degree`` random same-group partners for each node (never itself)."""
    chunks = []

    for members in groups:
        m = len(members)
        if m <= 1:  # a group needs two members to host an edge
            continue

        a = np.repeat(members, degree)
        b = members[rng.integers(0, m, size=m * degree)]

        bad = a == b
        while bad.any():
            b[bad] = members[rng.integers(0, m, size=int(bad.sum()))]
            bad = a == b
...
class AbstractGenerator[TParams: GeneratorParams](BaseModel, ABC):
    ...
    def config(self) -> JsonDict:
        """Return the resolved config recorded in ``truth.json`` (shape + n + params)."""
...
class BipartiteStarGenerator(AbstractGenerator[BipartiteStarParams]):
    ...
    def run(self, seed: int) -> Dataset:
        ...
        n_docs = max(1, round(n / (k + 1)))
        n_docs = min(n_docs, n)
        n_items = n - n_docs

        topic_centers = self.params.topic_scale * rng.normal(size=(self.params.n_topics, dim))
        ...
        embeddings = np.concatenate([doc_embeddings, item_embeddings]).astype(np.float32)
        ...
        if n_docs > 1 and self.params.multi_parent_frac > 0 and n_items > 0:
            n_multi = round(self.params.multi_parent_frac * n_items)
...
class CliqueCommunitiesGenerator(AbstractGenerator[CliqueCommunitiesParams]):
    ...
    def run(self, seed: int) -> Dataset:
        ...
        groups = [np.nonzero(labels == community)[0] for community in range(n_communities)]
        edges = _canonical_edges(_intra_group_edges(rng, groups, self.params.intra_degree))
        ...
        return Dataset("clique_communities", embeddings, edges, labels, truth, self.config(), seed)
...
class ChainsGenerator(AbstractGenerator[ChainsParams]):
    ...
    def run(self, seed: int) -> Dataset:
        ...
        type_names = [
            CHAIN_TYPE_NAMES[i] if i < len(CHAIN_TYPE_NAMES) else f"type_{i}" for i in range(depth)
        ]
        ...
        embeddings = (centers[type_id] + self.params.noise * rng.normal(size=(n, dim))).astype(
            np.float32
        )
...
class LatticeProductGenerator(AbstractGenerator[LatticeProductParams]):
    ...
    def run(self, seed: int) -> Dataset:
        ...
        embeddings = (
            vectors_a[level_a] + vectors_b[level_b] + self.params.noise * rng.normal(size=(n, dim))
        ).astype(np.float32)
        ...
        return Dataset("lattice_product", embeddings, edges, labels, truth, self.config(), seed)
...
class NoiseEdgesGenerator(AbstractGenerator[NoiseEdgesParams]):
    ...
    def run(self, seed: int) -> Dataset:
        ...
        requested = round(self.params.edge_factor * n)
        ...
        return Dataset("noise_edges", embeddings, edges, labels, truth, self.config(), seed)
...
class IsolatesGenerator(AbstractGenerator[IsolatesParams]):
    ...
    def run(self, seed: int) -> Dataset:
        ...
        n_isolates = round(self.params.isolate_frac * n)
        permutation = rng.permutation(n)
        connected = np.sort(permutation[n_isolates:])

        groups = [
            connected[labels[connected] == cluster] for cluster in range(self.params.n_clusters)
        ]
        edges = _canonical_edges(_intra_group_edges(rng, groups, self.params.intra_degree))
        ...
        return Dataset("isolates", embeddings, edges, labels, truth, self.config(), seed)
...
"""Every shape except ``mixed``.

Mixed components are leaves, so "mixed inside mixed" is unrepresentable rather than merely
checked.
"""
...
class MixedComponent(BaseModel):
    ...
    # The component's ``n`` is ignored: component sizes are derived from ``weight`` over the
    # mixed generator's total ``n``.
...
class MixedParams(GeneratorParams):
    ...
    def check_consistent_dims(self) -> Self:
        # Components are concatenated into one embedding matrix, so every component generator
        # must agree on the embedding dimension.
        dims = {component.generator.params.dim for component in self.components}
        if len(dims) > 1:
            raise ValueError(f"mixed components must share one embedding dim, got {sorted(dims)}")
        return self
...
class MixedGenerator(AbstractGenerator[MixedParams]):
    ...
    def run(self, seed: int) -> Dataset:
        ...
        if any(size < 1 for size in sizes):
            raise ValueError(f"n={n} is too small for {len(components)} weighted components")
...
"""Discriminated union over all shapes; the union itself is the dispatch mechanism.

Suite YAML entries (``{shape, n, params}``) validate directly into generator instances; call
``.run(seed)`` on the result.
"""
...
def _union_members(annotation: object) -> Iterator[type[BaseModel]]:
    """Yield the concrete generator classes of a (possibly nested or aliased) union."""
    if isinstance(annotation, TypeAliasType):
        ...
        if not (isinstance(annotation, type) and issubclass(annotation, AbstractGenerator)):
            raise TypeError(f"unexpected generator union member: {annotation!r}")

        yield annotation


def generator_shapes() -> tuple[str, ...]:
    """Return the shape names, derived from the union (there is no registry)."""
    return tuple(
        sorted(member.model_fields["shape"].default for member in _union_members(Generator))
    )
