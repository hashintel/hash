...
Reads embeddings (common.matrix contract), L2-normalizes rows by default so
euclidean PCA geometry matches the battery's cosine neighbor truth,
projects to 2-D with a deterministic full-SVD PCA, and writes layout.npz
(common.layout contract).

``--edges`` / ``--labels`` are accepted and ignored: PCA is an
embeddings-only baseline. Its behavior with and without edges is identical,
so the no-structure-from-noise differential passes trivially — which is the
