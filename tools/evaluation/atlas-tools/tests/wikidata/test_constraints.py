...
def test_symmetric_and_transitive_parsed():
    constraints = parse_constraints(
        [_constraint("Q21510862"), _constraint("Q18647515")]
    )
...
def test_uniqueness_constraints_parsed():
    constraints = parse_constraints(
        [_constraint("Q19474404"), _constraint("Q21502410")]
    )
    assert constraints.single_value
    assert constraints.distinct_values


def test_subject_and_value_type_classes_parsed_from_p2308():
    constraints = parse_constraints(
        [
...
def test_inverse_constraint_parsed_from_p2306():
    constraints = parse_constraints([_constraint("Q21510855", {"P2306": ["P527"]})])
    assert constraints.inverse_pid == "P527"


def test_unknown_constraint_type_ignored_without_error():
    constraints = parse_constraints(
