...
from atlas_tools.battery.engine_runner import EngineFile, EngineSpec, load_engines
from atlas_tools.battery.harness import RunResult, run_suite
...
def engine_entries() -> dict[str, EngineSpec]:
    """All committed engine specs (default + adversarial), by name."""
    entries: dict[str, EngineSpec] = {}
    for filename in ("default.yaml", "adversarial.yaml"):
        for spec in load_engines(ENGINES_DIR / filename):
...
def write_engines_yaml(path: Path, names: list[str]) -> Path:
    ...
    engine_file = EngineFile(version=1, engines=[entries[name] for name in names])
    path.write_text(
        yaml.safe_dump(engine_file.model_dump(mode="json", exclude_none=True)),
        encoding="utf-8",
    )
    return path
