...
class TestPrefixTransform:
    def test_truncate_then_normalize_vector(self):
        # Known vector: prefix (3, 4) has norm 5 AFTER truncation, even though
        # the full vector has a large third component.
        x = np.array([[3.0, 4.0, 100.0]], dtype=np.float32)
        out = prefix_transform(x, 2)
        np.testing.assert_allclose(out, [[0.6, 0.8]], atol=1e-6)

    def test_normalization_happens_after_truncation(self):
        # Normalizing first then truncating would NOT give a unit vector.
...
class TestExactCosineKnn:
    def _brute_force(self, queries, corpus, k, exclude=None):
    ...
    def test_matches_brute_force_across_blocks(self):
        rng = np.random.default_rng(2)
        corpus = rng.standard_normal((500, 16)).astype(np.float32)
        queries = corpus[:40]
        # Tiny memory cap forces many blocks: b = cap // (4 * (q + d)).
        idx, scores = exact_cosine_knn(
            queries, corpus, 10, memory_cap_bytes=4 * (40 + 16) * 7
        )
    ...
    def test_block_size_independence(self):
        ...
        idx_small, _ = exact_cosine_knn(
            queries, corpus, 15, memory_cap_bytes=4 * (20 + 12) * 11
        )
        idx_large, _ = exact_cosine_knn(queries, corpus, 15)
