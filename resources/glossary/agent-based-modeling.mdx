...
ABMs consequently have the potential to be highly-explainable in a way that many forms of AI or modeling are not. And whilst most ABM platforms limit you to seeing only the last-computed step, or end-state of a model, [HASH](https://hash.ai/platform) saves all states of a [simulation](https://hash.ai/wiki/simulation) to allow for retrospective analysis and exploration. This makes witnessing emergent phenomena and understanding complex systems a breeze. [Read more about what agent-based modeling is >](https://hash.ai/blog/what-is-agent-based-modeling)
