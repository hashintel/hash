 ...
 */
test.skip("user can view page in read-only mode but not update", async ({ page }) => {
  ...
  await expect(blockRegion.locator('[data-testid="block-handle"]').first()).toBeVisible();
  ...
  await expect(blockRegion.locator("text=typing in read-only mode")).not.toBeVisible();

  await expect(blockRegion.locator('[data-testid="block-handle"]')).not.toBeVisible();
