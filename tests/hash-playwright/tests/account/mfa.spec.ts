...
import { clearSessionCookies, signInWithPassword } from "../shared/signin-utils";
...
const enableTotpForCurrentUser = async (page: Page) => {
  ...
  const secret = (await secretKeyLocator.textContent())?.replace(/\s/g, "") ?? "";
  if (!secret) {
    throw new Error("Could not read TOTP secret key from settings page.");
  }

  await waitForFreshTotpWindow();
  await page.fill('[placeholder="Enter your 6-digit code"]', generateTotpCode(secret));
  ...
  await expect(page.locator('[data-testid="disable-totp-button"]')).toBeVisible();
...
test("user with TOTP is prompted for code at login", async ({ page }) => {
  const credentials = await createUserAndCompleteSignup(page, testUsers.mfaLogin);
  ...
  await waitForFreshTotpWindow();
  await page.fill('[data-testid="signin-aal2-code-input"]', generateTotpCode(secret));
...
test("user can use backup code instead of TOTP", async ({ page }) => {
  const credentials = await createUserAndCompleteSignup(page, testUsers.mfaBackup);
  const { backupCodes } = await enableTotpForCurrentUser(page);
...
test("user can disable TOTP", async ({ page }) => {
  const credentials = await createUserAndCompleteSignup(page, testUsers.mfaDisable);
  ...
  await expect(page.locator('[data-testid="show-enable-totp-form-button"]')).toBeVisible();

  // Backup codes should also be cleared as part of the disable flow.
  await page.reload();
  await expect(page.locator('[data-testid="show-enable-totp-form-button"]')).toBeVisible();
  await expect(page.locator('[data-testid="regenerate-backup-codes-button"]')).not.toBeVisible();
  ...
  await expect(page.locator("text=Get support")).toBeVisible();
  await expect(page.locator("text=Enter your authentication code")).not.toBeVisible();
});

test("wrong TOTP code shows error at login", async ({ page }) => {
  const credentials = await createUserAndCompleteSignup(page, testUsers.mfaWrongCode);
  await enableTotpForCurrentUser(page);
