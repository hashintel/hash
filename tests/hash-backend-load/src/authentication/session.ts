...
export const refreshSessionToken: BeforeRequestFn<SessionContext, TracingContext> = async (
  request,
  context,
  events,
) =>
  startSpan("refresh-session-token", context, async () => {
    ...
    if (context.vars.session.expiresAt && context.vars.session.expiresAt < Date.now()) {
      ...
      events.emit("histogram", "kratos.reauthenticate_time", timeAfterReauthentication - startTime);
    }
