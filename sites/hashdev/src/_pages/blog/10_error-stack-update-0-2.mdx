...
subtitle: "Redesigned and customizable outputs, related errors and multiple sources, and much more"
author: "Tim Diekmann"
jobTitle: "Platform Engineer, HASH"
date: "2022-10-03"
---

Back in June [we announced the first version](https://hash.dev/blog/announcing-error-stack) of `error-stack`, our context-aware error library. We developed this to help address a lot of challenges we encountered when implementing error handling in our simulation engine [LINK]. In the time since then we’ve started using it in our entity-graph query layer [LINK?] that we’re building as a datastore for HASH [LINK?]. Across that development we’ve learned a lot of lessons, and in joint efforts with open-source contributors we’re ready to release a new version of the library.

Almost every week since our last release, we’ve been fixing user-reported bugs, reviewing open-source contributor’s pull requests, and adding new features. [Version 0.2](https://github.com/hashintel/hash/tree/error-stack%400.2.0/packages/libs/error-stack) is available now and includes important changes from `std`, a completely redesigned output with new customization options, support for related errors and multiple error sources, and a number of other improvements.
...
- If the root context already contains a backtrace, no new `Backtrace` is captured. Otherwise, backtraces are captured automatically when `RUST_BACKTRACE`.
  or `RUST_LIB_BACKTRACE` env-vars are set.
...
```text
Error: cxperiment error: could not run experiment
...
`error-stack` v0.2 introduces support for related errors and multiple error sources, which can be invaluable when multiple errors occur at the same time, e.g. in loops or in multi-threaded applications. `Report`s are no longer a linear stream of events. You can now create a tree of different `Report`s, enabling the capture of multiple simultaneous errors without loosing information.
...
We’ve got a lot of ideas! Among them are: adding support for `serde` to provide a hook-based interface to serialize `Report`s; adding support for `defmt`; adding support for WASM environments; and continuing to iterate on the ergonomics of how we support related errors and multiple sources. Please let us know what you think.

We’ll also be using many of the new features of `error-stack` v0.2 in our own products, namely HASH ([learn more](https://hash.ai/platform/hash) or [view on GitHub](https://github.com/hashintel/hash/tree/main/packages/hash)). The new hook system will greatly simplify the creation of useful reports, and the support for related errors will allow us not to return the error the first time it occurs, but to give the user a comprehensive list of errors.
