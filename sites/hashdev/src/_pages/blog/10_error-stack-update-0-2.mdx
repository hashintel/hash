...
- Compatibility for `anyhow` and `eyre` has been added to convert their types into `Report`.
  ...
  This should make it much easier to try out `error-stack` in existing codebases which utilize these libraries, as well as improving the migration process if desired.
