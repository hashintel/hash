...
**Note:** The implementation for the `Provider` RFC is not available on nightly at the time of writing, so we have vendored in the functionality for now. We will switch to the upstream implementation in `core::any`. **Using the `Provider` API currently requires the Rust nightly compiler.**
...
During development of the library we included a `Provider` implementation while waiting for the implementation of the [`Provider` RFC](https://rust-lang.github.io/rfcs/3192-dyno.html) to be merged. We will be updating the library to swap to the `core::any` implementation in version 0.2.0 (This is the reason why this API currently **requires the nightly release channel**).
