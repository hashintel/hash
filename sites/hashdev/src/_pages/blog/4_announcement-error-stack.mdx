---
authorPhoto: blog/4_announcement-error-stack/tim.jpg
postPhoto: blog/2_open-source/OpenSource_Cover.jpg
title: "Announcing error-stack"
subtitle: "A context-aware error library with arbitrary attached user data"
author: "Tim Diekmann"
jobTitle: "Backend Engineer at HASH"
date: "2022-06-10"
---

# **Announcing `error-stack`**

We are proud to announce a new error library for Rust: `error-stack` - a context-aware error library with arbitrary attached user data. **[View the crate on GitHub >](https://github.com/hashintel/hash/tree/main/packages/libs/error-stack)**

This post outlines our trials and tribulations in error-handling, why we ultimately built the crate, what architectural choices we made in doing so, and what you can expect from it.

# Our error-handling journey

At HASH, we've been building a simulation engine — written in Rust — called [hEngine](https://hash.ai/platform/engine). It's a large code base, with a number of contributors, and its size and complexity has grown over time (check out the code on [GitHub](https://github.com/hashintel/hash)). Across some big refactors, as well as everyday feature development, we found error handling became unwieldy and ourselves struggling to keep up. In a bid to improve our debugging process and reduce maintenance overheads we decided to revisit our strategy for managing errors.

Up until recently our approach to error handling centered around using [`thiserror`](https://crates.io/crates/thiserror), a fantastic crate which abstracts away a lot of Rust's verbosity when creating errors. We used to create enumerations (generally a large, top-level one, per crate and/or major modules) to describe our different error types, and used `thiserror`'s macros to handle a lot of the trait implementations. We didn't want to be using `Box<dyn Error>` everywhere, meaning we wanted typed errors, so we utilized `thiserror`'s ability to generate `From` implementations from a macro to handle the transformation of our error types across the module and crate boundaries. (`thiserror` lets you succinctly define a `From` implementation like so: `IoError(#[from] std::io::Error)`).

## Handling Errors across Module and Crate Boundaries

This led us into a pattern where we'd have a function called across a boundary (say from the `engine` module to the `simulation` crate), necessitating that we transform it between error types, and so for ergonomic reasons, we'd create a `From` implementation for the whole `Error` enumeration:
...
These error messages would tell us very little about the context that led to the error and weren't very helpful in actually tracking down the causes of problems. And while we have logging infrastructure in place, which helps with tracking down events that lead to errors, it relies on a few things:
...
Perhaps most importantly though, hEngine is a multi-threaded, multi-language project; this means that there will often be a mix of logs from various threads appearing before an error, with it being unclear as to which piece of logic caused the problem.
...
**Note:** The implementation for the `Provider` RFC was [merged just today](https://github.com/rust-lang/rust/pull/91970). We chose to not use the upstream implementation, for now, not to force users to update their compiler to the very last nightly, so we have vendored in the functionality for now. We will switch to the upstream implementation in `core::any` with version 0.2.0. **Using the `Provider` API currently requires the Rust nightly compiler.**
...
## Some other cool things about `error-stack` that we don't have time for!

We don't have space here to talk about all of the features of `error-stack`, but here are a few more teasers. [Check out the repository](https://github.com/hashintel/hash/tree/main/packages/libs/error-stack) for even more information:
...
We will switch to the provider implementation in `core::any` in version 0.2.0 (the main reason why this API currently requires the nightly release channel).
...
Contributions are greatly welcome. View the [project code on GitHub](https://github.com/hashintel/hash/tree/main/packages/libs/error-stack) and, if you like it, please star the repository!
