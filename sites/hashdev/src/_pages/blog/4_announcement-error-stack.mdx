...
3.  be able to attach _any_ data to an error, not just string-like types, which then can be requested when handling the error.
...
- _Note_: _It's still possible to create something like `Error::Unique` but we do not provide shortcut functionality for that within the library, hoping that a lot of its uses can instead be solved through “attachments”_
...
We will switch to the provider implementation in `core::any` as soon as it lands (the main reason why this API currently requires the nightly release channel).
