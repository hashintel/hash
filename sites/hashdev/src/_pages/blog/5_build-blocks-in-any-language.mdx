---
authorPhoto: blog/5_build-blocks-in-any-language/ahmad.png
postPhoto: blog/5_build-blocks-in-any-language/FSharp_Cover.jpg
...
The purpose of the Block Protocol ('BP') is to provide a standardized interface for any embedding application (‘EA'). This interface can be thought of as a data provider-consumer layer, like a backend for the block and a frontend for the embedding application. Writing our calculation block _once_ with the Block Protocol allows any BP-compliant EA to make use of it.
...
Read more about Entities, Entity Types and embedding applications in the Block Protocol Specification (update reference to v0.2 when live).
...
2.  translate any types from services that the block requires e.g. types for requests and responses in the Graph Service
3.  Implement the Block Protocol's message handling
4.  the implementation can be hidden between easy-to-use abstractions.
5.  send the messages your block needs to update or retrieve data, as specified in the Graph Service specification
...
In order to properly explain the response to this initial message, it's helpful to look at how the Graph Service Specification can be typed.

### Block Protocol Graph Service types

The Graph Service is a special service as the core specification uses it in `initResponse`s as part of initializing blocks. The service supplies an interface for managing Entities, Entity Types, and Links. Entity Types are _domain types_ defined in embedding applications, and Entities are instances of these.
...
When a block announces itself through an `init` request to the embedding application, an `initResponse` will be returned to the block with the shape `BlockProtocolMessage<Entity<'state>>`.
...
For the calculation block in particular, the `updateEntity`, `aggregateEntityTypes`, and `aggregateEntities` Graph Service messages are most important. These provide a way for the block to persists its internal state and fetch Entities and Entity Types from the embedding application. From the specification, these messages' types can be constructed as follows:
...
These types are all valid message structures for communicating with the Graph Service in the embedding application. We can for construct an example `AggregateEntityTypes` request through a `BlockProtocolMessage<AggregateEntityTypes>` and get the following record:
...
To receive events from the embedding application, it's necessary to use some of the same primitives as when you are sending events. We must listen on `"blockprotocolmessage"` events through a DOM element and handle them individually and asynchronously from dispatching.
...
Some details have been left out in the code listing to focus on the important bits. The `responseSettlerMap` is a mutable, shared state which is used on both ends of the communication to match up request and response pairs. Since promises are returned from dispatching events, the caller isn't blocked by issuing a Block Protocol request and the interface to interact with the Block Protocol is a lot like making network calls. Typing the requests and responses is just a matter of type coercion. An interface with the following shape will allow such type coercion:
...
For example, calling the `BlockProtocolState.aggregateEntityTypes` would create a new promise and add it to the settler map, then the listener would eventually receive a response if all goes well and resolve the unresolved promise. This promise will be coerced into the type of the return value `JS.Promise<AggregateEntityTypesResponse<unit>>`.
...
<img
  src="blog/5_build-blocks-in-any-language/mvu.png"
  width={1004}
  height={178}
...
<img
  src="blog/5_build-blocks-in-any-language/person_count.png"
  width={418}
  height={118}
...
<img
  src="blog/5_build-blocks-in-any-language/expression_math.png"
  width={290}
  height={123}
...
That's how you can use the Block Protocol with somewhat non-traditional tools: F# and Fable. We've seen how the Block Protocol isn't tied to JavaScript, TypeScript, and React, and how you can build a block with your favorite JavaScript transpilation source without changing the way you write frontend code. The Elmish component itself doesn't have any special knowledge about the Block Protocol, relying instead on a couple of asynchronous functions which disguise a simple way of communicating through DOM events.

If you're interested in learning more, take a look at [the Block Protocol specification](https://blockprotocol.org/spec) and [the calculation block source code on GitHub](https://github.com/hashdeps/calculation-table-block).

If you're interested in building a block in a novel way, we'd love to [hear from you on Discord](https://hash.ai/discord).
