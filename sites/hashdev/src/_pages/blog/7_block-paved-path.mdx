...
In this case, we would declare the type “book” and declare it's properties and their expected [types](https://hash.ai/glossary/types). So we would expect a book to have a _title_ which is going to be a `Text` string, and an _ISBN number_, which is going to be a `Number`.
...
Most of you will have encountered blocks in a knowledge-management app like _Notion_, which have (arguably) been responsible for a huge boom in awareness and popularity of blocks.
