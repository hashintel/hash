...
A `Graph` contains all of the data within a single HASH `Instance`.

A `Subgraph` represents some _part_ (a strict subset) of the whole graph, and is returned by a `Structural Query`.
...
A `depth` of `0` implies that only the `root` entities will be returned for a given query. A depth of `1` would also return any entities connected by a link to any `root` entity. A depth of `2` would additionally return any entities connected to those entities (and so on). Depending on the shape of the data in a `Graph`, a high `depth` parameter can exponentially increase the amount of information returned by any given query, and great care should be taken.
...
1. The `EntityId`
2. Any two of `EditionId`, `TransactionTime`, or `DecisionTime`
