...
At least one of the `User`s associated with an `Organization` must be designated as the `Administrator` of the `Organization`, granting them the ability to administrate the `Organization` in HASH.
