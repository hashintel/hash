...
- `EntityId`: Primary identifier for an entity, encoded as `{WebId}~{EntityUuid}`
  - `WebId`/`WebId` (both terms mean the same): Identifies a 'Web', currently part of the primary key for entities but likely to be moved to entity metadata
  - `EntityUuid`: a UUIDv4
