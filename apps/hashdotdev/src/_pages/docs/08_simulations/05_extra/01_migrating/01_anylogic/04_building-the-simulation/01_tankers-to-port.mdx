...
On this agent, and many others in the sim, we'll use a published behavior, [Lng_Lat to Position](https://simulation.hash.ai/@hash/ll-to-pos/overview), to translate its latitude and longitude into a 3D position.

<Hint style="warning">
  To use a shared behavior like Lng_Lat To Position, [you'll need to import it
  into your
  simulation](/docs/simulation/creating-simulations/behaviors/composable-behaviors#importing-a-behavior).
</Hint>
...
To give the **Port** unloading behavior, we'll make use of a published behavior in hIndex called [In Flow](https://simulation.hash.ai/@hash/in-flow). If we look at its documentation \[here\] we can see what properties we need to initialize our **Port** with. It's not enough just to add this behavior to **Port** agents, since the **Tankers** need to cooperate during the unloading process. We'll give them the complementary [Out Flow](https://simulation.hash.ai/@hash/out-flow) behavior.
