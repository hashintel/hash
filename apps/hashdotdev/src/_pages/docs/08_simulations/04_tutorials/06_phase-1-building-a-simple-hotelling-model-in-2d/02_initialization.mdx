...
Now, open up init.json. This is where we will be writing our [agent creator](/docs/simulations/create/anatomy-of-an-agent/initial-state) that will generate all our simulation agents. First, generate the Customer agents.
...
<Hint style="info">
  Since our agent creator is first defined in `init.json`, which can't run
