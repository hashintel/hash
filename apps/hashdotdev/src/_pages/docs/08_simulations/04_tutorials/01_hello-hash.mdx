...
<Hint style="info">
  `agent_name` is a [reserved keyword on
  agents](/docs/simulations/create/anatomy-of-an-agent) - you can
  reference and send messages through `agent_name`
</Hint>

Beneath the workspace's right-hand view-pane, you'll find the simulation control buttons. Click the **Reset** button \(circular arrow\) and then select the [Raw Output](/docs/simulations/create/views/raw-data#raw-output) tab in the viewer. Congratulations! You've given life to Alice and Bob.
...
Alice and Bob aren't very interesting right now. Let's teach them some manners. We can give the agents [behaviors](/docs/simulations/create/behaviors) that enable them to act and respond to each other. In `init.json` let's add some file names into each of the behavior arrays.

<Hint style="info">
  You can build Python behaviors instead of JavaScript behaviors if you prefer.
  Just make sure to name your files with a ".py" ending.
</Hint>
...
HASH has built in support for [message](/docs/simulations/create/agent-messages/) passing. Push a message object to an agent's message array, and it will deliver the message to the target agent in the next time step. The message array attached to an agent's state is like its outbox.
...
<Hint style="info">
  [_state.addMessage is a helper
  function_](/docs/simulations/create/agent-messages/) _for
  pushing_ [_messages_](/docs/simulations/create/agent-messages/)
  _to the outgoing messages array._
</Hint>
...
To prevent our agents from straying too far, we can set [bounds](/docs/simulations/configuration/topology/bounds-and-wrapping) on their environment in the `globals.json` file as follows:
