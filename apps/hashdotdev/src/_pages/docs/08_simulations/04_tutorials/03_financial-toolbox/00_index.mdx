...
HASH makes agent-based modeling accessible through our browser-based editor ([hCore](https://hash.ai/platform/core)), and simulation platform ([hCloud](https://hash.dev/docs/simulations/create/h.cloud)). A large number of our users are interested in finance and crypto, so we decided to build a _financial toolbox_ that makes it easier for anybody to get started using HASH for financial and economic modeling.
