---
title: Agents
slug: simulation/create/anatomy-of-an-agent
...
<Embed
  url="https://youtu.be/PTW6R-PrT38"
  caption="Anatomy of an Agent"
  type="youtube"
/>
...
The HASH Engine by default will set the `agent_id`, so don't worry about setting it manually unless you have a particular need for an agent's id [value in the sim logic](/docs/simulations/create/libraries/hash/agent#generateagentid).
...
An individual agent has a [state](/docs/simulations/create/anatomy-of-an-agent/state) and a [context](/docs/simulations/create/anatomy-of-an-agent/context).

![An Agent](https://cdn-us1.hash.ai/site/docs/image%20%2814%29.png)

When we define the [initial conditions](/docs/simulations/create/anatomy-of-an-agent/initial-state) of a simulation, we're defining the initial agents that will be present in the first timestep of the simulation, each of which will have its own state and context.
