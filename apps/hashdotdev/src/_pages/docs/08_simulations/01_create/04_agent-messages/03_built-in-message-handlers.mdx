...
slug: simulation/create/agent-messages/built-in-message-handlers
...
<Hint style="warning">
  **Case sensitivity matters.** If a message is sent to `bill`, it will not be
  received by `Bill`. The only exception to this rule is that any message sent
  to hASh, Hash, HASH, haSh, or any similar variant will be forwarded to the
  engine.
</Hint>
...
- [Citi Bike Visualization](https://simulation.hash.ai/@hash/citi-bike-visualization)
- [Product Delivery](https://simulation.hash.ai/@hash/product-delivery)
