...
Complex systems are typically extremely sensitive to their initial conditions. Ensuring an as close to accurate starting or "initial state" is therefore key. Pre-structured data, especially if itself tested and reviewed, or produced in accordance with a well-understood process or pipeline, can improve confidence in this.
...
All this serves to highlight an interesting possibility. If to date structured data has existed for the benefit of _machines_, is it possible that in the future its primary use will in fact me to us, _humans_, in verifying and validating the outputs of machine-generated models: simulation, language, multimodal, or other?
...
1. **Entites** (semantic 'things' in a database) have entity types.
  - **Entity types** describe the _properties_ and type(s) of any outgoing _link entities_ that can be associated with an entity.
  - **Link entities** are the relationships, or 'things' that connect other entities. For example, one `Person` entity may be connected by a linked entity called `Spouse` to another `Person`.
...
2. **Properties** contain data associated with some particular aspect of an entity.
  - **Property types** describe the possible values can be associated with a property, by way of reference to _data types_ and other properties.
3. **Data types** describe a space of possible valid values (e.g. `string`, `number` or `boolean`).
  - Support for defining "custom" data types, i.e. "non-primitive" data types which are not found in the Block Protocol's [graph module specification](https://blockprotocol.org/docs/spec/graph) is coming soon.
...
In all agent-to-agent (human-human, machine-machine, and human-machine) communications, ontologies of types enable improved mutual understanding by formalizing and standardizing definitions of things and streamlining communication around them.
