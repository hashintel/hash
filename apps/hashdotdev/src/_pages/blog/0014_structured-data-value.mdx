...
Inherently tied to the idea of structured data is the idea of **typing** information. Data may be of a certain 'type' if it follows a set of rules established by that type.
...
- A **link entity** corresponds to the relationship, or 'thing' that connects other entities. For example, a standard entity `Person` may be connected by a link entity called `Spouse` to another standard entity `Person`.
...
3.  **Data types** describe a space of possible valid values (e.g. `string`, `number` or `boolean`). They allow for validation logic to be shared amongst different kinds of property types which represent semantically different things.

- Support for user-definition of "non-primitive" data types (e.g. numbers within a certain range, or strings matching a certain pattern) which are not already found in the Block Protocol's [graph module specification](https://blockprotocol.org/docs/spec/graph) is planned ([draft RFC](https://github.com/blockprotocol/blockprotocol/pull/355)).
