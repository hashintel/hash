...
At HASH, we're building an open-source platform for integrating vast amounts of data and using it in new and interesting ways. We're most commonly contrasted to systems like Palantir, and monolithic, legacy ERPs (none of which are open-source). Given their opacity and our breadth, actually _explaining what we do_ can sometimes be a bit of a headache, especially when our users deploy HASH for applications that themselves are incredibly diverse in nature, and often appear to have little in common on the surface.

In a bid to improve understanding, we're moving more of our documentation into public, starting with our **technology tree**, which you can find on our [development roadmap](https://hash.dev/roadmap).
...
Unlike in games, we can also attack work in parallel. There may be natural synergies between threads, meaning multiple can be addressed at reduced cost.
...
A good tech tree should be high-level, ideally describing the features relevant to the audience consuming it. In our case, buliding an open-source platform, with a public tech tree, we have to grapple dually with developers looking at the tree, and end-users, so ours contains both a combination of technical infrastructure that blocks feature-work, and the features themselves.

Whether your tech tree is an aspirational internal document or a public artefact like ours, you'll want to keep it up-to-date. Ours is new (and therefore currently manual), but we'll be linking it to our internal product management tickets via Linear sync with HASH shortly (during preparation for which we discovered a fun [data exposure vulnerability](https://twitter.com/nonparibus/status/1692255276550742453) in Linear).
...
Outside of video games, we'ee not really aware of any examples of tech trees in the wild, used for real-world product planning. This might be because roadmaps visualized as boards with a column-per-quarter do everything people need, but we find them pretty boring, and by far from the best way to visualize the dependencies between feature-work, and understand the big picture. For most products, that's probably fine, but for a big thing like HASH, it lacks something we require. If you're familiar with projects who've got their own public tech trees, or if you've worked on one internally, let us know over on our [developer Discord](https://hash.ai/discord).

If an "open-source alternative" to a tool like Palantir sounds of interest to you (or your org!), please feel free to get in touch [via the website](https://hash.ai/contact) or by dropping an email to <a href="mailto:hey@hash.ai?subject=Follow-up from tech trees blog post">hey@hash.ai</a>
