...
<a id="#text" />
Blocks for capturing and/or displaying text are the most commonly used kinds of blocks
in HASH
...
<a id="#media" />
Blocks that are used to display or support interacting with multimedia content such
as images, videos, or 3D models.
...
<a id="#action" />
Action blocks can be programmed to respond to user behavior, allowing for the creation
of interactive "Apps" in HASH
...
<a id="#custom" />
There's no limit to what a block can do. For example, there are interactive gaming
blocks, such as the Minesweeper block, as well as AI Chat blocks.
...
<Callout style="information" type="coming-soon">
  We'll be replacing block configuration bars with a much more powerful "block
  sidebar", which will appear on the right-hand side of any
  [document](/guide/pages/documents) and at the bottom of a
  [canvas](/guide/pages/canvases), when a block is selected.
</Callout>
