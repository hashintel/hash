...
1. by conerting an existing entity type to a link type, using the context menu on an entity type's page (accessed by clicking the three vertical dots near the top-right of the page).

# Inheritance

Links can extend other links. This can be useful for capturing additional or more specific information about a relationship, or enforcing stricter [constraints](/guide/entities/attributes#constraints).

# Multi-Type Links

A link can have more than one link type associated with it. This is relatively uncommon, and typically link types will extend other link types (to make them "more specific") instead. A link with multiple types can, however, be useful to denote that two relationships are tied.
