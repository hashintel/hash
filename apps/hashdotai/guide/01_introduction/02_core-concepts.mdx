...
All users and orgs have an account name, such as **@example**. This can be used to access an account's public profile, such as at [hash.ai/@example](/@example).
...
- An <ins>entity type</ins> which describes “what they are” and what kinds of attributes may be associated with them
- Any number of <ins>attribute values</ins> which contain an entity’s properties and links (structured data)
- Any number of <ins>observations</ins> (facts, statements or beliefs about an entity that don’t fit into its existing/available attributes)
- Any number of <ins>notes</ins>, which allow completely unstructured information to be associated with an entity.
</ParagraphIconWrapper>
...
[Attributes](/guide/entities/attributes) are structured information stored on an entity’s record. They are either **properties** (which contain data) or **links** (which point to other entities).
</ParagraphIconWrapper>

<ParagraphIconWrapper boxContent={<LinkIcon />}>
## Link Types

A <ins>link type</ins> describes a link attribute, giving it a label and providing it with meaning.

The specific (or type of) entities that are allowed to be stored in a link may also be constrained through the link’s type.

**Example link type:**  <code>Employed By</code>
</ParagraphIconWrapper>

<ParagraphIconWrapper boxContent={<TagIcon />}>
## Property Types

A <ins>property type</ins> provides a semantic description about what a value is, and outlines the <ins>data types</ins> (formats of data) that it accepts.

**Example property type:**  <code>Temperature</code>
</ParagraphIconWrapper>

<ParagraphIconWrapper boxContent={<BarcodeIcon />}>
## Data Types

While property types describe data semantically, data types define data structurally, and specify what label(s) should be used in conjunction with it.

**Example data type:**  <code>Celcius</code> or <code>Farenheit</code>
</ParagraphIconWrapper>
...
HASH <ins>workers</ins> are AI agents which work for you. They perform tasks on your behalf, helping grow your web with useful information, and keep it up to date.
...
Workers can be given <ins>goals</ins>. When provided with a goal, available workers will analyze your aims, develop a plan, and execute it autonomously upon your behalf, much as a capable human colleague might.

Right now, HASH lets you provide **research and analysis** goals (with support for more planned in the future).
</ParagraphIconWrapper>
...
<ins>Plugins</ins> allow you to interact with your web within other applications, and typically provide HASH with additional capabilities.

For example, installing the <ins>HASH browser extension</ins> lets you (optionally) allow workers to browse websites using any accounts you may be logged into. This can let them bypass paywalls, and access other protected content.
</ParagraphIconWrapper>
