...
The open-source and newly-released _HASH Physics Library_ provides behaviors that enable the easy addition of simple physics to simulations in HASH. The library is written in Rust, which means that it's optimized to run in our engine. The HASH docs contain more information about [each of the behaviors](https://hash.ai/docs/simulation/concepts/physics).
