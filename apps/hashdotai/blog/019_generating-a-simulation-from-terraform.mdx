...
By way of lightweight demonstration, above you'll see a [simulation that takes in a Terraform configuration](https://simulation.hash.ai/@hash/terraform-simulation) alongside a set of user data, and creates a Kubernetes cluster. This leverages HASH's [support for datasets](https://hash.dev/docs/simulation/creating-simulations/datasets) to define a unique simulation based off of the user's configuration files:
