...
There are three types of agents in the model. In our example  they are all initialized in random locations across the simulation grid. Real-world policymakers may choose to use actual location data in conjunction with HASH’s [geospatial](https://hash.dev/docs/simulations/creating-simulations/views/geospatial-viewer) functionality.
