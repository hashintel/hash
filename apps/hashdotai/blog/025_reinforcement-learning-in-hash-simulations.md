...
We've created a demonstration in HASH using a simple reinforcement learning algorithm, called Q-Learning, that agents can use to navigate a two-dimensional environment, avoid obstacles, and get to an end goal. You can follow along in our [example "Map Explorer" simulation](https://hash.ai/@hash/q-learning-map-explorer), or use the [Q-Learning behavior library](https://hash.ai/@hash/qrl) we've published to experiment with it in your own RL simulations.
