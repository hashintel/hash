...
Changing the state can change the 3D appearance of an agent, using [reserved fields](https://hash.dev/docs/simulations/creating-simulations/anatomy-of-an-agent/state).
...
We can model a strategy in HASH through writing a [behavior](https://hash.dev/docs/simulations/creating-simulations/behaviors). Behaviors in HASH are functions attached to agents called at every time step. Behaviors take in the agent’s mutable state and context, and return an updated state.
...
The grid layout is the simplest of many layouts of agents that can be simulated in HASH. In this simulation, we have imported the `@hash/create_grids.js` behavior to set up the grid of agents in 3D space. The behavior is part of the [HASH standard library](https://hash.dev/docs/simulations/creating-simulations/libraries).
...
Agents can also communicate with each other via use of [messages](https://hash.dev/docs/simulations/creating-simulations/agent-messages). Messages are sent via agents before the computation of every time step. This allows for more complex coordination between agents.
...
[Learn more about creating experiments in HASH >](https://hash.dev/docs/simulations/creating-simulations/experiments)
...
That's it for this post. If you want to take the ideas mentioned here course further, please check out any of the [open-source simulations](https://hash.ai/models) built on HASH which are available for download, or read the [HASH simulation-development docs](https://hash.dev/docs/simulations).
