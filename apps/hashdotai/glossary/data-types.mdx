...
**Data types** describe a space of possible valid [values](/glossary/values). For instance, the _string_ value `foo` might be an instance of a `Text` data type, or a _float_ value of `0.6` might be an instance of a `Number` data type.
...
The [Block Protocol](/glossary/block-protocol) specifies a set of primitive top-level data types, which we use in [HASH](/platform).
