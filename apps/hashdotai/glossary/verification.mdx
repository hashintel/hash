...
Processes in HASH are represented as what mathematicians call "_Petri nets_". Unlike most representations of process models, Petri nets can be formally verified, which means we can check their integrity in a number of different ways, including by looking at properties such as:
...
Right now, process modeling in HASH is in beta, and we'll be building out these formal verification tools in the near future.
