...
Closed-greybox sharing is similar to blackbox sharing, except that users are provided with **the ability to vary a fixed set of parameters** that feed a simulation (usually a simulation’s [global variables](https://hash.ai/docs/simulation/configuration)).
...
Output-only sharing refers to the distribution of certain specified simulation outputs only (i.e. single values of specific metrics tracked during a simulation run). The “entire run” of a simulation will not be visible, and only the individual metric tracked will be made available. Typically this is accessed via an Application Programming Interface, such as the [HASH API](https://hash.ai/docs/simulation/api).
...
- **Output-only** sharing is available as part of HASH’s pre-release program ([register for access](https://hash.ai/docs/simulation/api/register-for-access)).
