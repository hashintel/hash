...
import { getUserFromEntity, getUserVerifiedEmails } from "../../../system-types/user";
...
const validateAccountShortname = async (
  ...
  if (shortnameContainsInvalidCharacter({ shortname })) {
    throw Error.badUserInput("Shortname may only contain letters, numbers, - or _");
  }
...
export const userBeforeEntityUpdateHookCallback: BeforeUpdateEntityHookCallback = async ({
  previousEntity,
  propertyPatches,
  context,
  authentication,
  ...
  for (const patch of propertyPatches) {
    ...
    if (targetBaseUrl === undefined) {
      throw Error.badUserInput("Cannot replace the entire property object on a user entity");
    }
    ...
    if (
      ...
      throw Error.badUserInput(`Cannot update property '${targetBaseUrl}' on a user entity`);
    }
  }
  ...
  const getNewValueForPath = getDefinedPropertyFromPatchesGetter<UserProperties>(propertyPatches);

  const currentEmails = user.emails;

  const updatedEmails = getNewValueForPath("https://hash.ai/@h/types/property-type/email/");

  if (updatedEmails && updatedEmails.sort().join(",") !== currentEmails.sort().join(",")) {
  ...
  if (
    updatedFeatureFlags &&
    updatedFeatureFlags.sort().join(",") !== currentFeatureFlags.sort().join(",") &&
  ...
  const updatedShortname = getNewValueForPath("https://hash.ai/@h/types/property-type/shortname/");
  ...
  if (updatedShortname) {
    if (currentShortname && currentShortname.toLowerCase() !== updatedShortname.toLowerCase()) {
  ...
  const isDisplayNameRemoved = isValueRemovedByPatches<UserProperties>({
    baseUrl: "https://blockprotocol.org/@blockprotocol/types/property-type/display-name/",
    propertyPatches,
  });
  if ((updatedDisplayName !== undefined && !updatedDisplayName) || isDisplayNameRemoved) {
  ...
  if (isIncompleteUser && (updatedShortname || updatedDisplayName)) {
     ...
     */
    const accessResult = await userHasAccessToHash(context, authentication, user);
    ...
    if (onlyForEmails.length > 0) {
      ...
      throw Error.forbidden("You must verify your email address before completing account setup.");
    }
