...
import { getUserFromEntity, updateUserKratosIdentityTraits } from "../../../system-types/user";

import type { AfterUpdateEntityHookCallback } from "../update-entity-hooks";
import type { UserProperties } from "@local/hash-isomorphic-utils/system-types/user";

export const userAfterUpdateEntityHookCallback: AfterUpdateEntityHookCallback = async ({
  context,
  propertyPatches,
  updatedEntity,
}) => {
  ...
  const hasEmailChanged =
    updatedEmails && updatedEmails.sort().join(",") !== email.sort().join(",");
