import { createSystemEntityTypeIfNotExists, getCurrentHashSystemEntityTypeId } from "../util";
...
const migrate: MigrationFunction = async ({ context, authentication, migrationState }) => {
  ...
  const documentFileEntityType = await createSystemEntityTypeIfNotExists(context, authentication, {
  ...
  const _pdfFileEntityType = await createSystemEntityTypeIfNotExists(context, authentication, {
    entityTypeDefinition: {
  ...
  const _docxFileEntityType = await createSystemEntityTypeIfNotExists(context, authentication, {
    entityTypeDefinition: {
      allOf: [documentFileEntityType.schema.$id],
      title: "DOCX Document",
  ...
  const slideshowFileEntityType = await createSystemEntityTypeIfNotExists(context, authentication, {
    entityTypeDefinition: {
      allOf: [currentFileEntityTypeId],
  ...
  const _pptxFileEntityType = await createSystemEntityTypeIfNotExists(context, authentication, {
    entityTypeDefinition: {
