...
test.each([
  ...
  "relocated v1 categorically rejects the frozen campaign %s path",
...
test("relocated v1 categorically rejects a symlink into the frozen campaign", async () => {
  const directory = await mkdtemp(join(tmpdir(), "brunch-v1-baseline-link-"));
