...
test.each(["mutate_petrinaut_net", "apply_petrinaut_construction"])(
  "app registration classifies %s as a browser tool",
  async (browserToolName) => {
    ...
    await expect(
      registration!.interceptor(
        {
          type: "agent",
          operationId: `${browserToolName}-classification`,
