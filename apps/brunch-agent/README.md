...
The first step builds the Petrinaut libraries the panel imports (`dist/` and design-system codegen). Then it starts the Brunch server at `http://127.0.0.1:4321` and the real Petrinaut website at `http://127.0.0.1:4915`. The website proxies `/agents/chat/*` to Brunch without changing the request origin or Flue protocol. The typed panel and Voice mode initialize the agent in canonical Petrinaut mode: the server uses Petrinaut's stock AI prompt and mounts the complete stock `petrinautAiTools` catalogue with its canonical names, descriptions, and input schemas. The browser executes those tools through Petrinaut's stock handlers; Brunch does not add its workpiece, Ledger, elicitation, or deployment-diagnostic tools to this product mode.

Two non-product composition boundaries remain deliberately available. Batched construction retains the Brunch workpiece and Ledger/provenance path, including observed reads, ordered `mutate_petrinaut_net` batches, diagnostics, layout records, freshness, and explanation tools; it is retained for Ledger behavior and compatibility tests, not mounted by the product panel. Validated construction retains the smaller construct-only tool surface used by the headless runbook evaluator. Neither retained path changes the canonical product catalogue.
