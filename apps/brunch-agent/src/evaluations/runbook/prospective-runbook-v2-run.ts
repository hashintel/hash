/**
 * Candidate v2 prospective elicitation campaign.
 *
 * This is intentionally separate from the immutable Mission 3 v1 runner.
 * Build first, then run one explicitly numbered replication:
 *   BRUNCH_RUNBOOK_REPLICATION=1 \
 *     yarn workspace @apps/brunch-agent runbook:elicit:candidate-v2
 */
...
const PROTOCOL_ID = "prospective-runbook-v2";
const OUTPUT_NAMESPACE_ID = "vestera-prospective-candidate-v2";
...
const candidateOutputDirectory = fileURLToPath(
  new URL(
    "../../../../../libs/@hashintel/brunch-agent/docs/evidence/evaluations/vestera-prospective-candidate-v2/",
    import.meta.url,
  ),
);
...
if (isHermeticRun) {
  ...
  if (outputDirectory === canonicalCandidateOutputDirectory) {
    throw new Error(
      "Hermetic runs cannot write to vestera-prospective-candidate-v2.",
    );
  }
  if (apiKey) {
  ...
  if (!usesFrozenConfiguration) {
    throw new Error(
      "Paid candidate runs require the frozen prospective-runbook-v2 model, turn, and latency configuration.",
    );
  }
...
try {
  ...
  process.stdout.write(
    `PROSPECTIVE_RUNBOOK_V2_RESULT ${JSON.stringify({
      protocolId: PROTOCOL_ID,
      outputNamespaceId: OUTPUT_NAMESPACE_ID,
      replication,
      stopReason,
  ...
  process.stderr.write(
    `PROSPECTIVE_RUNBOOK_V2_FAILURE ${JSON.stringify({
      runId,
  ...
  if (cleanupFailures.length > 0) {
    ...
    process.stderr.write(
      `PROSPECTIVE_RUNBOOK_V2_CLEANUP_FAILURE ${JSON.stringify({
        runId,
