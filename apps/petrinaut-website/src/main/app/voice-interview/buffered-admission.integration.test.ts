...
import { runNodeScript } from "../../../../../brunch-agent/test/integration/run-node-script";
...
beforeAll(async () => {
  const { exitCode, stdout, stderr } = await runNodeScript(
    join(
      testDirectory,
      "../../../../../brunch-agent/test/integration/admission-controls.integration.ts",
    ),
    join(testDirectory, "../../../../../.."),
    {},
  );
...
test("buffered production output remains silent until approved and derives the whole finalized turn", () => {
  ...
  expect(completed.questionSegment?.text).toBe(
