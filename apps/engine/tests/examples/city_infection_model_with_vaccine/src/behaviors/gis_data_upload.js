...
function behavior(state, context) {
  ...
  let gis_dataset = context.data()["@b/property_data/sf900homes100offices.csv"];
  let gis_data = JSON.parse(JSON.stringify(gis_dataset));
  ...
  json_data = json_data.filter((d) => !(isNaN(d.lat) || isNaN(d.lon)));
  json_data = ll_array(json_data, x_bounds, y_bounds);

  let state_agents = state.get("agents");
  let scatter_templates = state.get("scatter_templates");

  state_agents = state_agents ? state_agents : {};
  ...
  scatter_templates.forEach((template) => {
    ...
    state_agents[name] = agents;
  });

  state.set("agents", state_agents);
  state.set("scatter_templates", scatter_templates);
  console.error(state);
}
