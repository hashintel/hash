...
test.skip(
  ...
  async () => {
    ...
    const allDereferencedEntityTypesById = await getDereferencedEntityTypesActivity({
      entityTypeIds: ["https://hash.ai/@h/types/entity-type/graphics-card/v/1"],
      graphApiClient,
      actorId: userAuthentication.actorId,
    });

    const { rejectedDelegatedTasks, acceptedDelegatedTasks } = await checkDelegatedTasksAgent({
      input: {
        humanInputCanBeRequested: false,
        prompt: "Find the best 3 consumer graphics cards for running AI models.",
    ...
    console.log(JSON.stringify({ rejectedDelegatedTasks, acceptedDelegatedTasks }, null, 2));
