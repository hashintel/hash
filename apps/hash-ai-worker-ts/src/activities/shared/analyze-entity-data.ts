...
export type AnalyzeEntityDataParams = {
...
export type AnalyzeEntityDataResult = {
  pythonScript: string;
  chartData: unknown[];
...
/**
 * Analyze entity data by executing a query and transforming the results using Python.
 */
export const analyzeEntityData = async (
  params: AnalyzeEntityDataParams,
): Promise<AnalyzeEntityDataResult> => {
  const {
    structuralQuery,
    userGoal,
    targetChartType,
  ...
  let suggestedChartType: ChartType = targetChartType ?? "bar";
  let explanation = "";
  ...
  const callModel = async (
    ...
    const llmResponse = await getLlmResponse(
      ...
      {
        customMetadata: {
          stepId: stepId ?? "analyze-entity-data",
  ...
  return { pythonScript, chartData, suggestedChartType, explanation };
};
