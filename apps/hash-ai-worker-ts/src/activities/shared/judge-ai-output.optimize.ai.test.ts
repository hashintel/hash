...
import { judgeAiOutputs, type JudgeCorrection, judgeSystemPrompt } from "./judge-ai-outputs.js";
...
const metrics: MetricDefinition[] = judgeTestData.map((testItem): MetricDefinition => {
  return {
    ...
    executeMetric: async ({ testingParams }) => {
      ...
      const { alreadyCorrectFieldPaths, nonInferrableFieldPaths, expectedCorrections } = evaluation;

      const rewardPerCorrection = 0.5 / expectedCorrections.length;
      const penaltyPerMistake =
        0.5 / (alreadyCorrectFieldPaths.length + nonInferrableFieldPaths.length);
      ...
      for (const correction of corrections) {
        const isAlreadyCorrect = evaluation.alreadyCorrectFieldPaths.some((jsonPath) =>
          isEqual(correction.jsonPath, jsonPath),
        );
        ...
        const isNonInferrable = evaluation.nonInferrableFieldPaths.some((jsonPath) =>
          isEqual(correction.jsonPath, jsonPath),
        );
        if (isNonInferrable) {
        ...
        const providedValueIsCorrect = applicableCorrection.isProvidedValueCorrect(
        ...
        if (providedValueIsCorrect) {
          ...
          wrongValueCorrections.push(
            `${correction.jsonPath.join(".")}: ${stringifyPropertyValue(correction.correctValue)}`,
          );
          /**
           * technically applying penalties here lets the score go below 0,
           * because penaltyPerMistake doesn't take account of provided corrections that are wrong.
