...
import { orcarouterModelToContextWindow, orcarouter } from "../orcarouter-client.js";
...
const mapLlmToolDefinitionToOrcaRouterToolDefinition = (
...
const mapOrcaRouterFinishReasonToLlmStopReason = (
  ...
  switch (finishReason) {
    ...
    default:
      throw new Error(`Unexpected OrcaRouter finish reason: ${finishReason}`);
  }
};
 ...
 */
const convertOrcaRouterTimeStringToMilliseconds = (timeString: string): number => {
...
const getWaitPeriodFromHeaders = (headers?: Headers): number => {
  ...
  if (!tokenReset) {
    return convertOrcaRouterTimeStringToMilliseconds(requestReset!);
  }
  if (!requestReset) {
    return convertOrcaRouterTimeStringToMilliseconds(tokenReset);
  }
  return Math.max(
    convertOrcaRouterTimeStringToMilliseconds(requestReset),
    convertOrcaRouterTimeStringToMilliseconds(tokenReset),
  );
};
...
const orcaRouterChatCompletionWithBackoff = async (params: {
  ...
  try {
    return await backOff(
      () =>
        orcarouter.chat.completions.create(completionPayload, {
        ...
        }),
      {
        ...
        retry: (error) => {
          ...
          if (isServerError(error)) {
            logger.debug(
              `Encountered server error with OrcaRouter, retrying with exponential backoff.`,
            );
            return true;
          }
    ...
    if (
      ...
      logger.debug(
        `Encountered rate limit error with OrcaRouter provider, delaying retry request until the rate limit wait period has ended.`,
      );
      ...
      return orcaRouterChatCompletionWithBackoff({
        completionPayload,
        metadata,
...
export const getOrcaRouterResponse = async <ToolName extends string>(
  params: OrcaRouterLlmParams<ToolName>,
  metadata: LlmRequestMetadata,
): Promise<{
  llmResponse: LlmResponse<OrcaRouterLlmParams<ToolName>>;
  ...
  const openAiTools = tools?.map(mapLlmToolDefinitionToOrcaRouterToolDefinition);
  ...
  if (typeof trimMessageAtIndex === "number") {
    const modelContextWindow = orcarouterModelToContextWindow[params.model];
  ...
  let orcaRouterResponse: OpenAI.ChatCompletion;

  const timeBeforeRequest = Date.now();

  try {
    orcaRouterResponse = await orcaRouterChatCompletionWithBackoff({
      completionPayload,
      metadata,
    });
  } catch (error) {
    logger.error("OrcaRouter API error", { error });
  ...
  const usage: LlmUsage = {
    inputTokens:
      ...
      (orcaRouterResponse.usage?.prompt_tokens ?? 0),
    outputTokens:
      (previousUsage?.outputTokens ?? 0) +
      (orcaRouterResponse.usage?.completion_tokens ?? 0),
    totalTokens:
      (previousUsage?.totalTokens ?? 0) +
      (orcaRouterResponse.usage?.total_tokens ?? 0),
  };
   ...
   */
  const firstChoice = orcaRouterResponse.choices[0];

  const stopReason = !firstChoice
    ? "stop"
    : mapOrcaRouterFinishReasonToLlmStopReason(firstChoice.finish_reason);
  ...
  const retry = async (retryParams: {
    ...
    llmResponse: LlmResponse<OrcaRouterLlmParams>;
  }> => {
    ...
    const orcaRouterResponseMessage = orcaRouterResponse.choices[0]?.message;

    const responseMessages = orcaRouterResponseMessage
      ? mapOpenAiMessagesToLlmMessages({
          messages: [
            {
              ...orcaRouterResponseMessage,
              /**
               * Filter out the tool calls that were successfully parsed,
               * as we won't have a response for the tool call in the retried
               * request.
               */
              tool_calls: orcaRouterResponseMessage.tool_calls?.filter(
                (toolCall) =>
    ...
    logLlmRequest({
      ...
      response: {
        status: "ok",
        provider: "orcarouter",
        model: orcaRouterResponse.model,
        id: orcaRouterResponse.id,
        created: orcaRouterResponse.created,
    ...
    logger.debug(
      `Retrying OrcaRouter call with the following retry message content: ${stringify(
        retryParams.retryMessageContent,
      )}`,
    );

    return getOrcaRouterResponse(
      {
        ...params,
        messages: [
          ...params.messages,
        ...
        previousInvalidResponses: [
          ...(previousInvalidResponses ?? []),
          { ...orcaRouterResponse, requestTime: currentRequestTime },
        ],
        retryContext: {
          retryCount: retryCount + 1,
  ...
  if (!orcaRouterResponse.usage) {
    logger.error(`OrcaRouter returned no usage information for call`);
  } else {
    const { completion_tokens, prompt_tokens, total_tokens } =
      orcaRouterResponse.usage;
    logger.info(
  ...
  const response: LlmResponse<OrcaRouterLlmParams> = {
    model: orcaRouterResponse.model,
    id: orcaRouterResponse.id,
    created: orcaRouterResponse.created,
    status: "ok",
