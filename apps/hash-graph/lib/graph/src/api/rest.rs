...
pub struct RestRouterDependencies<P: StorePool + Send + 'static> {
    ...
    pub type_fetcher: type_fetcher::fetcher::FetcherClient,
}
...
pub fn rest_api_router<P: StorePool + Send + 'static>(
    ...
    #[cfg_attr(not(feature = "type-fetcher"), expect(unused_mut))]
    let mut merged_routes = api_resources::<P>()
        .into_iter()
        .fold(Router::new(), axum::Router::merge);

    #[cfg(feature = "type-fetcher")]
    {
        merged_routes = merged_routes.layer(Extension(dependencies.type_fetcher))
    }
