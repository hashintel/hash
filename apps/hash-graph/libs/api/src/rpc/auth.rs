    ...
    C: role::ConnectionCodec,
{
    async fn authenticate(
        ...
        let request = role::encode_request::<meta::AuthenticationService, _, _>(
            codec.clone(),
            meta::AuthenticationProcedureId::Authenticate,
            [actor_id],
        )
        ...
        let data: Result<(), serde_json::Value> = items
            .next()
            .await
            .ok_or_else(|| Report::new(AuthenticationError))?
            .change_context(AuthenticationError)?;

        data.map_err(UnknownError)
            .change_context(AuthenticationError)
    }
}
