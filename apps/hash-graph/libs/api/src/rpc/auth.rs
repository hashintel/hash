...
impl<C> AuthenticationService<role::Client<C>> for AuthenticationClient {
    async fn authenticate(
        &self,
        session: C,
        actor_id: AccountId,
    ) -> Result<(), Report<AuthenticationError>> {
        // TODO: rework connection to also include the codec
        // we allocate and encode everything here at the start, this is one extra allocation.
    }
}
