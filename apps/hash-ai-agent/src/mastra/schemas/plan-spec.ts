...
export const zRequirement = z.object({
  ...
  priority: z.enum(['must', 'should', 'could']).describe('MoSCoW priority level'),
});
 ...
 */
export const zHypothesisStatus = z.enum(['untested', 'testing', 'supported', 'refuted', 'inconclusive']);
...
export const zHypothesis = z.object({
  ...
  assumptions: z.array(z.string()).describe('Assumptions the hypothesis depends on'),
  testableVia: z.string().describe('How this hypothesis can be tested (what experiment/evidence)'),
...
export const zUnknownUnknown = z.object({
  ...
  detectionSignal: z.string().describe('How would we notice? What would be the signal?'),
});
...
export const zUnknownsMap = z.object({
  knownKnowns: z.array(z.string()).describe("High-confidence facts we're building on"),
  knownUnknowns: z.array(z.string()).describe('Explicit questions we know we need to answer'),
  unknownUnknowns: z.array(zUnknownUnknown).describe("What would surprise us and how we'd detect it"),
  communityCheck: z.string().describe('What others would need to see to scrutinize our claims'),
});
...
export const zEvalCriteria = z.object({
  ...
  failureCondition: z.string().optional().describe('What constitutes failure (if different from !success)'),
});
...
export const zExecutor = z.discriminatedUnion('kind', [
  ...
  z.object({
    ...
    instructions: z.string().optional().describe('Instructions for human executor'),
  }),
]);
...
export const zBaseStep = z.object({
  ...
  dependencyIds: z.array(z.string()).default([]).describe('Step IDs that must complete before this step'),
  concurrent: z
    .boolean()
    .default(true)
    .describe('Whether this step may run concurrently with other ready steps'),
  requirementIds: z.array(z.string()).describe('Requirement IDs this step addresses'),
  ...
  evalCriteria: zEvalCriteria.optional().describe('How to evaluate step success'),
...
export const zResearchStep = zBaseStep.extend({
  ...
  stoppingRule: z.string().describe('What "done" means for this research (e.g., "3 relevant papers found")'),
});
...
export const zSynthesizeStep = zBaseStep.extend({
  ...
  mode: z.enum(['integrative', 'evaluative']).describe('integrative (combine) or evaluative (judge)'),
  evaluateAgainst: z
    .array(z.string())
    .optional()
...
export const zExperimentStep = zBaseStep.extend({
  ...
  expectedOutcomes: z.array(z.string()).describe('Possible outcomes and their interpretations'),
  successCriteria: z.array(z.string()).describe('What constitutes experimental success'),
...
export const zPlanSpec = z.object({
  ...
  aimType: zAimType.optional().describe('Primary aim type: describe, explain, predict, or intervene'),

  // Core plan elements
  requirements: z.array(zRequirement).describe('Requirements extracted from the goal'),
  hypotheses: z.array(zHypothesis).describe('Hypotheses to be tested (may be empty for pure research)'),
  ...
  // Metadata
  estimatedComplexity: zComplexity.optional().describe('Estimated complexity of the plan'),
});
