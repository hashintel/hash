...
import type { Executor, PlanSpec, PlanStep, StepType } from '../schemas/plan-spec';
import type { PlanningGoal } from '../schemas/planning-goal';
import { type CompositePlanScore, scorePlanComposite } from '../scorers/plan-scorers';
import { compilePlanToWorkflow, type PlanExecutionEvent } from '../utils/plan-compiler';
...
function formatStepForDisplay(step: PlanStep, depth: number): string[] {
  ...
  if (step.dependencyIds.length > 0) {
    lines.push(`${indent}   ${color.dim(`depends on: ${step.dependencyIds.join(', ')}`)}`);
  }

  // Executor
  lines.push(`${indent}   ${color.dim(`executor: ${getExecutorRef(step.executor)}`)}`);
...
function displayPlanVisualization(plan: PlanSpec): void {
  ...
  if (plan.requirements.length > 0) {
    ...
    for (const req of plan.requirements) {
      const priorityColor =
        req.priority === 'must' ? color.red : req.priority === 'should' ? color.yellow : color.dim;
      writeLine(`  ${priorityColor(`[${req.priority}]`)} ${req.id}: ${req.description}`);
    }
  }
  ...
  const entrySteps = plan.steps.filter((step) => step.dependencyIds.length === 0);
  ...
  writeLine(`  ${color.green('Known knowns:')} ${plan.unknownsMap.knownKnowns.length} items`);
  writeLine(`  ${color.yellow('Known unknowns:')} ${plan.unknownsMap.knownUnknowns.length} items`);
  writeLine(`  ${color.red('Unknown unknowns:')} ${plan.unknownsMap.unknownUnknowns.length} items`);
}
...
async function generatePlanFromFixture(
  ...
  if (!output.valid) {
    p.log.warn(`Plan generated but validation failed after ${output.attempts} attempts`);
  }
  ...
  for await (const chunk of stream.fullStream) {
    ...
    switch (event.type) {
      case 'data-plan-start': {
        writeLine(`${color.dim('┌')} Plan started: ${color.cyan(event.data.planId)}`);
        writeLine(
      ...
      case 'data-plan-step-start': {
        ...
        if (stepInfo?.executor) {
          writeLine(`${color.dim('│')}   executor: ${color.cyan(getExecutorRef(stepInfo.executor))}`);
        }
        break;
      }
      ...
      case 'data-plan-depth-transition': {
        const { fromDepth, toDepth, stepsCompletedAtDepth, stepsStartingAtDepth } = event.data;
      ...
      case 'data-plan-complete': {
        const { planId, success, totalDurationMs, stepsCompleted, stepsFailed } = event.data;
...
async function runDemoIteration(cliArgs: CliArgs): Promise<boolean> {
  ...
  if (cliArgs.fixture) {
    const found = FIXTURES.find((item) => item.fixture.input.id === cliArgs.fixture);
    if (!found) {
      p.log.error(`Unknown fixture: ${cliArgs.fixture}`);
      p.log.info(`Available: ${FIXTURES.map((item) => item.fixture.input.id).join(', ')}`);
      return false;
    }
    ...
    selectedFixture = FIXTURES.find((item) => item.fixture.input.id === fixtureChoice)!.fixture;
  }
  ...
  genSpinner.start(cliArgs.mock ? 'Loading cached plan...' : 'Generating plan...');

  let plan: PlanSpec;
  let fromCache: boolean;
  try {
    const result = await generatePlanFromFixture(selectedFixture, cliArgs.mock, genSpinner);
    plan = result.plan;
    ...
    genSpinner.stop(`Plan generation failed: ${error instanceof Error ? error.message : String(error)}`, 2);
  ...
  const { success, completedSteps, errorCount } = await executePlan(plan, delayMs);
  ...
  if (success) {
    ...
    p.log.error(`Completed with errors: ${completedSteps} done, ${errorCount} failed`);
  }
...
async function main() {
  ...
  const modeLabel = cliArgs.mock ? color.yellow(' MOCK MODE ') : color.green(' REAL MODE ');
  p.intro(`${color.bgCyan(color.black(' Plan Execution Demo '))} ${modeLabel}`);

  if (cliArgs.mock) {
    p.log.info(color.dim('Using cached plans for fast iteration. Remove --mock for real LLM calls.'));
  }
