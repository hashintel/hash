    ...
    steps: [
      ...
      },
      {
        ...
        inputs: [
          ...
          { name: "data", description: "From S2", fromStepId: "S2" },
        ],
      ...
      },
      {
        ...
        dependsOn: ["S4"],
        requirementIds: ["R3"],
        inputs: [{ name: "results", description: "From S4", fromStepId: "S4" }],
...
describe("scorePlanStructure", () => {
  ...
  test("returns zero score for invalid DAG", () => {
    const plan = createMinimalPlan();
    plan.steps[0]!.dependsOn = ["S1"]; // Self-cycle
  ...
  test("rewards parallel opportunity", () => {
    ...
    parallelPlan.steps.push({
      ...
      id: "S2",
      description: "Parallel research",
      dependsOn: [], // No dependency — can run in parallel
      requirementIds: ["R1"],
      inputs: [],
      outputs: [],
      query: "Query 2",
