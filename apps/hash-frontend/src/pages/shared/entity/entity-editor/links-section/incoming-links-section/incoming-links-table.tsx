...
import { memo, type ReactElement, useLayoutEffect, useMemo, useRef, useState } from "react";
...
type IncomingLinkRow = {
  ...
  sourceEntityTypes: Pick<PartialEntityType, "icon" | "$id" | "inverse" | "title">[];
  linkEntity: Entity;
  ...
  return (
    <>
      <TableCell sx={linksTableCellSx}>
        <PropertiesTooltip entityType="source entity" properties={row.sourceEntityProperties}>
          <ClickableCellChip
            onClick={() => row.onEntityClick(row.sourceEntity.metadata.recordId.entityId)}
        ...
        <Stack direction="row" gap={1}>
          {row.sourceEntityTypes.map((sourceEntityType) => (
            <ClickableCellChip
              ...
              onClick={() => row.onTypeClick("entityType", sourceEntityType.$id)}
            />
          ))}
        </Stack>
      </TableCell>
...
const createRowContent: CreateVirtualizedRowContentFn<IncomingLinkRow, FieldId> = (_index, row) => (
...
export const IncomingLinksTable = memo(({ incomingLinksAndSources }: IncomingLinksTableProps) => {
  ...
  const {
    ...
    for (const {
      ...
      filterDefs.linkedFrom.options[leftEntity.metadata.recordId.entityId]!.count++;
      filterDefs.linkedFrom.initialValue.add(leftEntity.metadata.recordId.entityId);
      ...
      for (const [propertyBaseUrl, propertyValue] of typedEntries(leftEntity.properties)) {
        const propertyType = getPropertyTypeForClosedMultiEntityType(
        ...
        sourceEntityProperties[propertyType.title] = stringifyPropertyValue(propertyValue);
      }
            ...
            switch (fieldId) {
              ...
              case "linkedFrom": {
                if (
                  !isValueIncludedInFilter({
                    currentValue,
                    valueToCheck: row.data.sourceEntity.metadata.recordId.entityId,
                  })
                ) {
                  return false;
                }
        ...
        .sort((a, b) => {
          ...
          switch (field) {
            case "linkTypes": {
              const aValue =
                /* eslint-disable-next-line @typescript-eslint/prefer-nullish-coalescing -- we don't want an empty string */
                a.data.linkEntityTypes[0]!.inverse?.title || a.data.linkEntityTypes[0]!.title;

              const bValue =
                /* eslint-disable-next-line @typescript-eslint/prefer-nullish-coalescing -- we don't want an empty string */
                b.data.linkEntityTypes[0]!.inverse?.title || b.data.linkEntityTypes[0]!.title;
            ...
            case "linkedFrom": {
              return a.data.sourceEntityLabel.localeCompare(b.data.sourceEntityLabel) * direction;
            }
            case "link": {
