 ...
 */
import { ClusterId, type EntityIdx } from "../ids";
import {
...
export interface WorkerHandle {
  ...
  entityIdToId(entityIdx: EntityIdx): EntityId | undefined;
  /**
   ...
   */
  entityIdxAt(layoutId: ClusterId, recordIndex: number): EntityIdx | undefined;
  /**
  ...
  locateRecords(
    layoutId: ClusterId,
    wanted: ReadonlySet<EntityIdx>,
  ): Map<EntityIdx, number>;
  /** Ask the worker for a selected node's ego (its neighbors' visible representatives). */
  queryEgo(entityIdx: EntityIdx): Promise<readonly EgoTarget[]>;
  /** Ask the worker for the link entities aggregated by a highway lane (its `laneId`). */
  queryHighwayLinks(laneId: number): Promise<readonly EntityIdx[]>;
...
export class WorkerConnection implements WorkerHandle {
  ...
  readonly #highwayRequests = new Map<
    number,
    (linkEntityIdxs: readonly EntityIdx[]) => void
  >();
  ...
  entityIdxAt(layoutId: ClusterId, recordIndex: number): EntityIdx | undefined {
    ...
    if (cluster.flatCapacity === undefined) {
      ...
      return idx === undefined ? undefined : (Number(idx) as EntityIdx);
    }
    const records = new Uint32Array(cluster.versionView.buffer);
    const slot =
      (FLAT_HEADER_BYTES +
        ...
        FLAT_ENTITYIDX_BYTE_OFFSET) /
      4;
    return records[slot] as EntityIdx | undefined;
  }
  ...
  locateRecords(
    ...
    const result = new Map<EntityIdx, number>();
    const cluster = this.#clusters.get(layoutId);
    ...
    if (cluster.flatCapacity === undefined) {
      ...
      for (let index = 0; index < cluster.nodeIds.length; index++) {
        ...
        const entityIdx = Number(raw) as EntityIdx;
