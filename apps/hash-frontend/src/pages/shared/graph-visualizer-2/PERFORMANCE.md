...
| R2  | ✅ Done           | All three fixes are in. (1) `resolveEntityIcon` is memoised by the `entityTypeIds` key in the bridge, so thousands of per‑dot type‑hierarchy walks collapse to one per distinct type‑set. (2) The icon scan is incremental (`render/scene/entity-icons.ts`): a grow‑only flat structure frame resolves only the added records `[prevCount, count)`, reusing the cached prefix — valid because flat records are ordered by ascending entity index in every path (sorted seed column + append‑only absorb; verified in `flat-tier.ts`/`ingest.ts`); a frame with an unchanged count (e.g. a Louvain‑only refresh) costs zero resolver calls and keeps the icon version stable, and full rescans happen only on shrink/tier change. Leaf caches reuse by `nodeIds` identity. Guarded by `entity-icons.test.ts` (counting resolver). Hub labels (`hub-labels.ts`) intentionally stay a full scan: their eligibility is by‑radius, radii legitimately change for EXISTING dots as degree grows, and the per‑dot cost is one SAB float read with ≤12 resolver calls. (3) Structure frames themselves are coalesced during ingest bursts (worker F2, `worker/core/commit-coalescer.ts`), so K streamed batches trigger ~K/8 scans (saturated‑queue worst case) instead of K — each incremental. |
