...
interface EntitiesTableProps {
  readonly activeConversions: {
    [columnBaseUrl: BaseUrl]: {
      dataTypeId: VersionedUrl;
      title: string;
    };
  } | null;
  readonly csvFileTitle: string;
  readonly disableTypeClick?: boolean;
  readonly handleEntityClick: (entityId: EntityId) => void;
  readonly loading: boolean;
  readonly isViewingOnlyPages: boolean;
  readonly maxHeight: string | number;
  readonly hasMoreRowsAvailable: boolean;
  readonly loadMoreRows?: () => void;
  readonly selectedRows: EntitiesTableRow[];
  readonly setActiveConversions: Dispatch<
    SetStateAction<{
      [columnBaseUrl: BaseUrl]: VersionedUrl;
    } | null>
  >;
  readonly setSelectedRows: (rows: EntitiesTableRow[]) => void;
  readonly setSelectedEntityType: (params: {
    entityTypeId: VersionedUrl;
  }) => void;
  readonly sort: GridSort<SortableEntitiesTableColumnKey>;
  readonly setSort: (
...
export const EntitiesTable: React.FC<EntitiesTableProps> = memo(
  ({
    ...
    /**
     * The columns/rows currently rendered by the grid (after its internal
     * sort/filter), captured so the CSV export matches what's on screen.
     */
    ...
    const visibleDataTypeIds = useMemoCompare(
      () => {
        return Array.from(
          new Set(
            Object.values(visibleDataTypeIdsByPropertyBaseUrl).flatMap(
              (types) => [...types].map((type) => type.schema.$id),
            ),
          ),
        );
      },
          ...
          if (columnId) {
            ...
            if (isBaseUrl(columnId)) {
              ...
              const appliesToEntity =
                row.applicableProperties.includes(columnId);
    ...
    const setSortWithConversion = useCallback(
      (newSort: GridSort<SortableEntitiesTableColumnKey>) => {
        const targetConversions =
          conversionTargetsByColumnKey[newSort.columnKey];
    ...
    }, [actorsByAccountId, csvFileTitle, webNameByWebId]);
    ...
    /**
     * Wraps the grid so the scrollbar measurement below can find THIS table's
     * scroller -- multiple EntitiesTables can be mounted at once (e.g. one in
     * a slide over the entities page).
     */
    const tableContainerRef = useRef<HTMLDivElement>(null);

    useEffect(() => {
      const gridEl =
        tableContainerRef.current?.querySelector<HTMLElement>(".dvn-scroller");
    ...
    return (
      <>
        ...
        />
        <Stack ref={tableContainerRef} sx={{ gap: 1, position: "relative" }}>
          <Grid
