...
export const ProfilePageTabs: FunctionComponent<{
  ...
  const currentTabLabel =
    currentTab.kind === "pinned-entity-type" ? currentTab.pluralTitle : currentTab.title;

  return (
    <Box sx={{ overflowX: "auto", overflowY: "hidden" }}>
      <Tabs value={currentTabLabel}>
        {tabs.map((tab) => {
          const label = tab.kind === "pinned-entity-type" ? tab.pluralTitle : tab.title;

          return (
            <TabLink
              active={currentTabLabel === label}
              key={tab.kind === "pinned-entity-type" ? tab.entityTypeBaseUrl : tab.kind}
              label={label ?? <Skeleton width={60} />}
              value={label ?? ""}
              href={`/@${profile?.shortname}${
                tab.kind === "profile" ? "" : tab.kind === "types" ? "/types" : `?tab=${label}`
              }`}
              count={
