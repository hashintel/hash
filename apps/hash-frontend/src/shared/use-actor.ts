...
import { AccountId, Entity, EntityRootType } from "@local/hash-subgraph";
...
export const useActor = (params: { accountId: AccountId }) => {
  const { accountId } = params;

  const { users } = useUsers();

  const userActor = users?.find((user) => user.accountId === accountId);
  ...
  >(structuralQueryEntitiesQuery, {
    variables: {
      includePermissions: params.accountId ?? false,
      query: {
        filter: {
          all: [
            {
