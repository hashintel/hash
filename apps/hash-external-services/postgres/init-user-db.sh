...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" <<-EOSQL
  ...
  CREATE USER $HASH_REALTIME_PG_USER WITH PASSWORD '$HASH_REALTIME_PG_PASSWORD';

  ALTER ROLE $HASH_REALTIME_PG_USER REPLICATION;
...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$HASH_GRAPH_PG_DATABASE" <<-EOSQL
  ...
  GRANT USAGE ON SCHEMA realtime TO $HASH_REALTIME_PG_USER;
  ...
  GRANT INSERT, SELECT, UPDATE, DELETE ON TABLE realtime.ownership TO $HASH_REALTIME_PG_USER;
