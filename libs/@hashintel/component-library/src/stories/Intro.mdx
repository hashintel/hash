...
It might be a good idea to provide a toggle/slider in the application settings to switch between the refractive design and the fallback design, so users can choose based on their preferences/accessibility and device performance.
