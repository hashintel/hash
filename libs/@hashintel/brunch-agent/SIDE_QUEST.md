...
In `packages/plugin-sdcpn/test/construction-tools.test.ts`, delete internal inventory assertions that compare an exported tool list with the same constants used to construct it unless that exact ordered list crosses the mounted model-facing boundary. Delete validator-equivalence assertions whose expected value is computed by immediately asking the delegated Petrinaut validator the same question. Retain concrete accepted, rejected, normalized, nested-path, mode-gating, and mounted-tool-name cases.
...
| Load-bearing behavioral coverage remains | Existing focused suites for capture store, local persistence, anchoring, session log, workpiece evidence/update, Flue history, transport, transcript/UI projection, mutation records, mutation application, native validation, reconciliation, and compiler-clean Inventory slice remain green. |
