...
**Active — authorized by Lu, 2026-09-09.** Authored by Lu with a read-only analysis session alongside live Mission 7; Lu subsequently directed the live-mission parent to execute it and take ownership of all side-quest changes. Implementation and proof remain pending. No paid activity is involved; every item is prose relocation or re-marking in skills and system prompts, plus the tests that assert on them. The previous side quest was moved to a Linear issue in `f9859250b0`; no other side quest is active.
...
- Why did the persona run make no `update_workpiece` call? Two hypotheses, one discriminator: does the same canonical history contain an `activate_skill` call for `sdcpn-modelling`? **No** → the teaching was never in context (the Mission 4 v2 S4 failure shape: guidance behind an activation the model did not perform). **Yes** → the trigger wording ("changes substantially", "before construction") is the cause. B1's always-on cadence addresses both; the answer decides whether skill-activation reliability is a separate residual failure to record for the live mission.
