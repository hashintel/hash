...
| Result | Oracle |
| --- | --- |
| Freshness is derived from history alone | `apps/brunch-agent/test/net-freshness.test.ts`: empty history is `never-read`; one verified read is `current`; a verified read followed by an applied `mutate_petrinet` whose post hash differs is `stale`; a matching re-read returns `current`; an unverifiable observation or a mutation without a delivered record does not count as a read and leaves the state conservative. |
| The marker reaches the model before its first turn | `apps/brunch-agent/test/integration/net-freshness.test.ts` (faux provider, no browser, built application): in `batched-construction` mode the first user turn's response records `brunch.net-stale` ahead of the model's first assistant message; after a correlated verified read result the next user turn records no marker; after an applied `mutate_petrinet` record whose post hash differs, the following user turn records the marker again. The user turn is recorded once. |
| Hydration stays clean | `apps/petrinaut-website/.../use-flue-chat-history.test.ts`: a `brunch.net-stale` dispatch record projects no UI message. |
...
- Existing verification primitives (`recordedBrowserObservation`, `parseClientToolResultMetadata`) are reused, not reimplemented.
