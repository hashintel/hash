...
On close, record: the ownership test and its verdict table as an ADR under `docs/adr/` if Lu accepts it as standing, otherwise in `MISSION.next.md` at planning fidelity — not as a new document under `docs/evidence/` (the existing [ownership audit](docs/evidence/design/core-plugin-ownership-audit-2026-09-09.md) is the ADR source and is retired or converted at that point); the "feed, not be" ledger decision and the normative-source plugin hypothesis in `MISSION.next.md` at planning fidelity; and the freshness discipline in `AGENTS.md` under **Plugin scope** if Lu accepts it as standing. Then remove this file. Intermediate checkpoints return in chat or the PR; they do not become files.
