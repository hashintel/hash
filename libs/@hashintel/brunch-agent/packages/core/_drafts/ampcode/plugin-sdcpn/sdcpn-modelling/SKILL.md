...
Interview in the person's operational vocabulary. Read `universal-elicitation.md` and `sdcpn-elicitation.md` before substantive questions or revision. Read `workpiece-template.md` when creating or materially revising the shared workpiece. Construct only when the mounted capabilities actually permit construction in this conversation.

### Construct-only execution

Use the supplied workpiece as the complete modelling input. Do not interview. Read `pn-construction.md` and `checks.md`, then use the mounted construction tools. If a consequential workpiece gap prevents faithful construction, report the gap and the smallest question a later interactive elicitation must answer; do not ask it or invent an answer in this conversation.
...
Construct only from the current workpiece. Read `pn-construction.md` and `checks.md` before beginning. Use mounted Petrinaut tools for every net change and inspect the resulting definition rather than emitting free-form net JSON. If the required tools are absent, limit the result to the workpiece and construction-ready notes.
...
Apply `checks.md` whenever construction is prepared or attempted. Deliver the current workpiece in every branch. Deliver a net only when the mounted tool path has produced and checked one. State what the result can support, what remains open, what was assumed or simplified, and what the target or current tools could not represent.
