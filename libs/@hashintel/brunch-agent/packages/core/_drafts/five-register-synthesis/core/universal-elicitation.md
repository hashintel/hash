# Universal Elicitation Reference

This progressive reference applies across domains and target formalisms. A plugin profile may add context, specialize an operation, or name target consequences under the same five registers. It may not silently weaken these directives.

The registers are addresses, not a procedure. **Directives** bind. **Recognition** changes what you notice or suspect. **Operations** are selectable moves. **Coverage** names information the workpiece may need, not questions to ask in order. **Verification** checks and repairs the interview and workpiece.

## Directives

### Work from purpose

Establish what the result must help the person decide, answer, compare, explain, or change. Let that purpose determine scope, depth, and which uncertainties deserve questions. Do not pursue a thread merely because it is interesting or because a target schema has a place for it.

### Accumulate posture conversationally

Learn who the result is for, what it must and must not support, the relevant boundary and horizon, the required confidence, the person's available time, and their tolerance for proposed assumptions. Accumulate this posture over the first exchanges rather than presenting an intake form.
...
Keep the person's evidence distinct from your normalization, inference, assumption, proposal, transformation, or default. Never introduce a value, rule, distinction, or level of precision as if the person supplied it when they did not. Agreement with wording you offered permits its use as accepted wording; it is not independent evidence that the person originated it.

Preserve unknown, not-yet-asked, declined, deferred, ambiguous, conflicting, corrected, context-dependent, and deliberately omitted material when those distinctions matter. Do not turn one state into another for the convenience of a complete-looking workpiece.
...
An answer that does not fit an earlier answer may indicate a correction, ambiguity, unnamed condition, source disagreement, or an error. Preserve both until their relationship is understood.
...
Quick agreement to your restatement may indicate acceptance, conversational accommodation, or insufficient attention. Use it as permission to retain the settled wording, not as evidence that the person independently supplied all of its content.
...
Compare the current account with what its stated purpose depends on. Choose the smallest missing or ambiguous distinction whose answer could materially change the result. Ask for that rather than everything adjacent to it, then let the answer determine the next operation.

### Orient around the intended use

Ask for the decision, comparison, worry, or change the result must support. Learn the audience, boundary, horizon, and accuracy bar only to the depth needed to choose a useful first case and avoid irrelevant structure.

### Slice a concrete case

Walk one remembered instance from its meaningful beginning to end. Let the case expose sequence, vocabulary, distinctions, decisions, resources, exceptions, and evidence. Escalate to a hypothetical only from the known case when possible, and identify any parameters you introduced.
...
Clarify a consequential statement until a suitably informed observer could report it without asking what the terms meant. Stop at the granularity the person or available evidence can actually observe.
...
Before deepening a quantity, determine whether the typical case, variation, or tail matters. Ask for the narrowest useful form the person can support. For a spread, quantiles or concrete frequency judgments are often safer than minimum/most-likely/maximum. A memorable incident establishes mechanism and consequence, not a rate; ask about opportunities and time period before inferring frequency.
...
### Change operation when yield drops

Move among case, contrast, basis, sweep, absence, and deferral when the current operation stops producing useful distinctions. Do not mistake repeated open questions for persistence.
...
Coverage is the universal information contract for a recoverable workpiece. It does not prescribe headings, semantic types, or question order. The active plugin profile adds the subject and target distinctions needed for a particular model.

A workpiece may need to preserve:

- the purpose, audience, decision or question, boundary, horizon, and accuracy expectation;
- a cold-readable account of the plan or system in the person's vocabulary, including consequential local terms;
- requirements, constraints, invariants, or safety conditions the purpose depends on;
- decisions, considered alternatives, rejected alternatives, and the reasons distinctions among them matter;
- criteria, observations, witnesses, examples, and counterexamples that could discriminate success from failure or one interpretation from another;
...
- ambiguity, unresolved conflict, correction history, and context-dependent coexistence;
- deliberate omissions, simplifications, defaults, and target-representation losses;
- open questions and deposits with the consequence of leaving them open and the condition for returning.

Coverage depth follows the purpose. A plugin category being available does not make it mandatory; a consequential dependency being unsupported does make the gap visible.
...
- Every load-bearing claim is supportable from the person's account or visibly marked as an agent inference, assumption, transformation, or default.
...
- A correction has not been left as two competing active claims.
- Contextual coexistence has not been collapsed into one average or universal rule.

### After updating the workpiece

- The result remains cold-readable without the transcript.
- Relevant evidence, interpretation, and uncertainty are distinguishable.
- Assumptions say why they exist and how to check them.
- Unknown, unasked, declined, deferred, conflicting, corrected, omitted, and lost material remain honest where present.
- Repeated summaries have not become competing sources of truth.

### Before closing

- The result's purpose and consequential account have been read back or otherwise offered for correction.
...
### Common failure signals and repairs
...
- **Never-asked blindness:** a consequential dependency is unsupported because no question reached it. Use Coverage as a gap check, not a questionnaire.
