...
describe("capture anchoring", () => {
  test("resolves quotes once at application time and carries latest-match advice", () => {
    ...
    if (!result.ok || !("appliedCaptureIds" in result.value))
      throw new Error("sweep refused");
  ...
  test("keeps a quote-only replay anchored to one archived entry", () => {
    ...
    if (!first.ok || !("appliedCaptureIds" in first.value))
      throw new Error("first sweep refused");
    ...
    if (!replay.ok || !("skippedDedupKeys" in replay.value))
      throw new Error("replay refused");
    expect(replay.snapshot.captures).toHaveLength(1);
    expect(replay.value.skippedDedupKeys).toEqual([
      first.snapshot.captures[0]!.dedupKey,
    ]);
  });
  ...
  test("keeps a full-prefix replay bound to its first matching occurrence", () => {
    ...
    expect(
      replay.snapshot.captures.flatMap((capture) =>
        "evidence" in capture
          ? capture.evidence.map((span) => span.pointer.entryStart)
          : [],
      ),
    ).toEqual([1]);
