...
// The runner resolves the case directory and its prompt files relative to its
// own location, so the copy mirrors the protocol and case paths under one root.
...
async function runBaseline(
  ...
  const checkpoint = v.parse(
    ...
    JSON.parse(
      await readFile(
        join(baselineCopy.outputDirectory, `condition-${condition}.raw.json`),
        "utf8",
      ),
    ) as unknown,
  );
...
describe.skipIf(!contextRootPresent)("baseline runner", () => {
  ...
  test("refuses the retired condition 3 entry point", async () => {
    const baselineCopy = await createBaselineCopy();
    const subprocess = spawn(
      process.execPath,
      [
    ...
    expect(exitCode).toBe(1);
    expect(stderr).toContain("usage: node run.ts <1|2|4>");
    expect(existsSync(baselineCopy.outputDirectory)).toBe(false);
  });
  ...
  test("preserves the condition 2 prompt and completion path", async () => {
    const testDirectory = await createBaselineCopy();
