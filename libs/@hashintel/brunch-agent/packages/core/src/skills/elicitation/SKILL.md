...
Create a first partial workpiece as soon as one consequential distinction exists, then update after each useful stretch or correction and before delivery. Ordinary settlement is direct: call `mutate_workpiece` with the full next Markdown account and current `baseRevisionId` (`null` for the first revision), without a preceding read. Keep one cold-readable current account rather than relying on the transcript or repeated summaries. Preserve unrelated meaning, evidence and unresolved material, then inspect the returned prior/next hashes and changed window instead of assuming the full replacement did so. Wait for the returned `revisionId` and `sha256`; together with the Markdown submitted in that successful call, they are the authoritative body and identity for that exact settlement. Reuse the submitted body for presentation instead of requesting full content again. Request full content only when it changed outside the current reasoning, is unknown, or is missing. A candidate, failure, pointer-only result, confirmation, or body under another revision is not reusable authority. This tool does not end the response.

Only when declaring new evidence and `read_workpiece` is mounted, draft the full candidate first. Combine `includeContent: false`, `includeSources: true`, candidate `markdown`, and `locateTexts` in one call so the response carries authorized true-user source IDs and literal UTF-16 candidate occurrence spans without retransmitting current Markdown. Inspect the returned occurrences, then use those locators in the evidence of the same candidate's single `mutate_workpiece` settlement; do not settle once before lookup and again afterward. Candidate Markdown without `locateTexts` is refused, so omit the candidate when no lookup is needed. The candidate hash/length is not a revision, state write, evidence settlement, or authorization. Changed candidate text requires a fresh lookup. Duplicates, overlapping matches and any omitted matches are explicit, not an automatic passage choice. After successful settlement, reuse its submitted Markdown with the returned `revisionId` and `sha256`. Only when immediate construction needs settled-revision spans, make a source- and content-free `read_workpiece` call with `includeContent: false`, `includeSources: false`, and `locateTexts` but no candidate Markdown. The settled revision pointer still returns.
