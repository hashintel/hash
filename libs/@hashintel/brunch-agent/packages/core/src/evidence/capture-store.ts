...
export const parseCaptureStoreSnapshot = (
  ...
  for (const records of [snapshot.captures, snapshot.issues, snapshot.events]) {
    if (new Set(records.map((record) => record.id)).size !== records.length) {
      throw new TypeError(
        "Capture-store record ids must be unique within their record family.",
      );
    }
  }
  for (const capture of snapshot.captures) {
    ...
    if (
      ...
      !snapshot.captures.some(
        (candidate) => candidate.id === capture.supersedes,
      )
    ) {
      throw new TypeError(
        `Capture ${capture.id} supersedes an unknown capture.`,
      );
    }
  }
  ...
  for (const event of snapshot.events) {
    ...
    if (event.type === "retraction") {
      if (
        !snapshot.captures.some((capture) => capture.id === event.captureId)
      ) {
        throw new TypeError(
          `Retraction ${event.id} references an unknown capture.`,
        );
      }
      continue;
    }
    const issue = snapshot.issues.find(
      (candidate) => candidate.id === event.issueId,
    );
    if (!issue)
      throw new TypeError(`Event ${event.id} references an unknown issue.`);
    ...
    if (event.type === "issue-closed") {
      if (issue.type === "conflicting") {
        throw new TypeError(
          "A conflicting issue cannot be closed without a resolution record.",
        );
      }
      continue;
    }
    ...
    if (
      ...
      throw new TypeError(
        `Resolution ${event.id} does not account for its conflict's captures.`,
      );
    }
  }
  for (const issue of snapshot.issues) {
    if (
      issue.references.some(
        (captureId) =>
          !snapshot.captures.some((capture) => capture.id === captureId),
      )
    ) {
  ...
  const addSuccessor = (captureId: string, successorId: string): void => {
    if (successorByCapture.has(captureId)) {
      throw new TypeError(
        `Capture ${captureId} has a forking supersession history.`,
      );
    }
  ...
  for (const capture of snapshot.captures) {
    ...
    while (
      current &&
      snapshot.captures.some((candidate) => candidate.id === current)
    ) {
      if (visited.has(current)) {
        throw new TypeError(
          `Capture ${capture.id} participates in a supersession cycle.`,
        );
      }
      visited.add(current);
  ...
  const openConflicts = snapshot.issues.filter(
    (issue) =>
      issue.type === "conflicting" && !closingEventByIssue.has(issue.id),
  );
  for (const [index, issue] of openConflicts.entries()) {
    ...
    const overlappingIssue = openConflicts
      .slice(index + 1)
      .find((candidate) =>
        candidate.references.some((captureId) =>
...
const priorEvidenceForOccurrence = (
  ...
  return snapshot.captures
    .filter(
      ...
      > =>
        "evidence" in capture &&
        capture.evidence.every(
          (evidence) => evidence.pointer.sessionId === sessionId,
        ) &&
...
const validateInputProposal = (
  input: CaptureInputProposal,
): CaptureStoreRefusal | undefined => {
...
const requireEvidenceContext = (
  ...
  context ?? {
    ...
    message:
      "Evidence-bearing commands require the harness-owned session context.",
  };
...
export const deriveCaptureStatus = (
  ...
  if (
    snapshot.events.some(
      (event) => event.type === "retraction" && event.captureId === captureId,
    )
  ) {
    return "retracted";
  }
  if (
    ...
    snapshot.events.some(
      (event) =>
        ...
        event.loserCaptureIds.includes(captureId),
    )
  ) {
    return "superseded";
  }
...
export const deriveIssueStatus = (
  snapshot: CaptureStoreSnapshot,
  issueId: string,
): IssueStatus =>
  snapshot.events.some(
    (event) =>
      (event.type === "resolution" || event.type === "issue-closed") &&
...
const currentHeads = (
  ...
  while (changed) {
    ...
    for (const capture of snapshot.captures) {
      if (
        capture.supersedes &&
        reachable.has(capture.supersedes) &&
...
const evidenceIdentity = (
  capture: CaptureProposal | CaptureEnvelope,
): string | undefined =>
  "evidence" in capture
    ? canonicalString(
        [...capture.evidence]
...
const applySweep = (
  ...
  for (const proposal of proposals) {
    ...
    const duplicateWithoutSupersession =
      !proposal.supersedes &&
      snapshot.captures.some(
        (capture) => captureRetryKey(capture) === retryKey,
      );
    const duplicateInBatch = accepted.some(
      (candidate) =>
        captureRetryKey(candidate) === retryKey &&
        candidate.supersedes === proposal.supersedes,
    );
    ...
    if (proposal.supersedes) {
      const target = snapshot.captures.find(
        (capture) => capture.id === proposal.supersedes,
      );
      if (!target) {
        return refusal({
  ...
  for (let leftIndex = 0; leftIndex < captures.length; leftIndex += 1) {
    ...
    for (
      ...
      rightIndex < captures.length;
      rightIndex += 1
    ) {
      const right = captures[rightIndex]!;
      if (
        (!appliedCaptureIds.includes(left.id) &&
          !appliedCaptureIds.includes(right.id)) ||
      ...
      const sameEvidence =
        evidenceIdentity(left) !== undefined &&
      ...
      const nearIdenticalPayload =
        ...
        leftPayload === normalizedPayloadText(right);
...
export const applyCaptureStoreCommand = (
  ...
  switch (command.type) {
      ...
      const inactiveReference =
        candidateIssue.type === "conflicting"
          ? candidateIssue.references.find(
              (captureId) =>
                deriveCaptureStatus(snapshot, captureId) !== "active",
            )
          : undefined;
      if (inactiveReference !== undefined) {
        return refusal({
        ...
        candidateIssue.type === "conflicting"
          ? snapshot.issues.find(
              (issue) =>
                issue.type === "conflicting" &&
                deriveIssueStatus(snapshot, issue.id) === "open" &&
                issue.references.some((captureId) =>
                  candidateIssue.references.includes(captureId),
                ),
            )
          : undefined;
    ...
    case "close-issue": {
      const issue = snapshot.issues.find(
        (candidate) => candidate.id === command.issueId,
      );
      if (!issue) {
        return refusal({
          code: "unknown-issue",
      ...
      if (issue.type === "conflicting") {
        return refusal({
          ...
          message:
            "A conflicting issue closes only through a user-cited resolution record.",
          issueId: issue.id,
        });
      }
    ...
    case "resolve-conflict": {
      ...
      if (
        ...
        return refusal({
          ...
          message:
            "A conflict resolution must cite non-empty verbatim user quotes.",
          issueId: issue.id,
        });
      }
      const context = requireEvidenceContext(evidenceContext);
      ...
      const citedCaptureIds = [
        command.winnerCaptureId,
        ...command.loserCaptureIds,
      ];
      const invalid =
        issue.type !== "conflicting" ||
    ...
    case "retract-capture": {
      const capture = snapshot.captures.find(
        (candidate) => candidate.id === command.captureId,
      );
      if (!capture) {
        return refusal({
          code: "unknown-capture",
