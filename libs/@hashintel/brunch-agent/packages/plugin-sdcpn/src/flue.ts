...
export function useSdcpnPlugin(
  ...
  const isOrdinaryConversation = initialData === undefined;
  ...
  useAgentStart(() => {
    if (isOrdinaryConversation && delivery.kind === "user")
      setCurrentNetReadState("available");
  });
...
`.replace(/^\s+|\s+$/gu, ""),
  ...
  } else if (isCurrentNetReadAvailable) {
    for (const constructionTool of petrinautConstructionTools) {
      if (constructionTool.name !== getLatestNetDefinitionToolName) continue;
      useTool({
        ...constructionTool,
        run(context) {
