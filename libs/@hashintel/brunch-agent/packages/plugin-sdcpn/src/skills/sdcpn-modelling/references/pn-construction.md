...
Required `x` and `y` are provisional presentation values: place new nodes on a rough grid, then use layout. For a new type, `iconSlug: "circle"` and a simple CSS `displayColor` are sufficient visual choices. These choices carry no operational meaning. Optional metadata and port fields are unnecessary for a simple root-net chunk; use them only when the task needs their capability. Existing component endpoints must be observed, not invented.
