...
Settle the current account with `mutate_workpiece` before construction. Wait for the result and reuse the submitted Markdown with its returned `revisionId` and `sha256` as authoritative for that exact settlement; never combine settlement and browser construction in one batch. Only when immediate construction needs settled-revision spans, call `read_workpiece` with `includeContent: false`, `includeSources: false`, and `locateTexts` without candidate Markdown. This locator-only read establishes spans against the settled revision without retransmitting its body. An unsettled candidate lookup does not authorize construction. Label retained prepared or legacy fenced material honestly rather than treating it as a settled revision.
