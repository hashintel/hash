...
- the net snapshot has no reported errors from canonical `getNetCompilationErrors` in integrated Brunch. That diagnostic does not check saved scenario or metric compilation; those are validated separately during experiment execution, so a clean net diagnostic is not proof their code compiles.
