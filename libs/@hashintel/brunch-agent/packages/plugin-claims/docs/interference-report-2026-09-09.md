...
- **The blind read-back is the one component Brunch cannot have.** "No second model call" excludes it. The probe's response is to make the *person's* comparison the check and to instruct the elicitor to present for comparison, not approval — which is stricter than the external system's click-to-confirm, where a human may approve a statement without reading it against the source. That is the strongest argument for graduating the "not an independent auditor" sentence to core: it is Brunch's substitute for a component it has decided not to build.
...
All three plugin resources carry `Aligned to core as of \`223d7218b0\``. On the next core change, re-read `claims-elicitation.md` Directives (each states which core entry it counterparts, narrows, or replaces) and the table above; reclassify each entry as unchanged, generalized into core, or stale. `rg -n "Aligned to core as of" packages/plugin-claims/src` should hit exactly three times.
