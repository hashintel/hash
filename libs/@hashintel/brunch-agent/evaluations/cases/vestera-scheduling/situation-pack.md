...
4. **Own your unknowns.** The facts below marked _(doesn't know)_ you genuinely do not know —
...
5. **Hold tacit knowledge back.** Facts marked _(tacit)_ surface only if the interviewer asks a
...
6. **Perspective colouring is honest error.** Facts marked _(believes)_ are what you say when
...
- _(tacit)_ You'd also love ammunition for the buffer argument — you think the tank between
...
- One **changeover crew** — two techs on day shift — serves all three lines. If two lines want
  a washdown at once, someone waits. _(believes)_ "Changeovers mostly overlap fine" — pressed
...
- Changeover times: quick rinse inside a family, ~20–30 min. Family switches are the expensive
  ...
  out is ~2 hours either way. _(tacit)_ After a dark tint, one white SKU (VW-02, the retail
...
- After any family switch the first units are junk while the line settles — "ramp scrap".
  Worse after the big washdowns. _(doesn't know)_ exact scrap per changeover type; quality
  tracks scrap only as a monthly percentage.
- Rates: _(believes)_ "Line 2 is about twice as fast as Line 1" — that's true for whites;
  pressed, you admit for tints they're nearly even, "funny, never thought about why".
  _(tacit — surfaces under probing about which stage limits which product)_ thick specialty
...
- Your sheet's arithmetic: each product-line pair has a rate; a run takes fill-up time plus
  ...
  breakdowns and slow QA. _(tacit)_ the mill→fill tank on Line 1 backing up is one reason.
- Breakdowns: the Line 2 filler jams "every week or two, half an hour to half a shift".
  Line 1's mill motor is the scary one — rare, but it took four days once. _(doesn't know)_
...
- Maintenance: preventive maintenance is triggered by units-run counters; maintenance plans
  it, tells you, and you argue. _(tacit)_ the good move everyone does informally is to
...
- Late-order pain: _(tacit, this is the one nobody has written down)_ there is no penalty
...
- _(tacit)_ Unwritten allocation rules: Meridian white orders **always** run on Line 2 —
