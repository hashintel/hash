...
// instead of retrying.
const anthropic: Pick<Anthropic, "messages"> = clientModule
  ? ((await import(clientModule)).default as Pick<Anthropic, "messages">)
...
function usage(): never {
  console.error("usage: node run.ts <1|2> [--resume|--continue-final]");
  process.exit(1);
}
...
async function writeArtifacts(): Promise<void> {
  ...
  await writeFile(
    `${transcriptDir}/condition-${condition}.md`,
    header + body + "\n",
  );
  ...
  if (
    ...
    await writeFile(
      `${transcriptDir}/condition-${condition}-model.txt`,
...
if (mode === "--continue-final") {
  ...
  if (final?.role !== "assistant") {
    console.error(
      "checkpoint does not end with an interviewer message; nothing to continue",
    );
    process.exit(1);
  }
