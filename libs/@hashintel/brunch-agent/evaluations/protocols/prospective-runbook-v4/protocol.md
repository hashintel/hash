...
Status: **draft repair protocol; no paid execution authorized**

Protocol id: `prospective-runbook-v4`

Observed-output namespace: `vestera-architecture-candidate-v4`
...
Their frozen omniscient range is `66.3–80.0 / 100`; their cold-utility range is `3.3–3.5 / 4`; both have conditional downstream readiness and no hard-failure gate. Their workpieces, grade reports, and campaign adjudication are hashed into the v4 instrument.
...
| Observed output | `../../../docs/evidence/evaluations/vestera-architecture-candidate-v4/` |
...
```sh
BRUNCH_RUNBOOK_REPLICATION=1 yarn workspace @apps/brunch-agent runbook:elicit:architecture-v4
BRUNCH_RUNBOOK_REPLICATION=2 yarn workspace @apps/brunch-agent runbook:elicit:architecture-v4
BRUNCH_RUNBOOK_REPLICATION=3 yarn workspace @apps/brunch-agent runbook:elicit:architecture-v4
```
...
- A valid quality member must contain a recoverable workpiece and no ordinary-path violation. Universal and profile guidance must have returned successfully before the first interactive question, and the workpiece template must have returned successfully before first workpiece creation.
...
The v2 artifacts remain in `vestera-prospective-candidate-v2/` with an abort adjudication. They are neither moved into v4 nor used as flat-prompt controls.

## Grading and adjudication

For each valid v4 workpiece:
