...
Brunch transport/prompt tests must separately establish that only the fixed response preference is used, typed requests omit it, and existing causally linked browser-tool results preserve it. The rest of Improved Relay remains a separate implementation/evidence step; this patch alone is not a naturalness result.
