...
This is (at least began as) Ciaran’s rough thoughts on things that might be useful in the process of going from an interview with a user to a Petri net representation of a process.

</aside>

# Things the agent should investigate
...
An **example** which demonstrates the two above patterns in one can be found in the ‘Supply Chain With Disruption’ example Petri net. This bit:
