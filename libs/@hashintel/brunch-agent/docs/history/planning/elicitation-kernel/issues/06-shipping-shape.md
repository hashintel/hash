...
- **Envisioned horizon** (named, not built): per-substrate binding packages (`flue-<name>`, `pi-<name>`, `codex-<name>`), same harness inside each. The payoff _if the second-binding test keeps passing_, not a commitment.
