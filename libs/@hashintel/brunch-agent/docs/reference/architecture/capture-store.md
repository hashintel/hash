...
3. **Provenance labels are trusted, not verified.** "Anchors only on true user entries" is
   enforced against a `source` field the caller supplies. The store refuses a span _labeled_
...
3. **The bridge with no middle span.** The directive asked for "the semantics FE-1389 adds
   ...
   Stating what the top branches establish _together_ required writing "they do not yet
...
7. **"Sweep" names two different things.** In the spec, a sweep is a pass over settled
   conversation that _produces_ captures; in this module, `apply-sweep` is the transaction that
   _stores_ proposals someone else produced. The rendering had to say "a sweep of capture
...
> to _see_ to enforce this?" as the first question for every proposed invariant, before "what
