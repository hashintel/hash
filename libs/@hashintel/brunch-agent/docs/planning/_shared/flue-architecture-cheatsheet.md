...
_Boundary implication_: single-owner-per-conversation is a **deployment** constraint, not a
...
_reference_, not this guide) filed `useAgentStart` firing and updater composition as
...
- _Capability 6 (private model call)_ has two non-interchangeable affordances. A harness tool can
...
- _FE-1386 (compaction pin)_: the source read retired the discovery spike; `useModel`'s
...
- **Durable tools**: `step.do(name, fn)` — "exactly-once-recorded, at-least-once-executed";
  completed steps replay their recorded values on recovery. Ordinary tools are _not_
...
- **FE-1403/FE-1406 (guidance cards, quiver): Flue skills _are_ the card-delivery
  ...
  _to_ skills in the binding.
- _FE-1392_: if sweep application does side-effectful writes from inside a tool, mark it
...
- _FE-1391 (archive)_: the durable entry projection we plan to archive is Flue's **public
  ...
  record") is about _retention authority_, not about re-capturing — the archive copies out of
...
- Workflows are a _pattern_, not a primitive — no `defineWorkflow`. Four approaches:
...
instrument (the failure catalogue becomes `createJudge` rubrics). _Do not rebuild_: no custom
...
builds a node-only assumption into a _harness_ layer — node-only assumptions belong in
...
1. **Consume directly (no wrapping).** UI transport (`useFlueAgent`, `createFlueClient`,
   ...
   judges), scheduling, deployment. These are _shell-facing_ affordances: the demo shell,
...
- **The at-least-once floor is universal.** Exactly-once-_recorded_, at-least-once-_executed_
