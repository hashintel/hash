...
**Definition.** _(amended by [ADR-0003](../adr/0003-three-register-ir.md), 2026-08-18 —
...
register 2 is a derivation, never a persistence surface.)_ The intermediate representation of
...
2. **Evidence granularity.** Each payload is one assertion at the resolution the evidence
   ...
   modelling work mid-conversation. _(new in this decision; amended FE-1397 — generalized from
   statement granularity for log-derived captures)_
3. **Projection-independence.** Kinds are defined in domain vocabulary, and the IR may
   ...
   proportional to domain–format distance, and where the target format _is_ the domain
   (Gherkin), the vocabulary clause degenerates gracefully rather than failing. _(new in this
   decision; amended FE-1397)_
4. **Relations are payload data, not envelope structure.** A plugin needing structure declares
   ...
   references). _(already spec canon, §5)_
5. **Domain labels and rollups derive at read time via `project`, never stored.**
   _(already spec canon, §5, §13.3)_

**Recommended patterns** (graded per FE-1397):

- **SHOULD** _(promoted from MAY, FE-1397 — all four worked designs use it)_: symbolic,
...
- **MAY**: **completion-anchor kinds** for a question-relative completion contract — a distinct
  ...
  _(amended FE-1397 — the pattern is "completion anchors on purpose-bearing captures", not
  "declare a kind named objective")_
- **MAY**, for process-shaped domains _(added FE-1397, promoted from Layer B)_: a
...
- **Named escape hatch only** _(demoted FE-1397 — zero uptake across all four designs)_:
...
| 3   | **activity**             | steps _as the expert states them_: actors, resources, preconditions, outcomes, duration                        | factored transitions (see granularity rule)                                                                                                                                |
...
sparsely populated — their presence _is_ the "net is one projection" story even at two
captures each.
