...
Status: **candidate design input for Mission 6, not a selected mechanism**. Drafted 2026-09-02 from a code survey of `@hashintel/petrinaut-core`, `@hashintel/petrinaut`, `@hashintel/brunch-agent/packages/plugin-sdcpn`, `@apps/brunch-agent`, and `@flue/runtime@2.0.3`. Live authority remains [`MISSION.md`](../../MISSION.md); [Mission 6 — traceable projection](../mission-drafts/6-traceable-projection.md) owns the provider-schema, mutation-sequence, and partial-failure boundaries this proposal addresses. Mission 6 must repair the known schema carrier first and then decide whether a bounded atomic batch is the least sufficient construction mechanism. Nothing here is evidence that the design works or authority to implement `pn_read` or `pn_edit`.
