...
The selected workpiece and source conversation are frozen in the Mission 5 handoff above. An honest prebuilt non-empty SDCPN and derivation fixture still have no canonical paths; they remain Mission 5 preparation obligations, not names to invent here.
...
| Selected Mission 4 workpiece, exact conversation, manifest, and adjudication are accepted | Closed by `vestera-architecture-candidate-v3/mission-5-handoff.md`: one selected pre-repair scored workpiece, canonical raw snapshot, source-message binding, instrument manifest, comparative reports/adjudication, owner acceptance, and explicit routing/sample limitations. |
...
- **Mission 4 exit join:** consume `vestera-architecture-candidate-v3/mission-5-handoff.md` and its hashed selected artifacts. Preserve the distinction between the scored pre-repair instrument and the owner-authorized post-campaign URI instruction repair; carry the missing-template/reference disclosure and acquisition gaps rather than laundering them into derivation rationale.
