...
**Demo allocation:** [Mission 7b](../mission-archive/7b-ordinary-batched-construction-provenance.md) owns the ordinary structural batch/correction seam. Live [Mission 7c](../../MISSION.md) owns a substantial connected flagship and therefore brings forward its exact construction/schema operation classes, safe template-copy/rehearsal behaviour, and one ordinary correction family without unrelated rebuilding. That includes deletion or nested/component operations when the selected artifact actually needs them; it is not a thin tracer. This draft retains general unchanged-repeat, changed-input, retirement, concurrency and broader schema/scenario coverage after the demo. Re-evaluate its whole-stratum gates against the substantial shipped 7c seam before cutting this broader mission.
...
- [`../mission-archive/3-structurally-typed-runbook-to-headless-pn.md`](../mission-archive/3-structurally-typed-runbook-to-headless-pn.md) — historical workpiece leg and construction limits. The implementation packet is retired; inspect current [earned contracts](../../MISSION.md#earned-data-and-execution-contracts) and their owning tests for present construction guarantees.
...
- [Mission 8 consumed deployment contract](../../MISSION.next.md#mission-8-consumed-deployment-contract) — application artifact landed on `main` through #9495/#9487/#9573; SRE-1013 still owns ECS provisioning and the remote proof matrix. Historical stop: `157730cc5a214dd9c543e8d95c7193a219c48aef` on `ln/fe-1569-brunch-agent-deployment`. Mission 9 names local posture unless a Mission 8 successor has landed.
...
- concurrent or hand change during projection: stale-base refusal per the Mission 7 mutation record, external-import semantics, and never a silent overwrite;
...
- basis and mutation records for every generated, changed, and retired element, and the current-state why answer over them.
...
- **Browser to record:** the mutation record is the only admissible statement of what happened; the plan is not.
...
| Mutation records are independently verifiable; duplicates resolve to unknown | Mission 7 close evidence; cite at cut time. |
| Unchanged repeat is idempotent | **ORACLE GAP:** bind to exact before/after canonical definitions, the mutation-record log showing attempt history only, and a stable-id assertion when implemented. |
...
- **Inner mechanism:** projection-plan construction from workpiece revision and live definition; intended-effect to basis mapping; stable id planning; epoch ledger; reconciliation of mutation records against the plan; no basis for unanticipated effects; no state advance on rejected or no-op calls.
- **Middle integration:** drive the built Brunch application through the Mission 5 transport into a real Petrinaut instance on the Mission 7 conversation. Run unchanged repeat, one changed input, one retirement, a duplicate delivery, a stale-base attempt, and a hand edit between plan and apply. Inspect canonical state, mutation records, epochs, and why answers.
...
- **The mutation record is the only statement of effect.** Guard: no basis, epoch, or why state advances from a plan or a self-reported effect set that fails diff accounting.
