...
8. **Source-to-production manifest.** The current-ancestry selected sources are the Ampcode (`A`) and Five-Register (`F`) drafts at `ca57b45729260cc657f89b718fc505997a4e1b3c:libs/@hashintel/brunch-agent/packages/core/_drafts/`, byte-identical on those trees to historical recovery commit `e087f570d77507c12a4862604a30c6fcd640aa2f`; the Gherkin paper instrument (`G`) is `A`'s `plugin-gherkin/` copied unchanged to `evaluations/protocols/gherkin-shape-c-paper-v1/instrument/`. Current-ancestry implementation commit: `baba973269ce7ecf1a47de8749c751033b2ce471`. Proof item 1 is run against this table; any other difference is a stop condition.
...
```text
accepted topology at baba973269
...
- Which three persona case families provide the smallest discriminating full-run/probe allocation, and the exact elicitor, persona, and independent adjudicator models.
...
- a first Substantive text is an opening Battery, or a template/construction resource is used before its accepted branch;
