...
**Current throughline:** compare the same 20–30-operation construction through batch versus individual operations now that the candidate-mode `mutate_petrinet` tracer crosses the real product. One direct `anthropic/claude-sonnet-4-6` call passed the selected-schema carrier on its first attempt: one valid 25-operation tool call in 28.3 seconds, 5,287 input / 3,041 output tokens, with unique IDs and every arc referencing earlier created nodes. The product comparison has not passed. Missing browser-tool classification and AI SDK dynamic-tool projection now have deterministic coverage; mixed `getLatestNetDefinition` plus construction-resource reads in one proposal remain refused. Product diagnostics now exist (`286e5780c2`, `b475155f53`, `638a0c2d02`). The throughline defect was premature browser dispatch plus an under-described wrapped operation dialect. Gate-then-flatten is in: withhold `mutate_petrinet` until server validation, mark gated `tool-input-start` `dynamic: true`, flatten operations to `{operationId, basisId, type, input}` with envelope descriptions, and make root-only exclusions structural. A live `?brunchTracer=root-creation` Sonnet turn then accepted a flat three-operation `mutate_petrinet` on the first attempt (`toolu_011ci1Ja56WvDmo8hBLeXxr9`): server outcome `{"awaiting":"client"}`, one `client-tool-result` for that id, `addPlace`/`addTransition` applied and the redundant `addArc` a no-op, Queue and Start visible. That turn used 11 completions and about $0.071; the mutation generation was 1,330 output tokens and browser settlement was 91 ms. After construction, one `update_workpiece` failed on unauthorized evidence locators and the model recovered without a second mutation. A faux Chrome witness then proved the withheld path: wrapped `{basisId, operation:{…}}` is a server schema error, produces no `client-tool-result` for the rejected id, and the flat retry executes. `root-creation.integration.ts` still asserts `conversationConstructionMode` against the batched URL; the individual route remains `?brunchTracer=construction`. No successful batch or individual timing/equivalence result exists yet. Lu authorizes Mission 7b development inference with no daily spend cap and directs sapper-like execution: record usage, stop on material failure, but do not turn each bounded reversible step into another ceremony gate. Scenario seeding, remote write, release and Linear write remain separate.
...
The combined no-edit browser/provider probe stopped honestly because the shipped ChatAgent and website recognize static tool catalogues. The direct selected-schema Sonnet discriminator then passed on the first and only generation request (`req_011CeunumYCAx5eiRweqnDyf`): the 15,449-byte / approximately 3,584-token schema produced exactly 10 places, 7 transitions and 8 arcs with valid canonical inputs, no repair and correct intra-call references. Implement the smallest candidate-mode tracer through actual Flue deferred-tool correlation and browser canonical execution, then compare that same coherent 25-operation target with the equivalent individual-operation route. Record request count, elapsed time, usage, final definition/hash equivalence and partial outcomes. Retain or deepen the tracer only for a bounded correctable defect; remove it if the batch premise fails. A provisional 5× improvement is a strain detector, not a sacred benchmark.
...
- **2026-09-10 — No daily inference cap.** Mission 7b development inference has no daily spend cap. Record usage and stop on material failure; do not invent a ceremony gate from spend.
- **2026-09-10 — Product-path `mutate_petrinet` observed.** A live root-creation tracer turn accepted a flat three-operation batch on the first attempt and landed one `client-tool-result`; a faux reject-then-accept witness proved the wrapped dialect never executes. Next is the 25-operation batch versus individual comparison. The individual catalogue stays on `?brunchTracer=construction`; do not treat the stale root-creation individual-tool suite as the batch baseline.

## Fog-line

- **Batch carrier and value:** local canonical mechanics pass, the selected-schema Sonnet carrier passed once, and a live three-operation product-path batch plus a faux reject-then-accept witness now land `client-tool-result`. Real 25-operation browser settlement latency, aggregate effect/mutation recording and equivalent individual comparison remain unproved. Do not deepen past that comparison.
