...
| --- | --- | --- |
| Conversation → workpiece → model → why | Primary; consumes Part A's bounded contracts | Ordinary construct → fluent why → ordinary correction → fluent updated why now hold on one net. Both whys skipped `brunch_why`, so the mechanical record/basis/passage pane stays empty. |
...
| Two seeded substantial scenarios | Primary | No production seeder/catalogue/copy contract exists. The mission's `?scenario=inventory-purchasing` spelling collides with the shared-example simulation-scenario location already owned by `example-search.ts`; do not overload that key. Second scenario waits on Inventory's dominant observed strain. |
...
| Broad flagship why coverage | Primary | Ordinary why is fluent and can distinguish original from correction, but does not call `brunch_why`. Mechanical completeness still requires the tool or an honest absence. |
| One ordinary correction/reopen | Primary | The receiving-dock accept/reject split applied as one 11-operation batch and preserved the upstream path. Closing and reopening the assistant restored the same conversation, revision 3 workpiece, and corrected topology without a new turn. |
| Start from scratch | Primary; recorded path even though live demo starts seeded | File → New now splits Build with Brunch and Start blank; Build with Brunch offers Interview first versus Quick preview. Live create-new through that picker, then a workpiece-and-net from those chips, is still unproved. |
| Mutation batching | Candidate enabler for primary construction speed; local mechanics, empty-net product-path comparison, ordinary catalogue mount, and a live remove/add correction passed | Full-registry schema is 69,897 bytes and local batching adds 8.3% accounting overhead. The selected batch still admits only place/transition/arc add and remove. Inventory types, differential equations, parameters, metrics and scenarios remain refused, so Inventory-scale chunks cannot start from admission alone. |
...
- **2026-09-10 — Ordinary why without `brunch_why`.** The receiving-dock accept/reject question was answered as exclusive-choice plus an unencoded decision. The model did not call `brunch_why` or `getLatestNetDefinition`; the why pane stayed empty. Do not name the why tool in a follow-up.
- **2026-09-10 — Ordinary correction needs remove operations.** The accept/reject correction stopped after a live read: add-only `mutate_petrinet` cannot rewire InspectionDone without leaving a dead parallel net. Admit root removePlace, removeTransition, and removeArc on the selected batch rather than asking the user to delete nodes or rebuild from scratch. Do not name those operations in the user prompt.
- **2026-09-10 — Live ordinary correction on the receiving-dock net.** After admitting removes, a stale-hash retry failed (cited the prior getLatest after a remount changed the live hash). A follow-up that re-read first applied 9 operations and no-op'd 2 cascaded arc removals, saved workpiece revision 3, and left TruckArrived → ReadyForInspection untouched.
- **2026-09-10 — Updated why stays fluent without `brunch_why`.** After the correction, the AcceptGoods/Passed question distinguished original shared InspectionDone from the new routing. The model did not call `brunch_why` or re-read; the pane stayed at revision 3 with no why record. Do not spend further ordinary-why turns hoping the tool appears.
- **2026-09-10 — Create-new picker.** File → New splits Build with Brunch and Start blank when an assistant is mounted; Build with Brunch offers Interview first versus Quick preview. Live submenu labels were observed on the receiving-dock net.
- **2026-09-10 — Reopen after ordinary correction.** Closing and reopening the assistant restored the same conversation, revision 3 workpiece, and corrected topology without a new turn.
- **2026-09-10 — Catalogue `?scenario=` collides with document location.** `validateLocalStorageDemoSearch({ scenario: "inventory-purchasing" })` keeps that value as the shared-example simulation-scenario id and stays on the ordinary route. Do not treat that key as a worked-scenario bundle until the location contract is recut.
- **2026-09-10 — Inventory-scale admission is still place/transition/arc.** Selected `mutate_petrinet` refuses addType, addDifferentialEquation, addParameter, addScenario, and addMetric. Do not start an Inventory-scale live construction against an add/remove-node catalogue.

## Fog-line

- **Batch carrier and value:** local mechanics, selected-schema following, product-path execution and 5.8× faux settlement now hold, a live 25-operation tracer generation applied, ordinary start-from-scratch applied an 11-operation `mutate_petrinet`, and an ordinary correction then rewired that net with remove+add. Ordinary and updated why were fluent without a `brunch_why` record. File → New now splits Build with Brunch and Start blank. Do not deepen past Inventory-scale admission, a non-colliding catalogue key, or a later why-tool strain that is not another ordinary-language follow-up.
