...
| --- | --- | --- |
| Conversation → workpiece → model → why | Primary; consumes Part A's bounded contracts | Ordinary configured Brunch now mounts the batched construction catalogue; Inventory-scale construction, create-new UX and a live ordinary-provider turn remain unproved. |
...
- **Batch carrier and value:** local mechanics, selected-schema following, product-path execution and 5.8× faux settlement now hold, a live 25-operation generation applied in one `mutate_petrinet`, and ordinary configured Brunch mounts that catalogue without `?brunchTracer=`. Do not deepen past Inventory-scale chunks, create-new UX or a live ordinary-provider turn.
