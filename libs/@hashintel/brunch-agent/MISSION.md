...
- **Next decision — Lu:** supply the bounded Brunch/persona model and run allocation under [execution safety](evaluations/README.md#execution-safety). The next product observation is whether a real Inventory persona develops a useful account and model on this browser-executed path; no paid inference has been authorized or run for the mechanism proof.
