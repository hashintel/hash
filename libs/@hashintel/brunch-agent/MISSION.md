...
- **Ledger fold** — partly landed 2026-09-11: `apps/brunch-agent/src/conversation/net-ledger.ts` folds canonical history into `read | mutation | layout | unrecorded` events; `net-freshness.ts` is its first consumer (a recorded layout is now a known change, not an unrecorded one) and `recordedBrowserObservation` lives there. The owner's authority constraints are pinned as tests in `test/net-ledger.test.ts` before any consumer moved: deterministic projection over Flue history, not a store — recomputable and unpersisted; no new event identities (each event is the assistant's own tool call at its own position); missing or ambiguous records preserved as `unrecorded` with their reason, never repaired or dropped; no live Petrinaut observation as input; history order kept, never reordered or deduplicated. Open: `why.ts` still carries its own attribution walk; moving it onto the ledger's mutation events (which would then carry verified attempts and declared bases) waits on the why-scope review.
