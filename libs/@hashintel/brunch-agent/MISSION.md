...
**Current throughline:** cross the real product boundary with the smallest candidate-mode `mutate_petrinet` tracer and compare the same 20–30-operation construction through batch versus individual operations. One direct `anthropic/claude-sonnet-4-6` call passed the selected-schema carrier on its first attempt: one valid 25-operation tool call in 28.3 seconds, 5,287 input / 3,041 output tokens, with unique IDs and every arc referencing earlier created nodes. The product comparison has not passed: real attempts exposed missing browser-tool classification, missing AI SDK dynamic-tool projection, and Sonnet combining `getLatestNetDefinition` with construction-resource reads in one proposal. The first two defects now have deterministic regression coverage; the remaining mixed proposal is refused because the unguarded production contrast can continue without the browser result, and candidate guidance now states the strict server-only → observed read → mutation proposal order. No successful batch or individual timing/equivalence result exists yet. Product diagnostics now exist (`286e5780c2`, `b475155f53`, `638a0c2d02`): every non-Voice chat failure reaches the server terminal (`[brunch] flue.*`, with runtime IDs and, in development, the original error) or the browser console (`[petrinaut] brunch.*` / `ai-assistant.*`), and the hermetic admission probe asserts it. The first attributable real attempt (2026-09-10, `?brunchTracer=root-creation`, roughly five Sonnet requests, usage not ledgered) followed the server-only → observed read → proposal order but shaped every `mutate_petrinet` operation flat as `{operationId, type, input}` instead of `{basisId, operation: {type, input}}`; server validation refused it twice, and a third call with the same shape was admitted and then refused by the browser's `mutatePetrinetInputSchema`, so the server and browser argument schemas also disagree. The batch premise is therefore blocked on carrier shape through the real path, not on opaque failure; the next decision is whether to align the model-facing schema/guidance with what Sonnet emits or to reshape the carrier, and to make the two schemas one. Lu authorizes Mission 7b development inference below USD 100 total per day and directs sapper-like execution: record usage, stop on material failure, but do not turn each bounded reversible step into another ceremony gate. Scenario seeding, remote write, release and Linear write remain separate.
