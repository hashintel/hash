...
- **Live observation — Lu records; builder follows the first failure:** headed Chrome at `http://127.0.0.1:4925/`, titled `Brunch persona · run-PUmmN6 · ready to record`, has passed its recording pause and is running. Herdr tab `Inventory recording` (`w0:t14`) contains launcher `w0:p43` and Pi persona `w0:p44`. Local run: `apps/brunch-agent/.data-wipe-me/persona-runs/run-PUmmN6/`. Empty start was inspected before release; both participants now have completed paid Sonnet requests in the shared US$100 ledger. The opening's browser read succeeded, but two initial workpiece writes failed with `Workpiece baseRevisionId does not name the current revision.` (`evidence/snapshot.json`, `evidence/trace.md`); Brunch continued the interview and Pi supplied its next utterance. This proves the live connection, not workpiece settlement or Inventory acceptance. Preserve the running session; do not repair its model by hand.
