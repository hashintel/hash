...
**Immediate blocker:** the [r2 request](docs/evidence/implementations/fe-1573-step-a/a5-real-provider-20260909-r2-outcome/result.md) returned HTTP401 `invalid x-api-key`. The shared ledger records six attempts, five complete, US$0.09113535 confirmed catalogue spend, one unknown request and US$7 held. Secure credential correction and explicit accounting/authorization disposition precede another paid request. Old activations are consumed. This recut authorizes neither a retry nor a hold release.
...
### Network and paid authority

**Development-network amendment — Lu, 2026-09-09:** ordinary dependency installation, builds and documentation research may use the network; missing cached dependencies are not by themselves an AFK blocker. Use repository tooling and pinned dependencies, without unrelated upgrades or sending private material to external services. Synthetic tests must not fall through to live providers. Apply verified OS process-tree denial with the retained [native-delivery profiles](docs/evidence/implementations/fe-1573-step-a/native-local-delivery/installation.md) when hermetic isolation is part of the claimed proof; local browser/listener proofs use the scoped loopback profile. Offline package flags alone do not establish denial, and a failed hermetic run is not silently retried online as equivalent evidence. This replaces the blanket development restriction, not any frozen experiment's network contract. Earlier downloads remain incidents under their then-current authority. Provider authentication checks require an explicit opt-in; paid inference retains the separate gates below. Follow the [interactive-work procedure](docs/agents/interactive-work.md) for configuration and delegation readiness.

The shared Step A envelope remains **US$100 / 200 combined calls**, whichever is exhausted first; it is not current launch permission. Exact model is `anthropic/claude-sonnet-4-6`, including a persona if used; no fallback to default Haiku. Count every participant, preparation, continuation, compaction, failure and retry. At most three rejected attempts of a canonical operation. Each request needs a conservative token/cost hold before invocation, actual identity and terminal usage reconciliation below admission. Unknown or pending-journal accounting blocks further dispatch; normalized zeros do not prove zero cost. Catalogue estimates are not invoices.

The integration owner alone allocates named runs against [usage-ledger.json](docs/evidence/implementations/fe-1573-step-a/usage-ledger.json) and [attempt-ledger.md](docs/evidence/implementations/fe-1573-step-a/attempt-ledger.md), with one delegated writer, not a clone. After Lu resolves the current credential/accounting stop, the existing bounded allocation delegation applies inside those limits; material route, model, budget or participant-accounting changes require a decision first. This amendment does not resolve that stop or allocate funds.
