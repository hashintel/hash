...
Make a product manager able to open a stable Inventory purchasing worked model from one Postgres-backed template (`?bundle=inventory-purchasing`) as an independently owned working copy, inspect a legible sophisticated SDCPN with its retained Brunch conversation and workpiece, ask why consequential content exists, correct it in ordinary language, see compiler-clean model changes and coherent layout, and close and reopen the same copy — with the template and sibling copies untouched, and a clean copy available on request.

Inventory is the fully recorded flagship and acceptance exemplar. It is not permitted to be the only path that works: no scenario-specific nouns or IDs in reusable guidance or execution. Portfolio evidence is tiered: Inventory receives flagship-level recording; all six named packs receive compatibility/capability probes through the construction route; at least one non-Inventory pack receives a fresh end-to-end construction and correction run through the product route, succeeding through canonical operations or refusing visibly where a capability is genuinely unsupported. Full end-to-end proof for every pack is not claimed.
...
```text
?bundle=inventory-purchasing → one owned working copy of the template
...
→ close/reopen resumes the same copy: conversation, workpiece and net coherently
```
...
- [`../../../apps/brunch-agent/src/conversation/why.ts`](../../../apps/brunch-agent/src/conversation/why.ts) and [`net-ledger.ts`](../../../apps/brunch-agent/src/conversation/net-ledger.ts) — history-backed explanation and the candidate shared projection (fog-line) that freshness already consumes.
- Website `local-storage-demo` host, `mutate-petrinet-tool.ts`, `mutation-record.ts` and `local-storage-demo-search.ts` — browser execution, `layoutRecord`, and URL/mode ownership. The search schema has no `bundle` parameter yet; `inventory-purchasing` is a `scenario=` document location. Crew reservation, whose selector, manifest, local-storage snapshot and preparation flow also live there, is a legacy, test-authored Mission 6 resume fixture. It is regression evidence only: not current demo content, provenance evidence, a worked-model template, or an owned-copy implementation. Do not adapt it into the Inventory path or cite it as precedent for one; once the real Inventory bundle path lands, evaluate removing its user-facing machinery and relocating surviving fixtures under tests.
- [`../../../apps/brunch-agent/src/database-config.ts`](../../../apps/brunch-agent/src/database-config.ts) — the SQLite/Postgres adapter the catalogue/copy path builds on.
...
**Product-manager script:** open `?bundle=inventory-purchasing` with Brunch selected and receive an owned working copy, inspect the connected model and workpiece, ask why two consequential elements exist, correct one operational fact in ordinary language, see a compiler-clean bounded net change and coherent layout, close and reopen the same copy, confirm the template and a sibling copy are unchanged, and create a clean copy. Then switch to the stock assistant and confirm its tools and history are its own.
...
| Capability-matrix coverage | Every admitted operation is exercised through the website-owned host executor and verifies `applied` at the receiving boundary; an unadmitted operation is refused at its position with the admitted list, nothing applied; the three admitting schemas agree. This is unit/host evidence; representative live-browser construction and correction across the matrix is owed by the portfolio and persona rows below, not by this one. | Yes at the host level — [`mutation-capability-matrix.md`](docs/reference/architecture/mutation-capability-matrix.md): 22 admitted shapes (add, edit, remove), `mutate-petrinet-tool.test.ts`, `mutate-petrinet.test.ts`, `selected-mutation-batch.test.ts`. No live-browser run per shape is claimed. |
...
| Stock-mode isolation | With the flag off, stock transport, tools and history are the same as before Brunch existed; switching does not merge histories or reinterpret prior tool calls. | Yes — `apps/petrinaut-website/.../assistant-selection.ts`, palette commands in `local-storage-demo-app.tsx`, tests under `assistant selection` in `local-storage-demo-app.test.tsx`: stock selected mounts no executor, tools, Workpiece pane, Voice or durable Stop, talks to `/api/chat`, keeps its messages in the local store; a Brunch-side write never reaches that store and the stock history is intact after switching back. Live-checked in the dev pair both ways. Residual: the byte-for-byte flag-off baseline comparison in the readiness gate is not yet an automated snapshot. |
| Inventory template and owned copies | `?bundle=inventory-purchasing` resolves one Postgres-backed template; each open yields an independently owned working copy; close/reopen resumes the same copy (conversation, workpiece, net); a clean copy can be created; template and sibling copies are isolated from a copy's edits. | **Not yet.** Prepared local fixture-resume machinery exists for crew reservation. No Postgres template/owned-copy path exists yet: the website search schema has no `bundle` parameter, `inventory-purchasing` is a `scenario=` document location (`local-storage-demo-search.test.ts`), and the Brunch app has no catalogue or copy tables. |
| Portfolio portability | Tiered: compatibility/capability probes over all six named packs under `evaluations/cases/` through the construction route; a fresh end-to-end construction and correction for at least one non-Inventory pack through the product route, with representative live-browser exercise of the capability matrix; reusable guidance contains no Inventory nouns or IDs. | **Not yet.** Packs exist; no probe suite runs them through the construction route; no second pack has been built end to end. |
| Realistic persona construction recording | A persona interview in ordinary language, recurring workpiece revisions, construction through the visible product in a live browser, native Brunch and Petrinaut records, no operator-built net, compiler repair where relevant, layout and fresh-hash coherence, recorded why traversal, correction and reopen of the same copy. | **Not yet.** One operator-driven live run exists as a witness, not as recorded evidence. Synthetic fixtures remain mechanism tests. |

### Readiness gate

The mission completes when Lu performs the product-manager script on an owned copy of the Inventory template without IDs, tool vocabulary or developer repair, the tiered portfolio contract below is met, and:

| Result | Required oracle |
| --- | --- |
| Inventory template and copies behave as one worked model | `?bundle=inventory-purchasing` opens an independently owned working copy; edits in it never reach the template or a sibling copy; close/reopen resumes the same copy; a clean copy is created on request. Arbitrary import/clone, attachments and complete historical-effect rebinding are out of scope. |
...
| Portfolio contract | Inventory receives flagship-level recording. All six named packs receive compatibility/capability probes through the construction route. At least one non-Inventory pack receives a fresh end-to-end construction and correction run through the product route. Full end-to-end proof for every pack is not required and not claimed. |
| Ordinary requests inside the envelope succeed or refuse visibly | Sampled ordinary construction and correction requests across the capability matrix on the live copy, in the browser: each succeeds through canonical operations or produces a clear unsupported-operation refusal; none crashes, stalls, corrupts the net, silently omits meaning, leaves hidden partial state or claims success. Host-executor tests do not satisfy this row; a scripted sequence passing is insufficient. |
...
- No Petrinaut simulation scenarios or metrics, structured-question widgets or questionnaires — pending PM confirmation as non-goals.
- No arbitrary import/clone, attachments or complete historical-effect rebinding for worked-model copies; the copy path is one Inventory template, owned copies, same-copy resume, clean-copy creation and template/sibling isolation.
- No end-to-end construction proof for every named pack; the tiered portfolio contract in the readiness gate is the claim.
...
- **Persistence:** Flue history is canonical conversation history; the workpiece is the recoverable operational account; Petrinaut is the model authority; the Inventory template and its owned copies live in the Brunch app's Postgres store through the existing adapter and migrations, with SQLite for lightweight tests and disposable local work only. No second persistence system, and no projection over these becomes a store.
- **Identity:** a `?bundle=` key identifies a template; it does not authenticate a user by obscurity. A secret URL proposed as a bearer capability needs explicit review of entropy, logging, sharing, revocation and authorization before it is accepted.
...
- The Brunch app owns composition, authorized history, browser/document reconciliation, freshness, history-backed why execution, operational diagnostics, and the Postgres catalogue/copy path (template resolution, copy ownership, same-copy resume, clean copy) on the existing adapter and migrations.
- The Petrinaut website owns browser execution, diagnostics/layout host integration, assistant-mode selection and URL routing, including the `?bundle=` route.
...
- **Portfolio portability probe** — the named packs exist under [`evaluations/cases/`](evaluations/cases/): Vestera Scheduling, Data Centre Thermal Operations, Industrial Gas VMI, Pharma Cold Chain, Semiconductor Fab Operations, Truck Fleet Maintenance (Inventory is the flagship, not one of them). The contract is settled (tiered: probes over all six, one non-Inventory pack end to end). Open: the shape and turn budget of the compatibility/capability probe per pack, which pack takes the end-to-end run, and the scan that keeps Inventory nouns out of reusable guidance.
...
- **Shared history projection** — the product contract is shared interpretation of canonical Flue history between freshness and why, with canonical-history fidelity; it is not the existence or name of any module. A shared deterministic projection over canonical Flue history is the candidate seam. Extract it only if parity tests show that it removes duplicate interpretation without creating a second store, identity scheme or authority. Authority constraints, whatever the shape: recomputable and unpersisted; no new event identities (each event is the assistant's own tool call at its own position); missing or ambiguous records preserved with their reason, never repaired or dropped; no live Petrinaut observation as input; history order kept, never reordered or deduplicated. State on this branch: `apps/brunch-agent/src/conversation/net-ledger.ts` is one candidate, consumed by `net-freshness.ts` (a recorded layout is now a known change, not an unrecorded one) with the constraints pinned in `test/net-ledger.test.ts`; `why.ts` still carries its own attribution walk. Whether why moves onto the candidate, or the candidate folds back into freshness, is decided by the parity test and the why-scope review, not assumed.
...
- Stop the copy path if a copy's edits can reach the template or a sibling, or if reopen cannot resume the same copy; do not widen it into import/clone or attachments to work around that.
- Stop extracting a shared history projection if the parity test shows it introduces a store, an identity scheme or a second authority; keep two walks over canonical history rather than one wrong seam.
...
- Crew-reservation fixture retirement: after the Inventory bundle path lands, evaluate removing the user-facing prepared-fixture selector, manifest and preparation flow from the website host, keeping only the frozen histories that still serve as regression fixtures, relocated under tests. Archives keep their historical references.
...
- **2026-09-11 — 7c stacks on the freshness side quest.** Reparented onto `ln/fe-1653-alternate` so `layoutRecord` and the freshness marker read one interpretation of history rather than two.
- **2026-09-11 — second reconciliation review, four revisions.** (1) The Postgres-backed Inventory template with owned copies, same-copy resume, clean-copy creation and template/sibling isolation is restored as 7c scope in the imperative and readiness gate; arbitrary import/clone, attachments and complete historical-effect rebinding stay out. (2) The portfolio claim is tiered: Inventory flagship recording, probes over all six named packs, one non-Inventory pack end to end; the earlier "any named pack" wording is withdrawn. (3) The shared history projection is a candidate seam, extracted only on parity evidence; `net-ledger.ts` is not a predetermined module. (4) Capability-matrix evidence is stated as website-owned host-executor coverage; live-browser exercise of the matrix is owed by the portfolio and persona rows.
