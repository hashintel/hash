...
- Website `local-storage-demo` host, `mutate-petrinet-tool.ts`, `mutation-record.ts`, `local-storage-demo-search.ts` and the `resolve-crew-reservation-bundle` / `use-crew-reservation-fixture-session` pair — browser execution, `layoutRecord`, URL/mode ownership, and the only bundle/copy path that exists today (crew reservation; Inventory is still a `scenario=` document location).
...
- **Persistence:** Flue history is canonical conversation history; the workpiece is the recoverable operational account; Petrinaut is the model authority; the Postgres-backed catalogue holds the Inventory template and its owned copies. No second stores beyond these, and no projection over them becomes a store.
...
- The Petrinaut website owns browser execution, diagnostics/layout host integration, assistant-mode selection, URL routing, and the bundle/copy path (template resolution, copy ownership, resume, clean copy).
