...
- **Current work — builder:** early incremental construction guidance remains approved but outstanding, followed by the pending diagnostics and tool-progress investigation. No developer-authored model repair counts as proof; do not automatically resume a retained run while resolving these findings.
...
- **Live inference — builder:** both participants produced paid replies. The launcher owns its services and fresh local store; it does not reuse unverified servers. Persona budget enforcement is removed under owner direction; native usage remains an estimate, not an invoice, and historical unknown usage stays unknown. The run deferred construction until the end and created only support definitions before the accounting stop. The approved incremental-construction guidance has not yet been implemented or observed with a real model.
