...
**Current throughline:** cross the real product boundary with the smallest candidate-mode `mutate_petrinet` tracer and compare the same 20–30-operation construction through batch versus individual operations. One direct `anthropic/claude-sonnet-4-6` call passed the selected-schema carrier on its first attempt: one valid 25-operation tool call in 28.3 seconds, 5,287 input / 3,041 output tokens, with unique IDs and every arc referencing earlier created nodes. The product comparison has not passed. Missing browser-tool classification and AI SDK dynamic-tool projection now have deterministic coverage; mixed `getLatestNetDefinition` plus construction-resource reads in one proposal remain refused. Product diagnostics now exist (`286e5780c2`, `b475155f53`, `638a0c2d02`). The first attributable real attempts showed the throughline defect is premature browser dispatch, not schema drift: server and browser share one Zod schema, and admission never checks argument shape. `mutate_petrinet` was omitted from `validatedClientToolNames`, so Flue's pre-validation `assistant_tool_call` became an executable `input-available` part; a later rejected `tool-output-error` was dropped while the panel still scheduled the bad call, terminated the turn, and never submitted `client-tool-result`. The wrapped `{basisId, operation:{…}}` dialect plus refine-only `targetSubnetId`/`endpoint` exclusions produced the flat shape the model actually emitted. The accepted repair is gate-then-flatten: withhold `mutate_petrinet` until server validation, mark gated `tool-input-start` `dynamic: true`, flatten operations to `{operationId, basisId, type, input}` with envelope descriptions, and make root-only exclusions structural. No successful batch or individual timing/equivalence result exists yet. The next observation is a rejected-then-accepted real-path retry that executes only the accepted call and lands a `client-tool-result`. Lu authorizes Mission 7b development inference with no daily spend cap and directs sapper-like execution: record usage, stop on material failure, but do not turn each bounded reversible step into another ceremony gate. Scenario seeding, remote write, release and Linear write remain separate.
...
- **Batch carrier and value:** local canonical mechanics pass and the selected-schema Sonnet carrier passed once. Premature dispatch and the wrapped operation dialect are the accepted product-path defects; the gate-then-flatten repair is in hand. Real browser settlement latency, aggregate effect/mutation recording and equivalent individual comparison remain unproved until a rejected-then-accepted retry lands a `client-tool-result` and the batch/individual comparison can run. Do not deepen past that observation.
