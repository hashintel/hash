...
| --- | --- |
| Batch premise is worth implementing | Local mechanics already establish ordering, partial outcomes and equivalence. Product-path faux settlement now matches hash at 5.8× for 25 operations versus 25 one-operation calls. The remaining discriminator is one real-provider 25-operation product-path generation. Failure of that generation returns to the construction design discussion without production batch code. |
...
- **2026-09-10 — Empty-net batch versus one-operation comparison.** The spike's 25-operation target produces the same definition hash through one `mutate_petrinet` and through 25 one-operation calls on `?brunchTracer=root-creation` (412 ms versus 2,373 ms, 5.8× settlement, all 25 applied). Compare chunk size on the empty-net batched tracer; do not treat `?brunchTracer=construction` or the stale root-creation individual-tool suite as that baseline. A real-provider 25-operation product-path generation remains the next discriminator.

## Fog-line

- **Batch carrier and value:** local canonical mechanics pass, the selected-schema Sonnet carrier passed once, a live three-operation product-path batch lands `client-tool-result`, and a faux empty-net comparison now matches hash with 5.8× settlement for one 25-operation call versus 25 one-operation calls. A real-provider 25-operation product-path generation is still unproved. Ordinary Brunch still gates useful construction behind tracer/fixture modes. Do not deepen past that paid generation or the first ordinary create/open path.
