...
- **Next preparation — builder:** pin `claude-sonnet-4-6` for Brunch and Pi, verify both actual provider selections, and connect the combined US$100 ceiling to request accounting under [execution safety](evaluations/README.md#execution-safety). Lu has supplied the allocation; the current launcher disables the historical accounting hook, so enforcement remains unverified. The next product observation is the live Inventory persona in headed Chrome. The mechanism proof used no live inference.
...
- **Run accounting — builder:** the model/budget choice is settled in Owner decisions. Verify that the live launch accounts for both participants and stops within the combined ceiling; an existing server may retain a different model/configuration, and the launcher currently omits the historical accounting hook. Configuration presence alone does not establish enforcement.
