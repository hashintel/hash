...
- **Established base:** the persona launcher now submits through the real browser composer. The synthetic browser proof passes: opening and subsequent client continuations, two visible net/workpiece updates, tab switching during execution, abort settlement and no replay on reload. This is [mechanism coverage](#throughline-proof-floor), not Inventory acceptance. The Inventory opening and persona request construction from scratch; operational source facts and the separate hand-built reference are unchanged.
...
- **Observed run, stopped:** local-only `apps/brunch-agent/.data-wipe-me/persona-runs/run-PUmmN6/` retains 14 completed user turns, three failed first-workpiece writes followed by nine successful revisions, and no net mutation or layout calls (`evidence/snapshot.json`, `evidence/trace.md`, `evidence/net.json`). The net is empty, not off-screen. Brunch's final reply asks for a read-back before construction. This proves live Sonnet participation and workpiece development, not construction failure or Inventory acceptance. Preserve the original stores and shared budget ledger; do not repair the model by hand.
- **Next observation — builder with Lu recording:** add and verify same-run resume, then reopen `run-PUmmN6` at a recording pause. Its database is ahead of the last exported snapshot: Pi's next correction was admitted, workpiece revision 10 and two construction-resource reads committed, and the interrupted submission remains marked running. Reconcile through Flue before Pi speaks again; never resend the correction or edit canonical records. Keep request 61 unknown with its full US$7.92 hold against the existing US$100 allocation. Start no backend recovery/inference before Lu releases the pause. XYFlow framing remains a separate observation after actual construction.
...
| Persona turns execute through the visible browser | [Persona construction integration](../../../apps/brunch-agent/test/persona-construction.integration.ts): built ChatAgent, synthetic provider, registered Pi extension, private IPC and ordinary Chrome composer; inspect native settlements, actual net/workpiece and screenshots. | Passed 2026-09-13. Local-only evidence: `apps/brunch-agent/.data-wipe-me/persona-runs/persona-construction-vhBnsP/` (`initial-snapshot.json`, `snapshot.json`, `stopped-snapshot.json`, `net.json`, screenshots, `execution.log`). No live Pi model or construction-quality claim. Stop records `aborted` but its existing HTTP endpoint logs an admission-body error. Synthetic nodes use visible coordinates; automatic viewport framing is not proven. The legacy spectator integration still fails its obsolete construction-mode validator and is not this path. |
