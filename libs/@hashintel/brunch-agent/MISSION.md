...
- **Next observation — Lu/builder:** choose the smaller fresh persona-run scope and recording start, then observe whether model-originated Petrinet mutations execute. No new paid run has started. Preserve the original stores and combined budget; request 63 still needs explicit unknown-usage disposition before further paid execution. Do not replay the interrupted utterance or repair the model by hand. XYFlow framing remains a separate observation after actual construction.
...
| Persona turns execute through the visible browser | [Persona construction integration](../../../apps/brunch-agent/test/persona-construction.integration.ts): built ChatAgent, synthetic provider, registered Pi extension, private IPC and ordinary Chrome composer; inspect native settlements, actual net/workpiece and screenshots. | Passed 2026-09-13. Local-only evidence: `apps/brunch-agent/.data-wipe-me/persona-runs/persona-construction-6qqOeJ/` (`initial-snapshot.json`, `snapshot.json`, `stopped-snapshot.json`, `resumed-snapshot.json`, `net.json`, screenshots, `execution.log`). Includes pre-completion prose, admitted tool display and ordinary panel Stop without admission-body errors, pending-write execution or replay. No live Pi model or construction-quality claim. Synthetic nodes use visible coordinates; automatic viewport framing is not proven. The legacy spectator integration is not this path. |
