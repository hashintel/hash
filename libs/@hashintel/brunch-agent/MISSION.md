...
Apply the standing [development defaults](AGENTS.md#development-and-evaluation-execution) and [evaluation execution safety](evaluations/README.md#execution-safety). The [native-delivery profiles](docs/evidence/implementations/fe-1573-step-a/native-local-delivery/installation.md) remain available for this mission's hermetic replays, not mandatory for ordinary development. Existing frozen instruments and original download incidents keep their historical dispositions.
...
The integration owner alone allocates named runs against [usage-ledger.json](docs/evidence/implementations/fe-1573-step-a/usage-ledger.json) and [attempt-ledger.md](docs/evidence/implementations/fe-1573-step-a/attempt-ledger.md). After Lu resolves the current credential/accounting stop, the existing bounded allocation delegation applies inside those limits; material route, model, budget or participant-accounting changes require a decision first. Standing guidance does not resolve that stop or allocate funds.
