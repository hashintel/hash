...
**Current throughline:** prove selected-schema batch generation with one real `anthropic/claude-sonnet-4-6` call, then—if it passes—cross the real product boundary with the smallest candidate-mode `mutate_petrinet` tracer and compare the same 20–30-operation construction through batch versus individual operations. The prior combined no-edit probe stopped before generation because neither shipped side has a tool-injection seam; no request or browser mutation occurred. Lu authorizes Mission 7b development inference below USD 100 total per day and directs sapper-like execution: record actual usage, stop on material failure, but do not turn each already-bounded reversible step into another ceremony gate. Production acceptance, scenario seeding, remote write, release and Linear write remain separate.
...
The combined no-edit browser/provider probe stopped honestly: the shipped ChatAgent and website recognize static tool catalogues, and a temporary alternate application would not have proved the product. The current route is therefore two direct steps without another owner gate between them. First, make one real Sonnet call against a Core-derived selected `addPlace`/`addTransition`/`addArc` batch schema and stop on material schema/carrier failure. If it passes, implement the smallest candidate-mode product tracer through actual Flue deferred-tool correlation and browser canonical execution, then compare one coherent 20–30-operation batch with the equivalent individual-operation route. Record validity, repairs, request count, elapsed time, actual usage, final definition/hash equivalence and partial outcomes. Retain or deepen the tracer only for a bounded correctable defect; remove it if the batch premise fails. A provisional 5× improvement is a strain detector, not a sacred benchmark.
...
- **2026-09-10 — Ordered mutation probe.** Local canonical mechanics/equivalence/partial outcomes pass; local latency, full-schema size and provider/browser evidence do not. The first combined no-edit real probe stopped before generation because no runtime injection seam exists. Proceed directly with one selected-schema Sonnet call, then a minimal candidate-mode real product tracer and full 20–30-operation comparison if it passes. Retain the tracer only if the premise survives. Development inference below USD 100 total per day is authorized; favor bounded reversible mission progress over repeated ceremony and re-grill after the real comparison, not between its already accepted steps.

## Fog-line

- **Batch carrier and value:** local canonical mechanics pass, but the full schema exceeds 64 KiB and local outcome accounting is slower. The absent injection seam makes a no-edit combined probe unavailable, not batching invalid. Selected-schema Sonnet array reliability is the immediate observation; passage authorizes the minimal real product tracer and comparison without another design round.
