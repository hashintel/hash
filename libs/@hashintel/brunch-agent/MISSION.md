...
| Ordinary why routes | Live witness only, `claude-haiku-4-5`, dev pair, after the routing instruction in `apps/brunch-agent/src/agents/chat-agent/agent.ts`. Fresh conversation on `?brunchTracer=construction`: "Why is there an Under final inspection stage between Batch ready and Ready for dispatch?" produced `getLatestNetDefinition` alone, then two `brunch_why` calls citing that `toolCallId` (`kind: "transition", name: "Sign-off"`; `kind: "place", name: "Under final inspection"`), refused honestly as "Current workpiece state is unknown", and the Workpiece pane rendered the structured result. Two residuals: in the conversation that had just built the net the model still answered from memory; and before the instruction named the two-turn sequence, the model proposed both tools together, which admission refused as a whole and the user saw as an error toast. An automated real-model case set needs a new paid-instrument reservation. |
