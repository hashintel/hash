...
- **Next observation — Lu starts recording; builder observes:** headed Chrome is staged at `http://127.0.0.1:4925/`, titled `Brunch persona · run-PUmmN6 · ready to record`; Herdr tab `Inventory recording` (`w0:t14`, launcher `w0:p43`) waits for Enter before the opening. Local run: `apps/brunch-agent/.data-wipe-me/persona-runs/run-PUmmN6/`. Empty net/panel and zero requests were inspected. Brunch and Pi are pinned to `claude-sonnet-4-6` with one US$100 ledger; [connection checks](#throughline-proof-floor) pass. No paid conversation or Inventory acceptance has occurred.
...
- **Live inference — builder:** shared accounting is connected and verified with synthetic native responses. The launcher owns its metered services and fresh local store; it does not reuse unverified servers. It uses the existing instrument's full-window dollar holds, unresolved-usage stop and no-retry contract, so it may stop below US$100. Catalogue costs are not invoices. Verify actual paid replies and settlements on both sides after the recording starts; configuration and request-free Pi startup do not establish credential validity or model quality.
