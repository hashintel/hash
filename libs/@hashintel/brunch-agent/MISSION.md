...
- **Current work — builder:** remove persona accounting enforcement on both participants, including launch/resume budget and unknown-usage gates, per Lu's direction below. Preserve native usage evidence, old ledgers, identity/replay protections, recording pause and operator Stop. Verify without paid inference; do not automatically resume any retained run. Early incremental construction guidance remains approved but outstanding, followed by the pending diagnostics and tool-progress investigation. No developer-authored model repair counts as proof.
...
- **Live inference — builder:** both participants produced paid replies. The launcher owns its services and fresh local store; it does not reuse unverified servers. Persona budget enforcement is being removed under owner direction; native usage remains an estimate, not an invoice, and historical unknown usage stays unknown. The run deferred construction until the end and created only support definitions before the accounting stop. The approved incremental-construction guidance has not yet been implemented or observed with a real model.
