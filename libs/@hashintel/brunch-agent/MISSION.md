...
- **Next work — builder:** qualify separate role configuration for OpenAI GPT-5.6 (`gpt-5.6`, Sol alias), low reasoning, on Brunch and Anthropic `claude-sonnet-4-6`, initially low reasoning, on the persona. Compare Chris's canonical optimization manifest and existing method/tool interfaces for configuration-only use before choosing storage or a new host abstraction. No paid inference or upstream integration starts with this documentation update.
