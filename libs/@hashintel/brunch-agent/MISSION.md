...
The owner selected **interruption by speaking with a half-duplex fallback** on 2026-09-06. By default the microphone remains open during canonical assistant playback; accepted user speech cancels and clears assistant output without clearing the input item that caused the interruption. The completed transcript remains the only answer authority and follows the existing admission path exactly once. The browser remembers this preference. When it is disabled, the original half-duplex behavior remains: the microphone closes during playback and **Your turn** opens fresh capture only after provider cancellation acknowledgement and Brunch settlement.
...
1. **Adopted by default — interruption by speaking.** Browser echo cancellation and provider speech detection keep capture open during playback. Accepted speech cancels assistant output only, retains the same input item through completed transcription, and queues that answer while the previous Brunch turn settles.
2. **Retained as an option — half-duplex explicit handoff.** Disabling interruption closes the microphone during assistant playback and restores **Your turn** with the existing acknowledged cancellation barrier.
3. **Rejected — true simultaneous full duplex.** Assistant output does not continue after accepted user speech starts; overlapping two-party audio would make turn ownership and authoritative transcription ambiguous.
...
Completed-transcript authority, configurable turn ownership, admission idempotency,
...
Extend Voice with interruption by speaking, enabled by default and remembered
per browser. Expose the preference as a checkable row named "Interruption by
speaking" in the existing playback menu. Show "Your turn" only when it is off;
do not add a keyboard shortcut. Use the existing browser echo cancellation and
provider speech detection.

Accepted speech during canonical playback cancels assistant generation and
clears output audio only. Keep the microphone and that input item alive through
transcription and submit the completed answer exactly once through the existing
Brunch admission path. Retain an interrupted answer while the previous Brunch
turn settles. Coordinate the session, bridge, and controller overlap gates;
cancel queued and in-flight speech without reviving it on late provider events.
Keep explicit mute, pause, Stop, end, and reconnect lifecycle behavior intact,
and preserve half-duplex handover when the preference is off.

Proof: test speech_started, response.cancel, output_audio_buffer.clear, and
submission of the same transcript identity through the real session/bridge/
controller seam, with no input buffer clear. Cover delayed admission, pending
response creation, late cancellation events, duplicates, lifecycle cleanup, and
preference/UI behavior. Mocked event tests establish protocol behavior, not
real-world acoustic echo rejection. Update the user guide and patch changeset.
This scope authorizes changes in the website Voice implementation and shared
Petrinaut Voice state, controls, and UI, superseding half-duplex-only constraints
...
→ interruption-aware Voice controller and shared panel submission path B
...
**Release note:** Voice now submits only what the microphone actually transcribed and, by default, lets a person interrupt assistant playback by speaking without losing their words. A browser-saved playback option restores the safe **Your turn** handoff. Voice can replay the exact full response or exact Brunch-marked question, and client-tool Voice origins survive canonical reopen. Restoring the Voice chip on direct spoken user messages remains blocked on an upstream Flue user-metadata contract.

**Demo script:** run `yarn dev:brunch` and select the Brunch preview. Speak one answer and see exactly one matching user turn. While Brunch is speaking, interrupt by speaking; confirm assistant audio stops and the same utterance appears once without repetition. Disable **Interruption by speaking** in the playback menu, confirm the microphone remains closed during output, choose **Your turn**, wait for the handoff, and speak again. After the response and audio settle, use the playback menu to read the full response exactly and repeat only the exact Brunch-marked question; a missing or unmatched marker keeps that action disabled. Start another turn, press durable **Stop** before settlement, and see a stopped turn rather than a Voice error. Hard-reload the settled conversation and confirm the canonical turn remains without resubmission or replay; direct-user Voice-chip restoration additionally waits on the Flue projection seam.
...
1. **Completed-transcript authority and configurable turn ownership.** Realtime session configuration has no tools and no model-created semantic-VAD response. Only a unique completed transcript can reach the shared panel submission path. With interruption enabled, accepted speech during canonical playback cancels assistant generation, clears output audio only, and keeps the same input item alive through transcription; a completed answer waits while the previous Brunch turn settles and submits exactly once when admission reopens. With interruption disabled, canonical speech invalidates unfinished accepted items and closes the microphone exactly as before; **Your turn** opens only a post-barrier input turn after provider cancellation acknowledgement and Brunch settlement. Duplicate, empty, failed, unavailable, stale, disallowed playback-overlapping, and over-limit transcripts do not submit and produce the specified passive or recoverable notice. Oracle: cases in `openai-realtime-session.test.ts`, `realtime-brunch-bridge.test.ts`, `voice-turn-controller.test.ts`, `voice-interview-control.test.tsx`, and `voice-preview.integration.test.ts`.
...
- With interruption by speaking enabled, keep the microphone open during canonical playback and preserve accepted input through output-only cancellation and completed transcription. With it disabled, close the microphone from the canonical speech request through output, cancellation, pause, error, and submission states; only a **Your turn** handoff completed by both provider cancellation acknowledgement and Brunch settlement can establish fresh post-request capture. A cancellation promise is part of the manual turn boundary, not a cosmetic animation state.
...
```text
~ apps/petrinaut-website/src/main/app/voice-interview/             transcript authority, interruption and half-duplex state, cancellation, replay tests/code
