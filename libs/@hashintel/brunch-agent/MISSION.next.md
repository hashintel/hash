...
- **Patch-style workpiece mutation:** the live mission keeps full-document resubmission and reduces duplication through tool defaults and model-context projection. Re-enter with diff or edit operations if one or two full bodies per revision still strain the prompt after that remediation, or when workpieces exceed roughly 8k tokens. Require the same revision, evidence and recovery contract as full resubmission; a patch must not become a second document authority.
