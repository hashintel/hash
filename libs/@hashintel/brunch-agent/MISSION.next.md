...
- Live [Mission 7e](MISSION.md) owns the Stage 1 no-silent-shrink guard, net-read projection economy, refusal-repair guidance, tool-row lifecycle colour and the carried contract closures needed for a fresh long-run observation. It consumes `run-SB5pgx`'s measured strain without inheriting Mission 7d semantic acceptance. Section-keyed workpiece operations require the Stage 1 latency discriminator and a separate owner-accepted recut.
...
Mission 7d's engineering branch closed without accepting the Inventory worked example. After Mission 7e makes the long-run path usable, an owner must recut or explicitly re-admit the remaining product bar before claiming the flagship complete:
...
The recovery source is deliberately unresolved. `history-retention`, not preference, chooses whether the successful output, persistent state or both carry the reconstructed body. Stop if canonical records alone cannot reconstruct every revision and validated locator after fold and fresh-process reopen; do not add a full-body fallback. Required proof includes operation/full-replacement sha256 equivalence, all current text-evidence cases over reconstructed bodies, unambiguous duplicate-heading behavior, explicit lossy removal, and create/fold/reopen recovery.
...
- **Section-keyed workpiece mutation — [after Stage 1](#section-keyed-ledger-operations-after-stage-1):** Mission 7d reduced settlement to one full body per revision; `run-SB5pgx` then showed a 22.3-second median and cost-driven collapse. Mission 7e Stage 1 guards the body and reduces net-read context first. Section operations require its latency discriminator and a recut; a patch must not become a second document authority.
- **Net-observation economy — Mission 7e:** retained `read_petrinaut_net` bodies were about 40% of the pre-compaction prompt in `run-SB5pgx`; layout is a minor share, the bulk is arcs and `lambdaCode`. The live mission owns projection dedupe of superseded reads and conditionally admits compact model rendering while preserving `output.observation.{toolCallId,sha256}`.
