...
Mission 4 selects one mounted plugin job skill with core-authored universal elicitation packaged as a lazy supporting resource. This is a runtime packaging decision, not a transfer of semantic authority: core remains the single authored home for context-, domain-, editor-, and output-formalism-independent elicitation judgment; plugin job skills package that source beside their own domain-typology / target-formalism resources. A concise core prompt and an earned plugin append carry mounted-lifetime invariants; the job skill carries procedures and judgment and discloses resources as needed. Flue `useSkill` is not always-on `useInstruction`.

The Flue composition side quest compared that topology with a separately mounted core `elicitation` skill through three frozen instruments. V3 removed the legacy-core-prompt confound and repeated the discriminating opening case: after `sdcpn-modelling` activation, independent elicitation activated in 0/3 Candidate A runs while packaged universal plus SDCPN disclosure completed in 2/3 Candidate B runs. Both candidates correctly avoided universal disclosure across all four construct-only and resolvable-review restraint runs. The only revised knowledge-gap review pair bypassed the shared job skill in both candidates, so v3 did not behaviorally validate B across jobs; it exposed review routing as Mission 4 content work. The architecture disposition is therefore: select packaged B for current production because A exhibits repeated topology-specific activation strain, while preserving the evaluation’s narrower invalid/both-weak protocol verdict. Do not mount A or run a fourth topology campaign by momentum. Independent skill identity may re-enter if a second real plugin or changed model/framework behavior demonstrates reliable cross-skill activation or material duplication/context strain.

Evidence: [`docs/evidence/evaluations/flue-skill-composition-side-quest-v3/comparison.md`](docs/evidence/evaluations/flue-skill-composition-side-quest-v3/comparison.md), with immutable v1/v2/v3 manifests and raw traces beside it.
