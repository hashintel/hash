...
- [Mission 7e](MISSION.md) reopened after its engineering-partial tie-off for one bounded WP-E: Brunch-owned expected `mutate_workpiece` validation becomes a typed non-applied result, with honest gold/green/neutral/red presentation. Pre-execution schema parsing remains a Flue/Pi error and no substrate patch is admitted. The abrupt-loss guard, net-read projection economy, bounded idle recovery and negative `run-1L2zTl` scalability disposition remain unchanged. Proportional workpiece operations with stable provenance still require a separate owner-accepted recut.
...
Mission 7e implemented bounded provider-idle recovery and graceful launcher Stop but tied off those acceptance leaves before reopening only WP-E. These leaves are not an automatic next mission and do not justify widening WP-E. Re-enter them after the Ledger successor makes the long-run product path viable, or earlier only for a named product-release consumer:

- publish a content-free provider-idle recovering disposition through the existing live-tool SSE path;
