...
> Canonical future-planning spine, shared frame and backlog index only; not execution authority. [`MISSION.md`](MISSION.md) is the sole live authority for Mission 7b's ordinary batched construction/correction/provenance close-out under FE-1573, partially addressing FE-1478 without closing it. [Mission 7a](docs/mission-archive/7a-workpiece-construction-explanation-groundwork.md) is archived on this child branch while its engineering PR completes external review. [Draft Mission 7c](docs/mission-drafts/7c-inventory-worked-model.md) is the substantial Inventory worked-model and provenance successor under Lu's explicit extension of the FE-1573 multi-PR exception. The former Step B programme remains [after-demo evaluation](docs/mission-drafts/7-explainable-construction.md). Older sequencing and deployment assertions below are historical planning inputs, not current execution or environment facts.

This spine, the live Mission 7b contract, Draft Mission 7c, the after-demo evaluation draft and three broader successor drafts form one planning record. The Voice reconciliation draft has been consumed into Mission 6b's parent-branch authority, not retained as competing planning authority. Keep each consequential meaning in one authoritative home: shared future constraints and unallocated concerns live here; future mission-specific detail lives in its packet or draft; live Mission 7 contracts live only in root authority. A spine pointer is not a second contract. Material omitted from a cut returns to this record at full fidelity, and consumed draft content is removed.
...
**Current scope:** [Mission 7b](MISSION.md) closes the ordinary structural `mutate_petrinet` path: selected add/remove place, transition and arc operations; canonical ordered-stop execution and complete effects; one small live ordinary construction/correction/reopen witness; failure attribution; and a `brunchDemoMode`-gated create-new affordance. It explicitly does not claim Inventory scale, code-bearing dynamics, compiler feedback, ELK layout, reliable mechanical why routing, worked-model bundles, Postgres copies or deployment. [Draft Mission 7c](docs/mission-drafts/7c-inventory-worked-model.md) owns those coupled obligations and the tool naming/homing/topology reconciliation required before broadening the catalogue.
...
M7a  FE-1573 engineering PR — workpiece/construction/why groundwork; external review/merge pending
M7b  FE-1573 successor, partial FE-1478 — ordinary batched structural construction, correction, recorded basis/effects, reopen, and experimental create-new entry
M7c  FE-1573 explicit third-PR exception, partial FE-1478 — Inventory worked-model bundle; compiler/layout/why/tool-topology/Postgres-copy closure
...
M10  revision — ship bounded authorized reviewer revision and a scoped patch over basis, mutation records, and epochs
...
Every numbered product mission after the proof-of-life exception must pass the **product-manager litmus**: a product manager who did not watch the work must be able to notice that the product materially moved forward. Each mission therefore states a release-note sentence, a demo script a product manager can run without an engineer, and the thing that was impossible before. Snapshots, manifests, event ledgers, and negative controls are oracles, not the visible advance. Mission 5's litmus is in the FE-1574 branch's authority; Mission 6's is in the [archive](docs/mission-archive/6-resumable-workpiece-petrinaut.md#visible-product-advance); Mission 7b's accepted scope is the ordinary batched structural construction/correction seam, and Draft Mission 7c restores the substantial worked-model product bar. Missions 7b–7c, 9 and 10 name the Petrinaut Brunch panel with the deployment posture actually inspected at cut time. Local is acceptable while Mission 8 has no remote deployment; 7c nevertheless exercises its catalogue/copy path on local Postgres because that path is intended to survive deployment. Architecture, schema repair, fixtures, evaluation, rehearsal and spikes support the advance but cannot be its sole outcome. Lu explicitly extends FE-1573's exception to three Mission 7 PRs; this does not relax one live mission per branch or authorize unrelated Linear writes.

## Successor mission précis

### M7a / M7b / M7c — Groundwork, ordinary batch seam, then Inventory worked model

All three parts track [FE-1573](https://linear.app/hash/issue/FE-1573/construct-and-explain-one-real-net-region-from-a-genuine-conversation), advancing [FE-1478](https://linear.app/hash/issue/FE-1478/provide-provenance-from-a-generated-net-back-to-the-requirements-graph). Lu explicitly extends the prior two-PR exception to a third Mission 7 PR; keep one live mission per branch. Part A's [archived contract](docs/mission-archive/7a-workpiece-construction-explanation-groundwork.md) preserves the bounded mechanical evidence and external PR-closeout state. No historical full-region or semantic-quality verdict is retrospectively changed into a pass.

The live [Mission 7b](MISSION.md) owns the accepted ordered structural mutation carrier, one ordinary small-net construction/correction/reopen path and the gated create-new affordance. [Draft Mission 7c](docs/mission-drafts/7c-inventory-worked-model.md) owns the Inventory-comparable connected model, provider-tested operation admission, compiler feedback and repair, ELK layout, reliable mechanical why routing, tool catalogue/naming/homing topology, Postgres-backed bundle templates and independently owned copies. Petrinaut simulation scenarios/metrics and a second worked-model bundle remain outside by default pending PM confirmation. The [evaluation draft](docs/mission-drafts/7-explainable-construction.md) retains broader semantic/behavioural reliability, expanded passage/adversarial/lifecycle matrices and population claims after the demo.

| Priority | Scope and authoritative home |
| --- | --- |
| Remaining 7a PR closeout | External CI/review/merge of the implemented workpiece display, #9634-compatible fixture chrome and bounded mechanical claim — [archived 7a contract](docs/mission-archive/7a-workpiece-construction-explanation-groundwork.md) and PR #9562 |
| Remaining 7b closeout | Retract unproved type/parameter/dynamics admission; gate create-new affordances on `brunchDemoMode`; repair branch-owned CI; perform a non-paid picker click-through; update PR claims — live `MISSION.md` |
| Before September live demo | Open an owned `?bundle=inventory-purchasing` copy with retained conversation/revisions/workpiece/connected compiler-clean and legible net; inspect why/source classes; correct without unrelated rebuilding; reopen/rehearse — Draft Mission 7c |
| Required September recording | Realistic persona interview, recurring workpiece revisions and native substantial-net construction through the visible product, with retained real records and no hidden operator construction — Draft Mission 7c |
| Demo delivery | Stable bundle URL, coherent template identities, Postgres startup catalogue updates, independently continuable copies, intended host/revision/access and safe rehearsal reset — Draft Mission 7c and deployment section below |
| After demo | Cross-scenario model-quality/behaviour/useful-coverage reliability; expanded passage/adversarial/lifecycle matrices; general repeat/change/retirement/concurrency and reviewer classes; universal dynamics/stochastics, broader source/plugin work and optimisation — evaluation draft, drafts 9–11 and named backlog sections |

Database-backed independent working copies are part of Draft Mission 7c. If executable simulation, optimisation, or accepted Chris/Yannis handoff becomes part of the demo claim, record that tier explicitly rather than implying it from the old six-beat frame. A meaningful recorded construction flow is required; Draft 7c's why/readiness obligations, not the old campaign schedule or a count alone, govern the flagship.
...
The smallest planned provenance seam, to be earned by Mission 7, is: settled workpiece revision identity (call id plus SHA-256), passage locator under the passage policy, optional revision-time evidence relation, stable net-element ids with identity epochs, declared basis per mutation request, and the mutation record. Storage is the Flue log plus per-conversation state; the compaction probe decides whether an archive lane is needed. Stable ids must be exercised rather than assumed. Unsupported defaults, stale or partial state, identity churn, repeated projection, and visible partial failure stay explicit.
...
The earlier "exactly one focused question" and template-gate content repairs were withdrawn on 2026-09-02 with the campaigns that motivated them; the accepted dosage wording and any re-admission of those repairs are owner decisions recorded in [`MISSION.md`](MISSION.md).
...
### Stable template delivery allocated to Draft Mission 7c

[Draft Mission 7c](docs/mission-drafts/7c-inventory-worked-model.md) owns the Postgres-backed worked-model catalogue, stable bundle URLs, coherent conversation/workpiece/evidence/net bundles and independently owned resume/fresh-copy path. The existing demo-mode fixture selector, crew-reservation tracer and Mission 7b's disposable SQLite conversation do not discharge it. Petrinaut simulation scenarios/metrics and a second worked-model bundle are excluded by default pending PM confirmation.

Retained terrain evidence: the installed native format has no public selective import/clone API. Limited private-format transformations worked only for a settled, attachment-free, single-root, workpiece-only source; two browser profiles continued independently, while the fixed tracer document key blocked co-resident copies. This supports the live mission's requirement to inspect native identity rather than authorizing private-format copying.

Future work beyond the selected 7c Inventory bundle retains arbitrary import/clone, broader attachment/effect-history rebinding and full release acceptance. At deployment, startup catalogue updates and working-session creation remain separate operations; a new ECS task being unready does not imply the old task is stopped. Any online update needs transactional catalogue/runtime/cache/fencing evidence at the actual boundary and explicit remote write/release authorization.
...
| Provider-visible nested schema | Mission 6 tests only the least mutation needed by its fixture; Mission 7 repairs the carrier and proves one real nested mutation on its proving scenario before admitting classes; Mission 9 closes the classes its extended region adds | Mission 5 and non-construction Voice work |
| Fixture materialization | Mission 7 proves one tiny genuine conversation can be exported or retained, relocated, reopened, authorized, and queried before any paid persona breadth; the probe's branch selects the demo fixture route | Voice work and prepared-fixture viability |
| Compaction survival | Mission 7 forces compaction and reads `history()`; the branch decides whether the existing archive lane is hardened | Everything before Mission 7 |
...
| Behavioural discriminator | Mission 7 derives one executable check from the workpiece for its constructed region and carries it unchanged to Missions 9 and 10; broader simulation coverage waits for a discriminating need | The adversarial tracer |

The provisional shared-interface names `EvidenceBackedWorkpieceItem`, `DerivationRecord`, and `NetPatch` were superseded on 2026-09-04: the revision identity, declared basis, mutation record, and identity epoch named in the provenance relations lock are the shared seam, and Mission 7 earns their exact shapes. Do not freeze richer names or field catalogs before two missions genuinely share them.
...
- [Draft Mission 7c — Inventory worked-model capability](docs/mission-drafts/7c-inventory-worked-model.md)
...
Do not create Mission 4 or Mission 8 drafts. Convert Mission 8 from the [successor cut](#mission-8-successor-cut-when-ready) when Tim's SRE-1013 resources exist. Mission 5 on FE-1574 was Mission 6's transport prerequisite; Mission 6 and Mission 7a are archived, Mission 7b is live in root authority, and Mission 7c remains planning-only until it is cut on its own branch. Mission 11 stays deliberately shallow until Chris and Yannis accept input artifacts, one optimisation question, scenario/parameter representation, execution boundary, expected result, and minimum credibility checks.
...
The standing ownership test is: guidance that holds for any source-side account regardless of target formalism belongs in core; person-practice guidance is an explicit core default; subject-typology and target-formalism consequences remain in plugins. Source acquisition and target transformation remain distinct responsibilities even when they consult the same store.
...
| M7 fog-line (source eligibility, exact range, derivation representation, inventory, names, durable owner, deployed host, compaction, latency, portfolio) | Draft 7, Fog-line and decision tables | Source eligibility replaced by source labelling; derivation representation replaced by basis and mutation record; others retained. |
...
| M9 constraints, obligations, touched paths, fog, stops | Draft 9, corresponding sections; Draft 7 where they concern the first region | Retained; "no automatic projection" and "derivation" wording replaced by basis and mutation record. |
...
| Draft 10 sweep, capture ledger, derivation seam, durable capture oracle gap | Draft 10, edited in place | Re-expressed over settled revisions, reviewer message ids, basis, mutation records; Mission 2 archive marked historical. |
