...
export const parseUrlForOntologyChip = (url: VersionedUrl) => {
  ...
  if (validationResult.type === "Err") {
    throw new Error(`Could not validate url as VersionedUrl: ${validationResult.inner.reason}`);
  }
  const parsed = validationResult.inner;
  const parsedUrl = new URL(parsed);
  const domain = parsedUrl.host === "localhost:3000" ? "localhost" : parsedUrl.host;
