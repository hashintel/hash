...
- Use stochastic transition lambdas for rate-based firing when stochasticity is available.
- Use predicate transition lambdas for boolean firing conditions only when colours are available and the transition has at least one standard input arc from a coloured place.
- Leave transition lambda code empty when neither stochasticity nor coloured standard inputs are available; the runtime treats the transition as always enabled once its arc weights are satisfied.
- Use transition kernels to transform or generate coloured output tokens, including stochastic distributions. Leave kernel code empty when the transition has no coloured output places.
...
- Transition lambda (\`transition.lambdaCode\`): \`export default Lambda((input, parameters) => …)\`. Available when stochasticity is enabled OR when colours are enabled and the transition has at least one standard input arc from a coloured place. \`input.PlaceName\` is a tuple sized to the input arc weight for coloured standard and read input arcs; tokens are \`{ <elementName>: number }\`. Read arcs expose tokens in \`input\` but do not consume them when the transition fires. Inhibitor arcs and uncoloured input places are NOT in \`input\`. Predicate → boolean; stochastic → non-negative rate in firings per simulation second (0 disables, Infinity always fires). Must be deterministic. If unavailable or empty, the runtime uses true for predicate-style transitions and Infinity for stochastic-style transitions.
- Transition kernel (\`transition.transitionKernelCode\`): \`export default TransitionKernel((input, parameters) => …)\`. Available only for transitions with coloured output places. Return \`{ OutputPlaceName: [token, …] }\` sized to the output arc weight. Include only coloured output places; uncoloured output places are auto-populated. Use \`Distribution.Gaussian(mean, sd)\` / \`Distribution.Uniform(min, max)\` / \`Distribution.Lognormal(mu, sigma)\` for stochastic attributes; chained \`.map(fn)\` on the same distribution shares one draw. Leave empty when no coloured outputs exist.
