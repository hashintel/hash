...
export const petrinautDocSummaries: Record<PetrinautDocName, string> = {
  ...
  "reactive-modules":
    "Zeroth Reactive Modules window: opened from the command palette, floating or docked beside the canvas and resizable in both, Petri Net IR and Python Reactive Module tabs, recompiles on net change from the Simulation Settings' initial state, parameters and dt, compiler flags (shape monolithic or modular, layout single file or file per module, marking Real or Int, control closed or open, slots of a coloured place) in the Python tab's header and the IR's zeroth section, the collapsible file list beside the Python editor, coloured tokens, dynamics, guards, rates and kernels carried as code in the IR, the linear subset the Python module holds and what is refused.",
  examples:
