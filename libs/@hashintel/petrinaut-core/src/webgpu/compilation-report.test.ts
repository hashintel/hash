...
describe("analyzeCompilation", () => {
  ...
  it("accepts every bundled example except the identity-keyed nets", () => {
    ...
    // the two status-view examples. Their tokens carry a \`uuid\` or
    // \`string\` identity key so Kanban cards read as machines and tickets,
    // and WebGPU integers are 32-bit, so those attributes cannot be
    // represented. (Production Machines' \`Start Repair\` also
  ...
  it("classifies every bundled example's model metrics", () => {
    ...
    // Machines, Ticket Processing) are `not-attempted` because eligibility
    // refuses those nets before emission.
