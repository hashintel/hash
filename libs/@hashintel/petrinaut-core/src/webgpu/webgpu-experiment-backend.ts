...
function assess(
  ...
  // a baked literal. Shapes the buffer cannot express — per-run seeds set on
  // a run record, per-run markings, mismatched override sets, non-numeric or
  // f32-overflowing values — are refused at instantiation by
  // `deriveRunParameters`.
