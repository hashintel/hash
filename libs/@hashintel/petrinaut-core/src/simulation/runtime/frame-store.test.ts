...
describe("createInMemorySimulationFrameStore string pool accumulation", () => {
  it("applies newStrings deltas in order and decodes string fields", () => {
    ...
    expect(store.get(0)!.getPlaceTokens(place, color)).toEqual([
      { label: "alpha", x: 1.5 },
    ]);
    expect(store.get(1)!.getPlaceTokens(place, color)).toEqual([
      { label: "alpha", x: 2.5 },
    ]);
    expect(store.latest()!.getPlaceTokens(place, color)).toEqual([
      { label: "beta", x: 3.5 },
    ]);
  });
