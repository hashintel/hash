 ...
 */
export function getPlaceIndex(layout: EngineFrameLayout, placeId: string): number {
 ...
 */
export function getTransitionIndex(layout: EngineFrameLayout, transitionId: string): number {
  const transitionIndex = layout.transitionIndexById.get(transitionId);
  if (transitionIndex === undefined) {
    throw new Error(`Transition ${transitionId} not found in Monte Carlo frame layout`);
  }
