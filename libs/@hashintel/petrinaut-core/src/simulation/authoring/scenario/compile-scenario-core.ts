/**
 * Scenario compilation orchestration, shared between the editor's sandboxed
 * evaluator (`compile-scenario.ts`) and the server-side HIR program
 * (`compile-scenario-program.ts`, Node-only). The two paths differ ONLY in
 * how the three user-code surfaces are evaluated (`ScenarioEvaluators`);
 * everything else — evaluation order, validation, coercion, rounding, error
 * shapes and the documented quirks (unknown place names silently skipped in
 * code mode, non-object tokens skipped) — lives here so the paths cannot
 * drift.
 */
...
export type NetParameterValues = Record<string, number | boolean>;

// -- Evaluator abstraction ----------------------------------------------------

/**
 * How the three user-code surfaces of a scenario are evaluated. The editor
 * evaluates the raw expression text in a best-effort sandbox
 * (`compile-scenario.ts`); the server dispatches to pre-compiled HIR
 * evaluators (`compile-scenario-program.ts`). Each call may throw — the
 * orchestration converts throws into per-item compilation errors.
 */
export type ScenarioEvaluators = {
  /** Evaluates one parameter-override expression (already trimmed). */
  parameterOverride(
    paramId: string,
    expression: string,
  ...
  /** Evaluates one per-place initial-count expression (already trimmed). */
  ...
  /** Evaluates the code-mode initial-state body (already trimmed). */
  initialStateCode(
    code: string,
    parameters: NetParameterValues,
    scenario: NetParameterValues,
...
/**
 * Compiles a scenario into concrete parameter values and initial token
 * counts, using the given evaluators for its user-code surfaces.
 *
 ...
 */
export function compileScenarioWithEvaluators(
  scenario: Scenario,
  netParameters: Parameter[],
  places: Place[],
  types: Color[],
  options: CompileScenarioOptions,
  evaluators: ScenarioEvaluators,
  ...
  for (const [paramId, expression] of Object.entries(
    ...
    try {
      const value = evaluators.parameterOverride(
        paramId,
        trimmed,
        parametersObj,
        scenarioObj,
  ...
  if (scenario.initialState.type === "code") {
    ...
    if (code !== "") {
      try {
        const result = evaluators.initialStateCode(
          code,
          parametersObj,
          scenarioObj,
        );
    ...
    for (const [placeId, value] of Object.entries(
      ...
      try {
        const result = evaluators.initialStateExpression(
          placeId,
          trimmed,
          parametersObj,
          scenarioObj,
