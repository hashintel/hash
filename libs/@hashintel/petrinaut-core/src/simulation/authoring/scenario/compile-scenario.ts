...
export function prepareScenarioCompiler(
  ...
  // An ad-hoc definition lowers to the same code-mode HIR
  // (`lowerScenarioToHir` synthesizes it against the net), and its derived
  // scenarioParameters and parameterOverrides are persisted on the scenario
  // itself, so steps 1 and 2 apply them like any other.
  const hasCode =
