...
- Adds an in-browser optimization capability that runs the Optuna study in a Pyodide worker and evaluates trials through a host channel. A study that completed or was cancelled stays in the worker until it is released, so the connected capability can extend it with more trials on the same sampler history, and a run may keep up to four trials in flight at once. Pyodide and Optuna download from jsDelivr and PyPI by default; `pyodide.indexURL` overrides where the runtime loads from. The `./optimization` entry gains `parseOptimizationManifest`, `isUnknownOptimizationRunError` and `PETRINAUT_OPTIMIZATION_UNKNOWN_RUN_ERROR_CODE`, and the main entry gains `createReadableStore`. ([@kube](https://github.com/kube), [#9515](https://github.com/hashintel/hash/pull/9515))

- Expose stable document revision identities and explicit browser and headless diagnostics for captured definitions, expand the selected mutation batch with supported update and removal operations, and start document observation only when subscribed. ([@lunelson](https://github.com/lunelson), [#9667](https://github.com/hashintel/hash/pull/9667))

- `classicNodeDimensions` gives places a 104 × 104 footprint and transitions a 180 × 60 footprint for canvas rendering and layout. ([@kube](https://github.com/kube), [#9595](https://github.com/hashintel/hash/pull/9595))

- Make the code editor guide available to the in-app assistant. ([@kube](https://github.com/kube), [#9664](https://github.com/hashintel/hash/pull/9664))
...
- Make bundled example scenarios editable in the ad-hoc scenario form while preserving their parameters and initial states. ([@kube](https://github.com/kube), [#9724](https://github.com/hashintel/hash/pull/9724))

- Register the createExperiment tool with its validated request schema for AI integrations. ([@kube](https://github.com/kube), [#9678](https://github.com/hashintel/hash/pull/9678))

- Accept state constraint expressions with optional trailing semicolons and no explicit return while preserving existing code bodies. Report a missing return when a state constraint contains separate statements. Update the assistant's guide summaries for experiment controls and settings available without simulation feature flags. ([@kube](https://github.com/kube), [#9725](https://github.com/hashintel/hash/pull/9725))
...
- File import defaults a place's `colorId`, `dynamicsEnabled` and `differentialEquationId`, and an arc's `weight`, instead of rejecting a file that omits them. ([@kube](https://github.com/kube), [#9715](https://github.com/hashintel/hash/pull/9715))
...
- Metric specs can sample every run that has not errored with `sampleRuns: "notErrored"`. ([@kube](https://github.com/kube), [#9685](https://github.com/hashintel/hash/pull/9685))

- Add constraint margin and state-indicator evaluators, an optional manifest constraintPolicy, and aggregateTime support in getRunValues. ([@kube](https://github.com/kube), [#9604](https://github.com/hashintel/hash/pull/9604))

- Export `PetrinautOptimizationDirection`, the objective's `maximize` | `minimize` union, from the package root and the `/optimization` entry. ([@kube](https://github.com/kube), [#9653](https://github.com/hashintel/hash/pull/9653))
...
- Preserve place capacities when normalizing documents or reopening a JSON document handle, including zero limits and explicitly unbounded places. ([@lunelson](https://github.com/lunelson), [#9562](https://github.com/hashintel/hash/pull/9562))
...
- Mark Space as the default starting place in the Probabilistic Satellite Launcher example. ([@kube](https://github.com/kube), [#9764](https://github.com/hashintel/hash/pull/9764))
...
- `selectedMutationBatchSchema` admits `addScenario`, `updateScenario`, `removeScenario`, `addMetric`, `updateMetric` and `removeMetric`, so a selected batch can save the scenario and metric an experiment names. ([@kostandinang](https://github.com/kostandinang), [#9762](https://github.com/hashintel/hash/pull/9762))

- Expose the selected add- and remove-place, transition, and arc ordered batch contract and executor. ([@lunelson](https://github.com/lunelson), [#9649](https://github.com/hashintel/hash/pull/9649))

- Add simulation panel guidance to the AI assistant's documentation catalog. ([@kube](https://github.com/kube), [#9723](https://github.com/hashintel/hash/pull/9723))

- Describe snapshot validation and its TypeScript and HIR diagnostics in the AI tool. ([@kube](https://github.com/kube), [#9675](https://github.com/hashintel/hash/pull/9675))
...
- Add validated request and result schemas and a `runExperiment` host interface for simulations and optimizations with up to 100,000 final runs. ([@kube](https://github.com/kube), [#9676](https://github.com/hashintel/hash/pull/9676))

- Adds the **Vaccination Campaign** example: an SIR model with pre-wave vaccination coverage and contact reduction, whose Total cost objective has its minimum inside the parameter domain, built for optimization demos. ([@kube](https://github.com/kube), [#9516](https://github.com/hashintel/hash/pull/9516))

- Reject arc creation when the referenced transition does not exist instead of treating it as an unchanged mutation. ([@lunelson](https://github.com/lunelson), [#9722](https://github.com/hashintel/hash/pull/9722))
