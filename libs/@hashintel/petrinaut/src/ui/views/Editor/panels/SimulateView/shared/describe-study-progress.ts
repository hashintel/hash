 ...
 * the experiment drawer's headline and in the sweep navigator's status line
 * once a study driving the sweep has settled. Both read the same words, so
 * the two homes of a study never describe it differently.
 */
import {
  currentTrialNumber,
  finishedTrialCount,
 ...
 * Stop ended it; "Finished 30 steps · …" and "Failed after … " likewise.
 */
export const describeStudyProgress = (
  ...
  switch (optimization.status) {
    ...
    case "cancelled":
      return `Stopped after ${finished} of ${requested} steps · ${best}`;
    case "error":
      return `Failed after ${finished} of ${requested} steps · ${best}`;
  }
};

/** "4 / 30 · 3 runs each", the runs part left out at one run per step. */
