...
describe("Show Zeroth Reactive Modules", () => {
  it("is listed only while the editor can open the window, and runs the latest opener", () => {
    const registry = createCommandRegistry();
    const show = vi.fn();
    const updatedShow = vi.fn();
    const command = () =>
      registry
        .list()
        .find((entry) => entry.id === "petrinaut.reactive-modules.show");
    const { rerender, unmount } = render(
      <CommandRegistryProvider registry={registry}>
        <EditorCommands />
      </CommandRegistryProvider>,
    ...
    rerender(
      <CommandRegistryProvider registry={registry}>
        <EditorCommands onShowReactiveModules={show} />
      </CommandRegistryProvider>,
    );
    ...
    rerender(
      <CommandRegistryProvider registry={registry}>
        <EditorCommands onShowReactiveModules={updatedShow} />
      </CommandRegistryProvider>,
    );
    registry.execute("petrinaut.reactive-modules.show");
    expect(updatedShow).toHaveBeenCalledOnce();
    unmount();
    expect(command()).toBeUndefined();
  });
});
