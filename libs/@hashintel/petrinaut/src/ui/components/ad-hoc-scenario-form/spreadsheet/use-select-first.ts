/**
 * The form's activation grammar for a click target that is also a selectable
 * cell: the first pointer click only selects (focuses) it, a click on an
 * already-selected target activates, and a keyboard "click" (Enter/Space,
 * which carries no pointer detail) always activates. The second click of a
 * double-click lands on an already-selected target, so double-click
 * activates without a handler of its own — important where activation is
 * not idempotent (materializing a phantom row).
 */
