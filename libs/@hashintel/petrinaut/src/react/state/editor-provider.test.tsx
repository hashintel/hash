...
describe("EditorProvider simulate presentation", () => {
  it("navigates the presentation as its own app-history step", () => {
    ...
    expect(editor!.simulatePresentation).toBe("drawer");

    act(() => {
      editor!.setSimulatePresentation("full");
    });

    expect(editor!.simulatePresentation).toBe("full");
    expect(recorded.at(-1)).toEqual({
      history: "push",
      intent: { cause: "user", action: "simulation-presentation" },
    });
    // The record itself is untouched: only the presentation moved.
    expect(editor!.simulateDrawer).toEqual({ type: "closed" });
  });
});
