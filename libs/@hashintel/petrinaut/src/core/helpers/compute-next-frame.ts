...
export function computeNextFrame(
  ...
  if (simulation.differentialEquationFns.size > 0) {
    ...
    for (const [placeId, placeState] of currentFrame.places) {
      ...
      // but computePlaceNextState expects a PlaceDifferentialEquation which takes (currentState, dimensions, numberOfTokens)
      // We need to wrap the user's differential equation to match the expected signature
      const wrappedDiffEq = (
