/**
 * Generates virtual TypeScript files for SDCPN entities.
 *
 * This is shared between the one-shot checker and the LSP worker.
 ...
 *
 * Creates type definition files and code files for:
 * - Parameters
 * - Colors (types)
 * - Differential equations
 * - Transitions (lambda and kernel)
 */
...
/**
 * Gets the list of code file paths (files that contain user-editable code).
 * These are the files we want to check for diagnostics.
 */
export function getCodeFilePaths(sdcpn: SDCPN): string[] {
  const paths: string[] = [];

  for (const de of sdcpn.differentialEquations) {
    paths.push(getItemFilePath("differential-equation-code", { id: de.id }));
  }

  for (const transition of sdcpn.transitions) {
    paths.push(
      getItemFilePath("transition-lambda-code", {
        transitionId: transition.id,
      }),
    );
    paths.push(
      getItemFilePath("transition-kernel-code", {
        transitionId: transition.id,
      }),
    );
  }
