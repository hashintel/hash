...
- Implements the ack policy per play mode (profiles defined by the core's
  `getPlayModeBackpressure`):
  - `viewOnly` — never acks: the worker computes nothing new.
...
| Place state visualization (user visualizers) | `getPlaceTokens(place, color)` → typed `TokenRecord[]`                                                        |
