...
| **Steps**       | Sweeps with a study, from the first **Optimize** on: the steps finished over the steps requested, with the runs per step, **4 / 30 · 8 runs each**; the count alone in a narrow drawer. |
...
From the first **Optimize** on a sweep, two more cards follow the metric charts in the same grid, at the same height, and stay there through every later study; a second **Optimize** swaps their content in place.
...
A parameter sweep is also where Petrinaut searches parameters: its **Optimize**
button hands the sliders to the in-browser optimizer, which explores the swept
intervals through the sweep's own compute (see [Optimizing a
