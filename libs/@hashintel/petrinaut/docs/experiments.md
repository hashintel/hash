...
State editors start on one line and grow with the code up to eight lines, then scroll. Use **Enter** for a new line when a condition needs intermediate calculations before its `return`. Empty fields show examples using the current scenario and model.
