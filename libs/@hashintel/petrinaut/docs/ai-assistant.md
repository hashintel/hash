...
4. **Command palette**. Choose **Toggle AI assistant**, or press **Cmd/Ctrl+Shift+K** directly. This opens the assistant and focuses the message field, or closes it when already visible, including compact Voice mode. Reopening preserves the conversation and docked or floating layout. From Simulate or Notebook mode, the command returns to Edit mode and opens the assistant. This command is available when the host provides an AI assistant.
