...
A host can offer more than one assistant. The first one answers until you choose another in **User Settings → General → AI assistant**, or run **Use the _name_ assistant** from the command palette. Each assistant keeps its own conversation, and switching never moves messages from one to the other.
