...
const fontSizeOrder = ["xs", "sm", "base", "lg", "xl", "2xl", "3xl", "4xl"] as const;
...
const LineHeightDemo = ({ category, size }: { category: string; size: string }) => {
  ...
  return (
    <HStack gap="4" alignItems="flex-start">
      ...
      >
        Multi-line text demo showing the line height value. This text wraps to show spacing.
      </div>
    </HStack>
  );
};
  ...
  <VStack gap="8" alignItems="flex-start" p="6">
    ...
    <HStack gap="12" alignItems="flex-start" width="100%">
      <VStack gap="4" alignItems="flex-start" flex="1">
        ...
        <p className={css({ fontSize: "xs", color: "neutral.500", mb: "2" })}>
          Line height equals font size — for single-line text
        </p>
        {["text-xs", "text-sm", "text-base", "text-lg", "text-3xl"].map((size) => (
