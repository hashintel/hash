 ...
 */
const csvParser = (t: string) => CSV.parse(t, { header: true, dynamicTyping: true })?.data;

/**
 * Load and parse a fixture file with type assertion, or with schema validation
 */
export function useFixture<TSchema extends z.ZodType<any>>(path: string, schema: TSchema): z.infer<TSchema>;
  ...
  const { data, error } = useSuspenseQuery({
    ...
    queryFn: () =>
      fetch(path, {
        ...
        const parsedData = isText ? textParser(await res.text()) : await res.json();
