...
describe("CLI transports", () => {
  ...
  it("emits JSON stderr diagnostics after the unchanged ready line", async () => {
    const input = new PassThrough();
    const output = new PassThrough();
    const errorOutput = new PassThrough();
    let stderr = "";
    ...
    input.end(
      [
        JSON.stringify({ id: 1, method: "metadata" }),
        JSON.stringify({ id: 2, method: "nope", params: { secret: "code" } }),
        "",
      ].join("\n"),
    );
    await serving;
    ...
    // The bootstrap handshake stays the exact first stderr line.
    expect(stderrLines[0]).toBe(`Petrinaut stdio ready for model ${modelPath}`);
    ...
    expect(stderrLines.slice(1).join("\n")).not.toContain("secret");
    for (const entry of diagnostics) {
      expect(Number.isNaN(Date.parse(entry.time as string))).toBe(false);
    }
  });

  it("reports a null correlation id when none is supplied", async () => {
    const input = new PassThrough();
    const output = new PassThrough();
    const errorOutput = new PassThrough();
    let stderr = "";
  ...
  it("logs an error diagnostic for an oversized request line", async () => {
    const input = new PassThrough();
    const output = new PassThrough();
    const errorOutput = new PassThrough();
    let stdout = "";
    ...
    const serving = serveStdio({ modelPath, input, output, errorOutput });
    input.end(`${"x".repeat(MAX_REQUEST_LINE_BYTES + 1)}\n`);
    await serving;

    expect(parseResponses(stdout)).toEqual([
      { id: null, error: { message: "Request line is too large" } },
    ]);
    const diagnostics = stderr
      .trim()
      .split("\n")
