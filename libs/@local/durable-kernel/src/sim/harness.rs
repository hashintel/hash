...
use super::{AppendOutcomeWeights, SimAppendOutcome, SimKey, SimLogHandle, SplitMix64};
use crate::{
    domain::{
...
impl Driver<'_> {
    ...
    fn observe_recovery(&self, coverage: &mut ScheduleCoverage) {
        properties::covered(
            coverage,
        ...
        match result {
            ...
            Err(error) => match error.kind {
                ...
                | ShardCommandErrorKind::Closed => {
                    ...
                    self.trace
                        .push(format!("command loop failed ({:?}); reopening", error.kind));
    ...
    async fn submit_invalid(&mut self, coverage: &mut ScheduleCoverage) {
        self.next_request += 1;
    ...
    async fn snapshot_commit(&mut self, step: usize, coverage: &mut ScheduleCoverage) {
        ...
        match self.handle().commit_snapshot(snapshot).await {
            ...
            {
                self.trace
                    .push(format!("snapshot save failed ({:?})", error.kind));
                self.crash_and_recover(coverage).await;
            }
            Err(_not_committed) => {
    ...
    async fn crash_and_recover(&mut self, coverage: &mut ScheduleCoverage) {
        self.started.task.abort();
        let durable_ids = self.reference_fold().event_ids;
    ...
    async fn finish_effects_and_check(&mut self, coverage: &mut ScheduleCoverage) {
        let mut turns = 0_u32;
...
mod tests {
    ...
    use super::{
        AppendOutcomeWeights, PlannedAction, ScheduleCoverage, SchedulePlan, ScheduleReport,
        SimAppendOutcome, derive_plan, run_plan,
    };
