...
accepting the third attempt. A poison payload fails all four attempts and is
moved to the dead-letter queue. The kernel stores a snapshot every eight events.
...
derived from those contents. If two identical actions need separate records,
include a request ID or another distinguishing field.
