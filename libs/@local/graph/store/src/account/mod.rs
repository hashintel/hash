...
use hash_graph_authorization::schema::{AccountGroupId, WebOwnerSubject};
...
pub trait AccountStore {
    ...
    /// Returns either an [`ActorId`] or an [`AccountGroupId`] for the specified [`OwnedById`].
    ///
    /// # Errors
    ///
    /// - if the [`OwnedById`] does not exist
    /// - if the [`OwnedById`] exists but is both, an [`ActorId`] and an [`AccountGroupId`]
