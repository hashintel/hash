    ...
    knowledge::{
        ...
        property::{
            ...
            visitor::{
                ...
                OneOfPropertyValidationReports, TraversalError, walk_array, walk_object,
                walk_one_of_property_value, walk_value,
            },
        },
    },
...
impl EntityVisitor for EntityPreprocessor {
        ...
        P: DataTypeLookup + Sync,
    {
        ...
        let has_error = if let Err(error) =
            walk_one_of_property_value(self, schema, property, type_provider).await
        {
            property_validation.passed = error.passed;
    ...
    async fn visit_object<T, P>(
        ...
        type_provider: &P,
    ) -> Result<(), HashMap<BaseUrl, ObjectPropertyValidationReport>>
    where
        T: PropertyObjectSchema<Value = ValueOrArray<PropertyTypeReference>> + Sync,
        P: DataTypeLookup + OntologyTypeProvider<PropertyType> + Sync,
    {
        let mut object_validation = walk_object(self, schema, object, type_provider).await;
        ...
        if object_validation.is_empty() {
            Ok(())
        } else {
            Err(object_validation)
        }
    }
}
