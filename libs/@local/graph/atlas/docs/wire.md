...
- Payload order equals slot order, and prefix, directory, `HEAD`, and columns are pure functions of the bound serving state `(generation, request, visibility, server secret, serving limits)` - identical requests under identical state yield byte-identical bytes there, the property the client's application-layer cache keys on. Secret and limits are restart-stable by operator contract - changing either for an active generation requires rotating the generation and clearing application caches - which is why the cache key carries neither. The tile trailer's labels and icons are also generation-local and byte-identical under that state. An edges trailer's link labels are not: fitted and delta links alike read from the server's held captured display - normally the link's currently served edition, an earlier one only while the warn-logged capture read keeps failing - and a fitted link the server holds no capture for reads the generation's own payload (section 7). A label edit after publication therefore changes the served edges label at the server's next capture. Every link's type reference reads its current edition's representative type the same way, and both may lag the newest edition by up to 65 seconds (section 7). Locate never consults captured displays for its link labels: they are generation-local payloads admitted only when the request-time store read resolves the corresponding entity. A fitted link revised after the fit therefore serves its post-fit label on edges and its fit-time payload on locate - a divergence that stands until the next refit (section 8). Locate type and property values hydrate from the live store. The store is bitemporal and an edition is immutable, so a lagging value reads as the recent past, never as a rewrite. Edges and locate trailer bytes may therefore differ between identical requests as store-derived values change or entities stop or start resolving. A client must not retain a detailed response as an immutable generation tile. Cache geometry sections and refetch edges and locate detail where request-time state matters.

The directory declares every column's extent before the first payload byte, so prefix + directory stream first and columns follow immediately. The trailer lives outside the directory as a self-delimiting CBOR tail - declared by a `HEAD` key on tile and edges, mandated by kind on locate - whose start is `align8` of the last present column's end and whose extent is its own CBOR structure. Edges and locate trailers arrive only after their request-time store reads. Edges labels themselves never ride that read - they come from the server's captured displays first and otherwise from the generation's payloads, both resolved in process. The tile trailer needs no store read. The layout is the streaming contract; current servers assemble the whole body before the first byte, so an edges or locate response's first-byte latency includes hydration. A streaming decoder is correct against both. Geometry sections decode from a partial body, and on successful responses a future streaming server changes delivery timing, never bytes. If edges hydration fails after a streaming server has sent the columns, the trailer arrives valid and empty-shaped: the in-process labels remain, while the type table is empty and type references are null. If locate hydration fails at that point, its empty-shaped trailer has null labels and values alongside empty tables, with every completeness bit unset. Neither route sends a truncated body. Problem documents cover only failures before the first body byte.
...
Labels and icons come from the generation's published identity artifacts, so the trailer is immutable for that generation and needs no live-store hydration. The trailer-last layout still permits geometry-first streaming, although current servers buffer the whole body. Section 2 defines this identity and timing distinction. Null marks a row whose published label did not resolve.
...
Locate trailer, always present - locate is the detail view. Its labels come from generation payloads. They read null when the request-time store does not resolve the corresponding entity or when the published payload is empty. The intern tables first, then node arrays in delivered order and link arrays in edge order, every type and property reference a uint index into its table:
