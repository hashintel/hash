...
- Payload order equals slot order, and prefix, directory, `HEAD`, and columns are pure functions of the bound serving state `(generation, request, visibility, server secret, serving limits)` - identical requests under identical state yield byte-identical bytes there, the property the client's application-layer cache keys on. Secret and limits are restart-stable by operator contract - changing either for an active generation requires rotating the generation and clearing application caches - which is why the cache key carries neither. The tile trailer's labels and icons, and the edges trailer's fitted link labels, are also generation-local and byte-identical under that state. A delta link's label is a request-time store read (section 7), and locate labels are generation-local payloads admitted only when the request-time store read resolves the corresponding entity. Every link's type reference reads its current edition's representative type, which may lag the newest edition by up to 65 seconds (section 7). Locate type and property values hydrate from the live store. The store is bitemporal and an edition is immutable, so a lagging value reads as the recent past, never as a rewrite. Edges and locate trailer bytes may therefore differ between identical requests as store-derived values change or entities stop or start resolving. A label edit after publication still changes no fitted label. Even so, a client must not retain a detailed response as an immutable generation tile. Cache geometry sections and refetch edges and locate detail where request-time state matters.
