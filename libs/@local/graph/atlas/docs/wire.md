...
Trailer property values are the entity's **deliverable set**: every property the deployment's property protection does not withhold. Protection is a per-actor rule in the store; the trailer applies it to every caller alike, so a trailer's property map is one function of the entity and carries no protected property for anyone - and a property's absence is invisible in the completeness flags, which are stated over the deliverable set. Labels are outside that rule: a label is a property value materialized per edition through the type's `labelProperty` path, delivered as the graph's own read path delivers it, so a type whose label property is protected carries that value in `LABELS` and in the trailer's link labels.
...
| 9   | `linkPropertiesComplete` | `bstr`               | LSB-first bitmask, bit e set = edge e's property map is the link entity's whole **deliverable** set                                                                                                                 |
