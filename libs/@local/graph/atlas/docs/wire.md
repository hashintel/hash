...
- Payload order equals slot order, and prefix, directory, `HEAD`, and columns are pure functions of the bound serving state `(generation, request, visibility, server secret, serving limits)` - identical requests under identical state yield byte-identical bytes there, the property the client's application-layer cache keys on. Secret and limits are restart-stable by operator contract - changing either for an active generation requires rotating the generation and clearing application caches - which is why the cache key carries neither. The tile trailer's labels and icons, and the edges trailer's link labels, are also generation-local and byte-identical under that state. Locate labels are generation-local payloads admitted only when the request-time store read resolves the corresponding entity. Edges first-type references and locate type and property values hydrate from the live store. Trailer bytes may therefore differ between identical requests as store-derived values change or entities stop or start resolving, but a label edit after publication does not change the generation's payload. Even so, a client must not retain a detailed response as an immutable generation tile. Cache geometry sections and refetch edges and locate detail where request-time state matters.

The directory declares every column's extent before the first payload byte, so prefix + directory stream first and columns follow immediately. The trailer lives outside the directory as a self-delimiting CBOR tail - declared by a `HEAD` key on tile and edges, mandated by kind on locate - whose start is `align8` of the last present column's end and whose extent is its own CBOR structure. Edges and locate trailers arrive only after their request-time store reads. Edges labels themselves come from the generation. The tile trailer needs no store read. The layout is the streaming contract; current servers assemble the whole body before the first byte, so an edges or locate response's first-byte latency includes hydration. A streaming decoder is correct against both. Geometry sections decode from a partial body, and on successful responses a future streaming server changes delivery timing, never bytes. If edges hydration fails after a streaming server has sent the columns, the trailer arrives valid and empty-shaped: generation-local labels remain, while the type table is empty and type references are null. If locate hydration fails at that point, its empty-shaped trailer has null labels and values alongside empty tables, with every completeness bit unset. Neither route sends a truncated body. Problem documents cover only failures before the first body byte.
...
Labels are outside property masking. Tile and edges labels are generation-local payloads. An edges trailer carries the generation-local label for every delivered edge. The store answer supplies only each edge's first-type reference. Locate node and link labels are generation-local payloads admitted only when the request-time store read resolves the corresponding entity. A type whose `labelProperty` falls under protection can therefore carry that value in the published label payload even when property masking would withhold it. No distinct `LABELS` column exists. This mirrors the graph's actor-independent materialized-label behavior. `BE-313` tracks graph-side label protection, and Atlas changes with that behavior rather than defining an independent refusal.
...
The bulk response delivers type references, never rendered display: the client resolves labels and icons through its own type metadata - one owner per display concern.
