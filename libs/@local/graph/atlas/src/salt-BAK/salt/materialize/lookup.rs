...
use super::{
    MortonKey,
    base::{
        BUCKET_OFFSETS, BUCKETS, COORDINATES, DRAFT_IDS, DRAFT_PRESENT, ENTITY_UUIDS, MORTON_KEYS,
        ROWS, WEB_IDS,
    },
};
...
/// One entity located by its durable graph identity.
#[derive(Debug, Copy, Clone, PartialEq)]
pub(crate) struct LocatedEntity {
    /// Durable graph identity of the point.
    pub entity_id: EntityId,
    /// Generation-stable row identity, matching tile-wire records.
    pub row: u32,
    ...
    /// Importance rung of the point's delivery bucket.
    pub bucket: u16,
    /// Shallowest tile zoom at which the point is delivered under the
    /// serving point budget, so a client zooming its covering tile to this
    /// depth is guaranteed to receive the record.
    pub minimum_zoom: u8,
}
...
pub(crate) struct SpatialIndex {
    ...
    // Delivery-order Morton keys and per-rung offsets, retained so locate
    // can replay the tile scan's budget arithmetic without the artifact.
    morton: Vec<u32>,
    bucket_offsets: Vec<u64>,
    ordinals: HashMap<EntityId, u32>,
}
...
impl SpatialIndex {
    ...
    pub(crate) fn build(artifact: ArtifactView<'_>) -> Result<Self, LookupError> {
        ...
        let bucket_offsets = u64_section(artifact, BUCKET_OFFSETS, "bucket offsets")?;
        let count_u64 = u64::try_from(count).map_err(|_error| LookupError::SectionRows)?;
        if bucket_offsets.len() < 2
            || bucket_offsets.first() != Some(&0)
            || bucket_offsets.last() != Some(&count_u64)
            || !bucket_offsets.is_sorted()
        {
            return Err(LookupError::SectionRows);
        }
        let mut ordinals = HashMap::with_capacity(count);
        for (ordinal, entity_id) in entities.iter().enumerate() {
            ordinals
                .entry(*entity_id)
                .or_insert(u32::try_from(ordinal).map_err(|_error| LookupError::SectionRows)?);
        }
        ...
        Ok(Self {
            ...
            entities,
            morton: morton.to_vec(),
            bucket_offsets: bucket_offsets.to_vec(),
            ordinals,
        })
    }
    ...
    /// Resolves one durable graph identity to its position and delivery zoom.
    ///
    /// Returns [`None`] when the generation does not contain the entity.
    /// `budget` is the serving tile point budget; the reported
    /// [`LocatedEntity::minimum_zoom`] replays the tile scan's bucket
    /// backfill and fair-truncation sampling, so it names the shallowest
    /// zoom at which the point's covering tile actually delivers it.
    #[must_use]
    pub(crate) fn locate(
        &self,
        entity_id: &EntityId,
        budget: NonZeroUsize,
    ) -> Option<LocatedEntity> {
        let ordinal = self.ordinals.get(entity_id).copied()? as usize;
        Some(LocatedEntity {
            entity_id: self.entities[ordinal],
            row: self.rows[ordinal],
            grid: self.positions[ordinal],
            canonical: self.canonical[ordinal],
            bucket: self.buckets[ordinal],
            minimum_zoom: self.minimum_delivery_zoom(ordinal, budget.get()),
        })
    }

    /// Returns the shallowest zoom whose covering tile delivers `ordinal`.
    ///
    /// Delivery is not monotonic in zoom (fair truncation resamples every
    /// level), so the scan checks each depth from the root down. At depth 16
    /// a tile range is one grid cell; only coincident points beyond the
    /// budget could still be undelivered there, and those report 16 as the
    /// closest honest answer.
    fn minimum_delivery_zoom(&self, ordinal: usize, budget: usize) -> u8 {
        (0..16_u8)
            .find(|&zoom| self.delivered_at_zoom(ordinal, zoom, budget))
            .unwrap_or(16)
    }

    /// Replays the tile selection for the tile covering `ordinal` at `zoom`.
    fn delivered_at_zoom(&self, ordinal: usize, zoom: u8, budget: usize) -> bool {
        // The tile's Morton range at this zoom, exactly as the tile scan
        // derives it from the route coordinates.
        let suffix_bits = 2 * u32::from(16 - zoom);
        let key = u64::from(self.morton[ordinal]);
        let minimum = (key >> suffix_bits) << suffix_bits;
        let maximum = minimum + (1_u64 << suffix_bits);
        let mut remaining = budget;
        for bucket in self.bucket_offsets.windows(2) {
            let &[start, end] = bucket else {
                unreachable!("two-wide windows always contain two offsets");
            };
            let (start, end) = (start as usize, end as usize);
            let keys = &self.morton[start..end];
            let local_start = keys.partition_point(|&key| u64::from(key) < minimum);
            let local_end = keys.partition_point(|&key| u64::from(key) < maximum);
            let range_length = local_end - local_start;
            let contains = (start + local_start..start + local_end).contains(&ordinal);
            if contains {
                if remaining == 0 {
                    return false;
                }
                ...
                let offset = ordinal - (start + local_start);
                return stride_selects(offset, remaining, range_length);
            }
            remaining = remaining.saturating_sub(range_length);
        }
        unreachable!("the covering tile range always contains the point's own key")
    }
}

/// Returns whether fair truncation picks `offset` from a bucket range.
///
/// The tile scan midpoint-stride samples `total` picks over `range_length`
/// records; midpoints are strictly increasing because the stratum width
/// exceeds one, so a binary search over picks decides membership.
fn stride_selects(offset: usize, total: usize, range_length: usize) -> bool {
    let midpoint =
        |pick: u64| -> u64 { (2 * pick + 1) * (range_length as u64) / (2 * total as u64) };
    let offset = offset as u64;
    let (mut low, mut high) = (0_u64, total as u64);
    while low < high {
        let pick = low + (high - low) / 2;
        if midpoint(pick) < offset {
            low = pick + 1;
        } else {
            high = pick;
        }
    }
    low < total as u64 && midpoint(low) == offset
}
...
fn u64_section<'artifact>(
    ...
    id: SectionId,
    name: &'static str,
) -> Result<&'artifact [u64], LookupError> {
    artifact
        .section(id)
        .ok_or(LookupError::MissingSection { name })?
        .as_u64()
