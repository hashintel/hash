...
pub(crate) fn type_table_statement(axes: &TemporalAxes) -> BoundStatement<'_, TypeTableColumns> {
    ...
    let corpus_editions = SetOperation::union_all(
        edition_column(Scope::EntityEditionId.name(), CorpusTable::Scope),
        edition_column(Links::EntityEditionId.name(), CorpusTable::Links),
    );
    ...
    let statement = SelectStatement::builder()
        .with({
            ...
            WithExpression::default()
                .with_statement(CorpusTable::Scope, scope(axes_points, link_root))
                .with_statement(CorpusTable::Links, links(axes_points, attachments))
        })
        .distinct({
