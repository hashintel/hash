...
//! development store and drives the production run end to end: prior
//! resolution, fit, admission probe, and the activation decision. The
//! fit and quality seams measure their halves in isolation; this seam
//! measures the composition production takes, wrapping the operator
//! seam ([`crate::run`]) with the measurement contract. Nothing here
//! is API for consumers of the crate.
//!
...
/// # Panics
///
/// Panics when any step fails; a measurement target reports its
/// failures by failing, and the [`crate::run::Error`] chain is the
/// diagnosis.
