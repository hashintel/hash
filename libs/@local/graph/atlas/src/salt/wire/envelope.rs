...
//!
//! One response is a 16-byte prefix (magic with kind byte, wire
//! version, zero flags, slot count, zero reserved), a directory of
//! `slotCount` absolute `(start, end)` byte offsets, the payloads
//! sequential in slot order and zero-padded to 8, and an optional
//! self-delimiting CBOR trailer after the last padded payload. All
//! envelope integers are little-endian.
//!
//! The directory is the locating mechanism: `(0, 0)` marks an absent
//! slot, `start == end` at a nonzero offset marks a present-but-empty
//! payload, and every present `start` is 8-aligned by construction
//! (the payload region begins at the 8-aligned `16 + 8 * slotCount`
//! and each payload pads to the next 8 boundary). Emitting the right
//! mark carries request semantics - a zero-point tile's columns are
//! present-empty while an unrequested section is absent - so
//! [`present`](EnvelopeWriter::present) and
//! [`absent`](EnvelopeWriter::absent) are distinct calls, never an
//! `Option` collapsed to emptiness.
//!
//! Offsets are `u32`: responses are bounded below 4 GiB by the
...
/// Prefix size in bytes: magic, wire version, flags, slot count,
/// reserved.
...
///
/// Slots are recorded strictly in slot order, one call per slot; the
/// directory entry is backfilled into the reserved region as each
/// payload lands, so the buffer is complete the moment the last slot
/// is recorded. [`finish`](Self::finish) checks the slot count and
/// returns the bytes; [`finish_with_trailer`](Self::finish_with_trailer)
/// appends the trailer tail first.
#[derive(Debug)]
...
impl EnvelopeWriter {
    ...
    ///
    /// `slots` is at least the kind's v1 table size at every call site;
    /// appended slots beyond the table are legal by the evolution rule.
    #[must_use]
    ...
    ///
    /// The trailer starts at the 8-aligned end of the last present
    /// payload - where the buffer already stands - and is never
    /// padded: its extent is its own CBOR structure.
    ///
    /// # Panics
    ///
    /// Panics when fewer slots were recorded than declared.
