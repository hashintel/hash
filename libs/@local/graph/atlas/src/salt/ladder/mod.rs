//! The condition ladder: one projected layout per relation-lens
//! condition, aligned and measured against its neighbours.
//!
//! A generation may publish several coordinate fields, one per value of
//! the relation-strength condition; the ladder is that schedule with
//! its cross-condition evidence. [`Conditions`] carries the schedule,
//! valid by construction: the first rung is the exact semantic
//! baseline `0.0`, the rungs ascend strictly, and every value is
//! finite. The rung count is deliberately unbounded - each rung costs
//! one projection pass plus four alignment passes, so the schedule
//! length is configuration, not a format limit.
//!
//! [`measure_ladder`] derives each rung's evidence. Every rung aligns
//! onto the baseline and onto its predecessor with the unweighted
//! Procrustes fit ([`Similarity::fit_uniform_par`]); the RMS movement
//! the alignment cannot explain is the rung's real geometric change,
//! invariant under the scale, rotation, and translation freedom the
//! projector never promises to pin down. Two signals summarize the
//! ladder's health:
//!
...
//!
//! [`select_canonical`] names the rung that publishes as the canonical
//! field: the configured condition must be an exact member of the
//! measured schedule and must have passed both criteria. The canonical
//! field itself is the rung's coordinates with its baseline alignment
//! applied - all rungs publish in one shared frame, the baseline's, so
//! clients interpolate between conditions without per-rung frame
//! bookkeeping.
//!
//! Projection itself is the conditioned projector's inference
//! (`salt/projector`), and the per-rung relation loss is its frozen
...
pub(crate) enum LadderError {
    ...
    /// A rung's field has no Procrustes alignment onto the compared
    /// field: its points are coincident or the covariance cancels
    /// exactly.
    Degenerate {
...
pub(crate) enum CanonicalError {
    ...
    /// The rung's relation loss rose beyond tolerance: publishing it
    /// would worsen how well relations are honoured.
    Monotonicity {
        /// The requested condition.
        value: f32,
    },
    /// The rung collapsed onto its predecessor: publishing it buys
    /// nothing over the neighbouring condition.
    Distinguishability {
        /// The requested condition.
        value: f32,
    },
...
///
/// The invariants hold by construction: at least two rungs, the first
...
pub(crate) struct Field<'coordinates> {
    /// The rung's projected coordinates, row-aligned with every other
    /// rung's.
    pub coordinates: &'coordinates [Vec2],
    /// The field's frozen attraction-energy loss, computed by the
    /// projector objective at projection time.
...
pub(crate) struct MeasurementOptions {
    /// Minimum RMS movement against the preceding rung for a rung to
    /// count as distinguishable. Defaults to `1e-8`.
    ...
    /// Slack by which a rung's relation loss may exceed its
    /// predecessor's and still count as monotonic. Defaults to `0.05`.
...
pub(crate) struct RungMeasurement {
    ...
    /// preceding rung's. The baseline is monotonic by definition.
    pub monotonic: bool,
    /// Whether the adjacent movement reached the floor. The baseline is
...
///
/// `fields[i]` is the projection of the whole corpus at
/// `conditions.values()[i]`; rows correspond across fields. Each
/// non-baseline rung fits its alignment onto the baseline and onto its
/// predecessor in parallel, and the returned measurements carry one
/// entry per rung in schedule order.
///
/// # Errors
///
/// Returns an error when a threshold is invalid, the field count does
/// not match the schedule, a field's rows differ from the baseline's, a
/// relation loss is not finite, or a field admits no similarity
/// alignment (coincident points or an exactly cancelling covariance).
...
pub(crate) struct CanonicalSelection<'ladder> {
    /// The rung's position in the schedule: the index of its coordinate
    /// field among the ladder's per-rung artifacts.
    pub index: usize,
    /// The rung's measurement. Its [`RungMeasurement::alignment`] maps
    /// the field into the baseline frame, and applying it row by row is
    /// the canonical field's production.
    pub measurement: &'ladder RungMeasurement,
}

/// Selects the rung publishing as the canonical field.
///
/// The value must be an exact (bit-level) member of the measured
/// schedule - the canonical condition is configuration naming a rung,
/// not an interpolation point - and the rung's evidence must pass both
/// cross-condition criteria. The baseline rung always passes: both
/// criteria hold for it by definition.
///
/// # Errors
///
/// Returns an error when the value names no rung or the named rung
/// failed monotonicity or distinguishability.
...
/// Fits the alignment of `source` onto `target` and measures the RMS
/// movement it cannot explain.
///
/// Returns [`None`] when the fit rejects the pair (coincident points or
/// an exactly cancelling covariance); the residual of a successful fit
/// over validated fields is always finite.
