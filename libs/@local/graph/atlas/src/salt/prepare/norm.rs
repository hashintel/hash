...
//!
//! Every row of the persisted representation matrix promises the
//! source contract: finite components and a unit L2 norm, normalized
//! where the data lives. [`spot_check`] verifies the promise by
//! acceptance sampling instead of recomputing it: checking
//! [`acceptance_sample_size`] uniformly sampled rows and finding all
//! of them valid certifies, with the configured confidence, that
//! fewer than the configured defect rate of all rows violate the
//! contract. One defective sampled row refutes the contract, and the
//! evidence lists every defective sampled row with its diagnosis.
//!
//! The check reads the rows a mapped `f32[N, 512]` artifact yields, so
//! it faults only the sampled pages; sampled rows are visited in
//! ascending order, keeping a cold mapping's faults forward. The
//! sample is small and each row is one kernel pass, so the check runs
//! serially.
...
impl NormSpotCheck {
    ...
    ///
    /// A passing check certifies, with the recorded confidence, that
    /// fewer than the recorded defect rate of all rows are defective.
    #[inline]
    #[must_use]
...
///
/// `embeddings` holds the persisted representations in row order; a
/// mapped `f32[N, 512]` artifact yields the slice directly. A corpus
/// smaller than the sample size is checked exhaustively, making the
/// certification exact.
///
/// # Errors
///
/// Returns an error when the matrix is empty or the sampling budget
