...
//! relation energy, the stage projects the whole corpus at every ladder step and publishes the
//! canonical step's field aligned into the baseline frame. Otherwise it publishes the
...
impl<'fit> PlacementPass<'fit> {
    ...
    pub(super) fn new(
        ...
        // running the schedule and projecting every step for an invalid selection.
