...
//! [`file::identity2`](crate::file::identity2)'s, and the row domain a file covers travels in
...
mod tests {
    ...
    use crate::{
        file::{
            identity2::{FileHeader, KeyKind, Kind, PaddedFileHeader, read::IdentityFile},
            region::write_region,
        },
    ...
    fn scratch(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!(
            "hash-graph-atlas-prepare-identity2-{}",
            std::process::id(),
        ));
