...
/// The source dataset and the staging ground every ingest stage shares.
pub(super) struct Ingest<'fit, D> {
    /// The dataset the stages drain.
    pub dataset: &'fit D,
    /// The staged generation that receives the artifacts.
    pub staging: &'fit StagedGeneration,
    /// The scratch directory backing the instance spool.
    pub scratch: &'fit ScratchDirectory,
}
    ...
    D: Dataset,
{
    ...
    where
        ...
        P: Progress + Sync,
    {
        // Nodes: write every representation into the staged matrix and pair it with its identity
        // table. Map the matrix back to certify the source contract on the rows. The type column
        // stays resident for the quadtree build.
        ...
        let norm = self.certify_representations(config)?;
        ...
        // Ontology: the parent column stays resident for the postings build, and the card stream
        // below covers the same rows.
        let type_parents = self
            .collect_type_parents()
        ...
        let cards = self
            .embed_card_table(embedder, prior, progress)
    ...
    ///
    /// The representations fill the staged `f32[N, 512]` matrix and the ids the staged identity
    /// file. The type column stays resident for the quadtree build.
    ///
    ...
    async fn stage_representations<E>(&self) -> Result<NodeArtifacts, FitError<D::Error, E>> {
        ...
        let digest = digest_file(self.staging.path_of(&artifact::Representations::NAME))?;
    ...
    fn certify_representations<E>(
        &self,
        config: &FitConfig,
    ) -> Result<norm::NormSpotCheck, FitError<D::Error, E>> {
        let _span = tracing::info_span!("norm-check").entered();

        let representations =
            ArrayFile::open(self.staging.path_of(&artifact::Representations::NAME))
    ...
    ///
    /// Ids into the staged identity file, endpoints into the staged `u64[E, 2]` column, the
    /// relation universe into its ascending set, and one spooled reading per `(edge, relation)`
    /// pair for the relation stage.
    ///
    ...
    async fn stage_edges<E>(&self) -> Result<EdgeArtifacts, FitError<D::Error, E>> {
        ...
        let mut spool = InstanceSpoolWriter::create(self.scratch)?;

        let mut writer = BufWriter::new(self.staging.create(&artifact::EdgeEndpoints::NAME)?);
        let mut endpoints = <[NodeRowId; 2]>::writer(&mut writer)?;

        let mut stream = pin!(self.dataset.edges());
        while let Some(edge) = stream.try_next().await.map_err(FitError::Dataset)? {
            ...
            // Every direct type produces its own reading at share 1/multiplicity: a multi-typed
            // link is a mixture of its relation domains' opinions, one link's worth of
            // force in total.
        ...
        let digest = digest_file(self.staging.path_of(&artifact::EdgeEndpoints::NAME))?;
    ...
    async fn collect_type_parents<E>(
        ...
        let mut stream = pin!(self.dataset.ontology());
    ...
    {
        let (cards, ids) = async {
            let mut stream = pin!(self.dataset.render_cards());
        ...
        let embeddings = self
            .staging
            .stage_with(artifact::CardEmbeddings, |writer| {
                table.write_embeddings_into(writer)
            })?;
        let hashes = self.staging.stage_with(artifact::CardHashes, |writer| {
