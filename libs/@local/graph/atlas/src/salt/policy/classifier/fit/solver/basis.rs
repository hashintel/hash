...
//!
//! Adding a common scalar to every class logit moves no probability, and one degree of freedom
//! among the class logits is redundant. The solver therefore parameterizes the classifier in a
...
//! columns are orthonormal (`BᵀB = I₂`) and orthogonal to the constant vector (`Bᵀ1 = 0`).
//! Contrast coordinates therefore span exactly the shift-free directions of logit space: [`expand`]
//! maps a contrast vector to class logits and [`reduce`] projects a class-space vector back, and
//! the roundtrip is the identity up to rounding.
//!
...
///
/// Doubling is exact in binary floating point, and this is also the correctly rounded value of the
/// real `2/√6`.
