...
//! own representational spacing. The solver therefore treats such a reduction as a stall rather
//! than progress. [`objective_resolution`] returns that threshold, `R(F̄) = ulps · ulp(|F̄|)`, where
//! `ulp` is the spacing of the f64 grid at the objective's magnitude. Below the maximum finite
//! value that spacing is the next-up spacing. At the maximum it becomes the predecessor spacing,
//! because the next-up spacing would be infinite. Zero and subnormal magnitudes take the minimum
//! positive subnormal, the grid's smallest step. A finite negative objective remains valid evidence
//! and contributes only its magnitude to the spacing.

use core::num::NonZero;

use crate::math::{DFinite, DNonNegative, DPositive};

/// Returns the f64 grid spacing at a finite non-negative magnitude.
...
/// Computes the resolution `R(F̄) = ulps · ulp(|F̄|)` of a finite objective value.
///
