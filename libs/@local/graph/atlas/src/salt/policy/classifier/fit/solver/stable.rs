...
///
/// Returns [`None`] when the reduced value is not finite; a non-finite value entering the fold can
/// only produce a non-finite accumulator, so checking the result covers every component and
/// intermediate.
...
///
/// Subnormal-only vectors keep their norm and magnitudes near [`f64::MAX`] stay finite where naive
/// squared accumulation would not. The norm of the empty and the all-zero vector is `0.0`. Returns
/// [`None`] when a component or the result is not finite.
