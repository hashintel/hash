...
pub(crate) struct AuthorizedSnapshot<'snapshot> {
    links: Box<[AuthorizedLink<'snapshot>]>,
    rejection_counts: LinkRejectionCounts,
}

impl<'snapshot> AuthorizedSnapshot<'snapshot> {
    /// Borrows admitted links in candidate order.
    #[must_use]
    #[inline]
    pub(crate) fn links(&self) -> &[AuthorizedLink<'snapshot>] {
        &self.links
    }
...
pub(crate) async fn authorize_snapshot<'snapshot, Store>(
    store: &Store,
    actor: AuthenticatedActor,
    temporal_axes: QueryTemporalAxesUnresolved,
    candidates: &'snapshot [LinkCandidate<'snapshot>],
) -> Result<AuthorizedSnapshot<'snapshot>, Report<SnapshotError>>
where
...
fn permission_inputs(candidates: &[LinkCandidate<'_>]) -> (Vec<EntityId>, Vec<VersionedUrl>) {
