...
/// `try_read_from_bytes`, the path a mapped byte region validates through. One test per type.
/// The profile selects by module path, so moving a test in or out of this module is the whole edit.
mod miri {
    #![expect(
        clippy::host_endian_bytes,
        reason = "is_bit_valid reads the candidate as a native-endian primitive with no \
                  byte-order conversion, so the fixture bytes must be the host's native order too"
    )]
    ...
    #[test]
    fn open_unit_fraction_try_from_bytes() {
        assert_eq!(
