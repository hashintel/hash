...
/// Every `TryFromBytes` impl generated by `unsafe_impl_try_from_bytes!` accepts exactly its
/// type's canonical bit pattern and refuses everything else, read straight through
/// `try_read_from_bytes` - the path a mapped byte region validates through.
#[test]
#[expect(
    clippy::float_cmp,
    reason = "the read-out values are the exact literals encoded into the byte arrays above them"
)]
#[expect(
    clippy::host_endian_bytes,
    reason = "is_bit_valid reads the candidate as a native-endian primitive with no byte-order \
              conversion, so the fixture bytes must be the host's native order too"
)]
fn try_from_bytes_accepts_only_canonical_bit_patterns() {
    assert_eq!(
    ...
    NonNegative::try_read_from_bytes(&(-1.0_f32).to_ne_bytes()).expect_err("negative is refused");
    ...
    GreaterThanOne::try_read_from_bytes(&1.0_f64.to_ne_bytes()).expect_err("one itself is refused");
    ...
    PositiveUnitFraction::try_read_from_bytes(&f64::NAN.to_ne_bytes()).expect_err("NaN is refused");
