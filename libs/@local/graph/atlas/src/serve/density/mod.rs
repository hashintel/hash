...
pub(crate) struct DensityBand {
    /// The least count inside the band, L.
    lower: NonZero<u64>,
    /// The greatest count inside the band, U.
    upper: NonZero<u64>,
}
...
/// A recorded schedule that leaves the density policy no delivery-cut offset to resolve.
#[derive(Debug, Copy, Clone, PartialEq, Eq)]
pub(crate) enum DensityPolicyError {
    /// The deepest bucket `max_tile_depth + span` exceeds the Morton key width.
    Schedule {
    ...
    /// The deepest zoom is the root, whose one catch-all tile no density policy deepens.
    TerminalRoot,
}
...
pub(crate) struct DensityPolicy {
    /// The occupied-cell target.
    band: DensityBand,
    /// The span exponent s, the cells per tile axis of the delivery cut as a base-2 log.
    span: Log2,
    /// The offset ceiling h = 32 − (z + s).
    ceiling: Zoom,
}
...
impl DensityPolicy {
    ...
    /// Returns the coarser of `zoom` and [`resolve`](Self::resolve)'s offset for `occupancy`.
    #[must_use]
