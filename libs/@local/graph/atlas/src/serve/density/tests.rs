...
/// Builds a density policy over the band `[lower, upper]` with the fixture span and tile depth.
///
/// # Panics
///
/// Panics if `upper` lies below `lower`.
...
/// Builds a view with one additional occupied cell per depth through depth 24.
///
...
/// Returns the corner key of one cell of the depth's grid.
///
/// # Panics
///
/// Panics if `depth` exceeds the key width or `(x, y)` lies outside the depth's grid.
...
/// Builds a view whose occupancy plateaus once and then splits.
///
...
/// [`DensityBand::new`] accepts ordered and single-count bands and refuses inverted ones.
#[test]
...
/// `distance` is zero at both band endpoints and one immediately outside either.
#[test]
fn band_endpoints() {
...
/// `distance` is zero inside the band and the gap to the nearer endpoint outside it.
#[test]
...
/// [`DensityPolicy::new`] refuses a root-only schedule and one beyond the key width.
///
/// It returns [`DensityPolicyError::TerminalRoot`] at the minimum zoom and
/// [`DensityPolicyError::Schedule`] when the span and maximum depth exceed the key width.
#[test]
...
/// An empty view occupies no cell at any depth and has no distinct keys.
#[test]
...
/// A view whose occupancy plateaus reaches its four distinct keys first at depth three.
///
/// Depth three is its saturation depth.
#[test]
...
/// Resolving an empty view yields the minimum zoom.
#[test]
...
/// The profile equals a direct census of distinct prefixes at every depth.
///
/// `occupied_cells` at each depth equals the count of distinct key prefixes at that depth,
/// `distinct_keys` equals the set size, and the saturation depth is the coarsest depth reaching
/// it.
#[property_test]
...
/// `resolve` is order-invariant, within the ceiling and nearest the band.
///
/// `resolve` is invariant under key order, never exceeds the ceiling, and resolves to a zoom whose
/// occupancy is nearest the band.
#[property_test]
...
/// Rebinding from a deep resolution onto an empty view yields the minimum zoom.
#[test]
