//! Cases covering withdrawal subtraction: rows leave, runs stay contiguous and re-sum.

use hashql_core::id::Id as _;

use super::subtract;
use crate::{identity::NodeRowId, morton::Depth, serve::schedule::DeliveredNodes};

/// Builds a [`DeliveredNodes`] fixture over `rows` partitioned by `runs`.
///
/// The rows convert to [`NodeRowId`]s, and the first bucket is the minimum depth.
...
/// Converts raw row numbers to [`NodeRowId`]s.
///
/// The result compares against a [`DeliveredNodes`]'s `rows` after a `subtract` call.
fn nodes(rows: &[u64]) -> Vec<NodeRowId> {
    rows.iter().copied().map(NodeRowId::new).collect()
}

/// Asserts that `delivered`'s run lengths still sum to its row count.
///
/// This is the invariant `subtract` must preserve.
///
/// # Panics
///
/// Panics when the run lengths do not sum to the number of delivered rows.
#[track_caller]
