...
impl AdjacencyRows {
    /// Records `edge` as incident to `node`, keeping each node's edge list sorted and unique.
    ...
    /// Returns `node`'s recorded incident edges, in sorted order.
    fn get(
        &self,
...
/// Added-edge incidence for a topology's nodes, indexed by direction.
#[derive(Debug, Default)]
...
/// Endpoint pairs and visibility decisions for a topology's edges.
#[derive(Debug, Default)]
...
impl EndpointDelta {
    /// Returns `edge`'s endpoint pair at `revision`, or at the current state for `None`.
    fn get(
        &self,
...
impl TopologyDelta {
    ...
    /// Composes this delta with `base` into one topology provider.
