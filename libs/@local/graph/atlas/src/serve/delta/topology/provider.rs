...
use crate::{
    ...
    serve::{codec::RowDomain, delta::DeltaRevision, world::topology::TopologyProvider},
};
...
pub(crate) trait VersionedTopologyProvider: TopologyProvider {
    /// Returns the snapshotted node row domain, including withdrawn and unbound rows.
    fn provide_node_domain(&self) -> RowDomain<NodeRowId>;

    /// Returns the snapshotted edge row domain, including withdrawn and unbound rows.
    fn provide_edge_domain(&self) -> RowDomain<EdgeRowId>;

    /// Returns `edge`'s endpoint pair at `revision`, applying the retention policy on eviction.
    ...
    /// Returns `node`'s incoming edges at `revision`, applying the retention policy on eviction.
    ...
    /// Returns `node`'s outgoing edges at `revision`, applying the retention policy on eviction.
...
impl<T: ?Sized> NaiveTopologyProvider<T> {
    /// Reinterprets a borrowed `T` as an ordinary topology provider, without copying it.
...
impl<'delta, B: ?Sized> DeltaTopologyProvider<'delta, B> {
    /// Composes `data`'s local bindings over `base`.
...
impl<'delta, B: VersionedTopologyProvider + ?Sized> DeltaTopologyProvider<'delta, B> {
    /// Returns `node`'s incoming edges at `revision`, or at the current state for `None`.
    fn lookup_incoming(
        &self,
        node: NodeRowId,
    ...
    /// Returns `node`'s outgoing edges at `revision`, or at the current state for `None`.
    fn lookup_outgoing(
        &self,
        node: NodeRowId,
        revision: Option<DeltaRevision>,
