//! Validated projection of incoming embeddings into fitted-world coordinates.
...
impl Position {
    /// Wraps `point`, or returns `None` if either coordinate is non-finite.
...
/// A projection failure for one input row.
#[derive(Debug, Copy, Clone, PartialEq)]
...
/// A projector open or roundtrip-validation failure.
#[derive(Debug)]
...
/// Reused per-chunk buffers for one forward pass.
...
impl ProjectorScratch {
    /// Rows processed per model invocation.
    const CHUNK_SIZE: usize = 256;

    /// Allocates buffers sized for one chunk.
    fn new() -> Self {
        Self {
...
/// An opened embedding projector, validated against a sample of its fitted coordinates.
