...
use crate::{
    identity::NodeRowId,
    math::Vec2,
    serve::{codec::RowDomain, delta::DeltaRevision, world::layout::LayoutProvider},
};
...
pub(crate) trait VersionedLayoutProvider: LayoutProvider {
    /// Returns the snapshotted node row domain, including withdrawn and unplaced rows.
    fn provide_node_domain(&self) -> RowDomain<NodeRowId>;

    /// Returns `node`'s position at `revision`, applying the retention policy on eviction.
...
impl<T> NaiveLayoutProvider<T> {
    /// Wraps `value`, an ordinary [`LayoutProvider`].
...
impl<'delta, B> DeltaLayoutProvider<'delta, B> {
    /// Composes `data`'s local placements over `base`.
