//! Stable row allocation with revision-dependent identity visibility.
...
/// An ordinary identity provider whose keys and payloads are the same at every revision.
#[repr(transparent)]
pub(crate) struct NaiveIdentityProvider<T: ?Sized>(T);

impl<T: ?Sized> NaiveIdentityProvider<T> {
    /// Reinterprets a borrowed `T` as an ordinary identity provider, without copying it.
    #[inline]
...
impl<K, R, P> IdentityProviderResidual<K, R, P> {
    /// Starts an empty residual, snapshotting `base`'s domain.
    #[inline]
    ...
    /// Composes this residual's additions with `base` into one identity provider.
    #[inline]
...
/// Identity queries composing a residual's additions with a base provider.
...
impl<'ctx, B, K, R, P> DeltaIdentityProvider<'ctx, B, K, R, P> {
    /// Composes `data`'s additions over `base`.
...
{
    /// Returns whether `row` is visible, at the current state or at `revision`.
    ...
    /// Resolves `row`'s visible key at `revision`.
    ///
    /// `revision = None` selects the current state. The lookup returns `None` when the row is not
    /// visible at the selected state.
    ...
    /// Resolves the row visible for `key` at `revision`.
    ///
    /// `revision = None` selects the current state. The lookup returns `None` when the resolved row
    /// is not visible at the selected state.
    ...
    /// Resolves `key`'s current payload when visible at `revision`.
    ///
    /// `revision = None` selects the current state. A residual payload takes precedence over
    /// `base`. The lookup returns `None` when the key's row is not visible at the selected state.
    ...
    /// Resolves `row`'s current payload when visible at `revision`.
    ///
    /// `revision = None` selects the current state. Re-deriving the row's key through a borrowed
    /// copy of this provider preserves the payload's borrow.
...
{
    /// Borrows `row`'s current payload when it is visible at `revision`.
    ///
    /// The returned borrow outlives `self`. `revision = None` selects the current state. The row's
    /// key follows [`DeltaIdentityProvider::lookup_key`]'s visibility rules.
    ...
    /// Borrows `row`'s current payload beyond this provider when visible at `revision`.
...
where
    ...
    B: VersionedIdentityProvider<K, R>,
{
    /// Returns the allocated row-domain's size, including rows the residual has withdrawn.
    #[inline]
    fn count(&self) -> usize {
        self.data.domain.size()
    }

    /// Resolves `row`'s key at the current state, hiding a withdrawn row.
    #[inline]
    fn key_of(&self, row: R) -> Option<K> {
        self.lookup_key(row, None)
    }

    /// Resolves `key`'s row at the current state, hiding a withdrawn key.
    #[inline]
    fn row_of(&self, key: K) -> Option<R> {
        self.lookup_row(key, None)
    }

    /// Resolves `key`'s current payload, preferring the residual before `base`.
    #[inline]
    fn payload_of_key(&self, key: K) -> Option<&K::Payload> {
        self.lookup_payload_of_key(key, None)
    }

    /// Resolves `row`'s current payload, preferring the residual before `base`.
    #[inline]
    fn payload_of_row(&self, row: R) -> Option<&K::Payload> {
