...
/// A synthetic generation and the delta opened over it, retaining the generation's temp files.
struct Fixture {
    _generation: TamperFixture,
    delta: Delta,
}

/// Opens a synthetic world under `name` and creates a delta over it, retaining its temp files.
///
/// # Panics
///
...
/// Observes an update for `seed` and classifies it as a node, queuing it for placement.
...
/// Builds display parts carrying `label`, a fixed icon, and a fixed representative id.
...
/// Projects the zero vector to a single finite position, reused across placement fixtures.
///
/// # Panics
///
/// Panics if the projector returns no row or rejects the projected position.
...
/// Collects `pairs` into a display-answer map keyed by edition id.
fn displays(
...
/// Keeps equal and older overlaps in their pending phase.
///
/// Only a later transaction time replaces the retained edition and resets its stage to
/// classification.
#[test]
fn observe_overlap() {
    let mut pending = Pending::default();
    let entity = ArchivedEntityId::from(entity(1));
    assert_matches!(pending.observe(update(1, 10, false)), DeltaAction::None);
    ...
    pending
        ...
        .stage = Stage::Place;
    assert_matches!(pending.observe(update(1, 10, false)), DeltaAction::None);
    assert_matches!(pending.observe(update(1, 9, true)), DeltaAction::None);
    assert_eq!(pending.updates[&entity].id, first);
    assert_matches!(pending.updates[&entity].stage, Stage::Place);
    assert_eq!(pending.updates.len(), 1);
    assert_matches!(pending.observe(update(1, 11, false)), DeltaAction::None);
    assert_ne!(pending.updates[&entity].id, first);
    assert_matches!(pending.updates[&entity].stage, Stage::Classify);
    assert_eq!(pending.updates.len(), 1);
}

/// Keeps a replacement waiting for the prior request, whose result cannot apply to it.
#[test]
fn receive_superseded() {
    ...
    assert_matches!(pending.updates[&entity].stage, Stage::Normalize(_));
}

/// Retains outstanding placement state across every removal.
///
/// Each removal cancels its update, but the submitted placement remains until completion.
#[test]
fn observe_removals() {
    let id = entity(1);
    ...
    for removal in removals {
        ...
        assert_matches!(pending.observe(removal), DeltaAction::Withdraw(withdrawn) if withdrawn == entity);
...
/// Keeps versions and outstanding requests independent for equal UUIDs in different webs.
#[test]
...
/// Reuses fitted-node coordinates for metadata updates and revival.
#[test]
...
/// Preserves queued input under admission backpressure.
///
...
/// Ends input admission on closure without marking an unsent update as running.
#[test]
fn pump_closed() {
...
/// Cancels queued placement when an update archives its entity.
///
/// No allocation or revival can follow the archived result.
#[test]
fn receive_archived() {
...
/// Prevents overlap from retrying a failed placement budget.
///
/// Failure removes pending work but retains the event's transaction-time high-water mark.
#[test]
fn receive_failed() {
...
/// Normalizes successful placements before display capture.
#[test]
fn receive_wire_coordinates() {
    ...
    assert_eq!(scratch, [] as [math::vec2::Vec2; 0]);
...
/// Ignores a superseded edition's display response.
///
/// Only a response for the current edition advances capture.
#[test]
...
/// Releases admission after a failed superseded request without discarding its replacement.
#[test]
...
/// Reuses an added node's first position after revival.
///
/// Older epochs remain withdrawn.
#[test]
...
/// Restores request order and drains normalization scratch state.
///
/// The classification and dispatched-edition counts equal the queued counts. Each captured
/// position differs from the expected wire-frame normalization by no more than the tolerance.
#[test]
fn normalize_batch() {
    let mut pending = Pending::default();
...
/// Discards a completed placement superseded before normalization.
///
/// The replacement returns to classification and dispatches no edition.
#[test]
fn normalize_superseded() {
    ...
    assert_matches!(pending.updates[&key].stage, Stage::Normalize(_));
...
/// Defers an edge until both endpoint identities resolve.
///
/// An edge with only one resolved endpoint captures its display but cannot apply. Once both
/// endpoints resolve, the edge applies and its identity resolves through the topology provider.
#[test]
