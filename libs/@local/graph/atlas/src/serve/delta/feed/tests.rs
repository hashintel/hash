use alloc::sync::Arc;
use core::{assert_matches, future, ops::ControlFlow, pin::pin, time::Duration};
...
/// A feed task opened over a synthetic generation, with its placement channel ends.
struct Fixture {
...
/// Builds a feed task over a synthetic world published under `name`.
///
/// The task has disconnected placement channels and a fresh publication handle.
///
/// # Panics
///
...
async fn new_interval_zero() {
    ...
    assert_matches!(error.current_context(), DeltaFeedError::InvalidInterval);
}

/// Rejects replay windows outside the representable transaction-time range.
///
/// An unrepresentable replay window - an infinite safety lag at the epoch, or a lag exceeding
/// the earliest representable timestamp - is refused. A zero lag at the earliest timestamp is
...
async fn new_replay_window() {
    ...
    for (watermark, lag) in [
        ...
        assert_matches!(error.current_context(), DeltaFeedError::InvalidSafetyLag);
    }
...
async fn publication_closed() {
    ...
    assert_matches!(error.current_context(), DeltaFeedError::Closed);
}

/// Keeps placement pending when projector channels are absent.
///
...
/// Builds an updated-entity event for `seed` at `seconds`, optionally marked archived.
...
async fn step_result_closed() {
    ...
    assert_matches!(error.current_context(), DeltaFeedError::PlacementClosed);
}
...
/// Preserves an allocated row while replaying its withdrawal.
///
