...
use super::{
    ...
    topology::provider::NaiveTopologyProvider,
};
use crate::{
    ...
    serve::{
        ...
        walk::Walk,
        world::{World, topology::TopologyProvider as _},
    },
};

/// Opens a synthetic world under `name` and allocates a delta over it.
///
/// # Panics
///
...
/// Builds an entity id from a fixed web and `seed`, for compact literal test identities.
...
/// Snapshots `delta` into an epoch usable for read-side queries.
fn epoch(delta: &Delta) -> Epoch {
    Epoch::from(Guard::from_inner(Arc::new(delta.clone())))
}

/// Counts allocated edge rows, including withdrawn and unbound rows.
///
/// # Panics
///
/// Panics if `epoch` does not belong to `world`.
fn edge_count(world: &World, epoch: &Epoch) -> usize {
    epoch
        .topology(&world.topology)
        .bind(NaiveTopologyProvider::from_ref(&world.topology))
        .provide_edge_count()
}

/// Round-trips every base row and rejects rows outside the captured domain.
///
/// Rejected rows include the first unallocated row and the largest representable wire id.
#[test]
fn decode_base() {
...
/// Tracks decoded row visibility at each capture.
///
/// It is absent before allocation (except a base row, present from the start), present once
/// live, absent again after withdrawal, and present after revival. The id's own encoding never
/// changes throughout. A wire id past the final capture's domain still decodes to absent.
#[test]
fn decode_captures() {
...
/// Panics when decoding a wire id against an epoch from another world.
#[test]
...
/// Retains the resolved actor on full and partial masks.
///
/// Both user and machine principals preserve their identity and instance-administrator status.
#[test]
fn mask_actor() {
...
/// Preserves a node's row and first coordinates across insertion and revival.
#[test]
fn node_added_revival() {
    ...
    assert_eq!(
        first
            .topology(&delta.world.topology)
            .bind(NaiveTopologyProvider::from_ref(&delta.world.topology))
            .provide_node_count(),
        count
    );
...
/// Uses full identities to distinguish equal entity UUIDs in different webs.
#[test]
...
/// Restores incident edges on node revival unless an edge has its own withdrawal.
#[test]
...
/// Keeps added edges unbound until endpoint rows resolve, then retains their first pair.
#[test]
fn edge_unbound_revival() {
    ...
    assert_eq!(
        edge_count(&delta.world, &unbound),
    ...
    assert_eq!(
        edge_count(&delta.world, &revived),
        usize::try_from(EDGES).expect("should fit the fixture count") + 1
    );
}

/// Tracks an added self-loop with its endpoint node's visibility.
#[test]
...
/// Preserves fitted and added ontology-row identities across icon replacement.
#[test]
...
/// Keeps captured inherited icons stable across later replacements.
///
/// An ontology row without its own icon inherits an ancestor's, borrowing the same captured
/// payload. A distinct direct icon on a descendant is not shadowed by that inheritance. Replacing
/// the ancestor's icon later changes what a fresh capture inherits without invalidating an
/// earlier capture's already-borrowed icon.
#[test]
...
/// Panics when resolving an ontology icon against an epoch from another world.
#[test]
...
/// Borrows the mapped base legend without copying it.
#[test]
fn payload_base() {
...
/// Excludes later registrations and unknown rows from captured ontology keys.
#[test]
...
/// Panics when resolving an edge payload against an epoch from another world.
#[test]
...
/// Panics when resolving an ontology key against an epoch from another world.
#[test]
#[should_panic(expected = "ontology must belong to the epoch's world")]
...
/// Panics when resolving a node payload against an epoch from another world.
#[test]
#[should_panic(expected = "index must belong to the epoch's world")]
...
/// Panics when resolving an ontology payload against an epoch from another world.
#[test]
#[should_panic(expected = "ontology must belong to the epoch's world")]
...
/// Reuses a delta allocation without mutating an earlier publication.
#[test]
...
/// Composes schedule visibility from the mask and captured placements.
#[test]
...
/// Keeps corpus delivery stable while scoped delivery follows withdrawal.
///
/// A withdrawal hides a fitted row from placement without changing the corpus-wide delivery
/// schedule. Scoping to the remaining visible rows drops exactly that one row's bucket assignment
/// and reduces the total count by one.
#[test]
...
/// Builds a partial visibility mask admitting exactly `nodes`, with no edges admitted.
...
/// Combines admitted base and extension positions into view bounds.
///
/// A view's bounds extend the base corpus bounds by an added node's position when its mask admits
/// that node, narrow to only that node's point when the mask admits nothing else, and cover every
/// added and base node under a full mask. Withdrawing the node collapses a narrow view's bounds
/// to none while leaving a wider admitted view and an unrelated cached view unaffected, and
/// reviving it restores the narrow view's bounds.
#[test]
fn bounds_extension() {
    ...
    let captured = epoch(&delta);
    let admitted = schedule_mask((0..NODES).map(NodeRowId::new).chain([row]));
    ...
    let narrow = ViewSchedule::of(Arc::clone(&world), &captured, &schedule_mask([row]));
    ...
    let withdrawn = epoch(&delta);
    let current = ViewSchedule::of(Arc::clone(&world), &withdrawn, &schedule_mask([row]));
    ...
    let revived = epoch(&delta);
    let current = ViewSchedule::of(Arc::clone(&world), &revived, &schedule_mask([row]));
    assert_eq!(current.bounds(), Some(point));
}

/// Distinguishes captured, scoped and corpus bounds after base withdrawal.
///
/// Withdrawing every base node collapses a view scoped to only those nodes to no bounds, while a
/// view captured before the withdrawal keeps the original base bounds and a corpus-wide view
/// after the withdrawal still recovers the recorded base bounds.
#[test]
...
/// Panics when scoped bounds include a non-finite placement.
///
/// A corpus-wide view containing the same node still resolves its bounds.
#[test]
...
fn bounds_nonfinite_placement() {
    ...
    let base = ViewSchedule::of(
        ...
        &captured,
        &schedule_mask((0..NODES).map(NodeRowId::new)),
    );
    assert_eq!(base.bounds(), delta.world.layout.base_bounds());
    ViewSchedule::of(Arc::clone(&delta.world), &captured, &schedule_mask([row]));
}

/// Checks matched [`ViewSchedule`] and [`ScopeSchedule`] cuts over selected offsets.
///
/// At each offset, the check visits every node row through the first unallocated row. It samples
/// the root and its child cells at every served zoom. Bounds, root delivery, minimum resolution,
/// and every visited node and cell entry must agree.
///
/// # Panics
///
/// Panics on fixed offset, zoom, or cell construction failure, on cut failure, if the root has no
/// children, or if the schedules disagree.
#[track_caller]
...
/// Orders visible extension rows and preserves captured views across changes.
///
/// Added rows delivered under a partial mask sort after the base rows and in identity-priority
/// order among themselves, matching a matched-scope reference walk. Withdrawing one added row
/// removes it from a fresh view while an earlier captured view's totals stay unaffected, and
/// reviving it restores the original delivery.
#[test]
...
/// Keeps corpus delivery stable across fitted-row withdrawal and revival.
///
/// An added row past the base domain is assigned to the corpus's deepest bucket. Withdrawing a
/// base row removes it from a scoped view while a corpus-wide view recorded afterward reproduces
/// the same totals, root delivery, minimum resolution, and per-node schedule entries as before
/// the withdrawal, and reviving the base row restores agreement between the two schedule paths.
#[test]
...
/// Asserts that a delivery's per-bucket run lengths sum to its total delivered row count.
///
/// # Panics
///
/// Panics if the run lengths do not sum to the number of delivered rows.
#[track_caller]
...
/// Subtracts current withdrawals from a recorded corpus schedule.
///
...
/// Subtracts a newer withdrawal from the scoped schedule it reads.
#[test]
...
impl LaterEpochScene {
    /// Builds a scene fixture whose schedule predates added neighbor rows.
    ///
    /// Opens a synthetic world under `name`, captures a full-visibility schedule before two
    /// partner nodes attach to fixture node 4, then places and links them so the captured
    /// schedule predates their delivery zoom while their placement is already visible.
    ///
    /// # Panics
    ///
    /// Panics on fixture publication or world opening failure, a missing source row, rejected
    /// partner or link insertion, row-domain exhaustion, zero-offset cut failure, or a failed
    ...
    /// Builds a locate document capped at `edges` incident edges.
    ///
    /// The document uses the fixed source and this scene's captured world, epoch, mask and
    /// schedule.
    ///
    /// # Errors
    ///
    /// Returns [`LocateDocumentError`] if document construction fails.
    ///
    /// # Panics
    ///
    /// Panics if the captured schedule cannot bind its zero-offset cut.
    #[track_caller]
    fn locate<'scene>(
        &'scene self,
        edges: u32,
...
/// Skips partner ranking while a locate remains within capacity.
///
...
/// Rejects an unrankable partner before hydrating a truncated locate.
///
...
/// Normalizes geometry against fitted rather than already-normalized bounds.
#[test]
