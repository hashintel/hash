...
/// The number of revisions [`History`] retains before falling back to the initial state.
pub(in crate::serve::delta) const CAPACITY: usize = HISTORY_SIZE;

/// Defines visibility answers around the retained history boundaries.
///
/// A revision before the initial entry has no answer. The initial entry holds through the entry
/// pushed after it, and the most recent push becomes the current and future answer.
#[test]
...
/// Evicts only the oldest entries after pushes exceed retained capacity.
///
/// The first `HISTORY_SIZE` revisions past the drop point still resolve to their recorded state.
/// The immediately preceding revision has no answer, while a revision after the last push falls
...
/// Preserves retention capacity when a pushed entry changes nothing.
///
/// Every redundant push reports no change. The sole real transition remains resolvable at and
/// after its revision.
#[test]
...
/// Keeps only the last transition pushed at one revision.
///
/// Earlier revisions are never affected.
#[test]
...
/// Panics when a pushed revision precedes the last recorded revision.
#[test]
...
/// Accepts the maximum revision for both pushing and querying.
#[test]
...
/// Preserves a value's birth revision after bounded-history eviction.
///
/// Before birth, liveness is false. At and after birth, liveness comes from the (now-evicted)
/// initial entry, and the retained data remains unaffected by history rollover.
#[test]
...
/// Reports whether a push changes the current visibility.
///
/// Pushing the value's current visibility again reports no change. Pushing the opposite one does,
/// and liveness reflects the latest pushed kind.
#[test]
...
/// Panics when a visibility decision predates the value's birth.
#[test]
#[should_panic(expected = "history revisions must be nondecreasing")]
