//! Immutable request captures of one published delta revision.
...
/// The captured delta, held either by an owned [`Arc`] or an [`arc_swap::ArcSwap`] [`Guard`].
enum InternalEpoch {
    Full(Arc<Delta>),
...
impl InternalEpoch {
    /// Returns an owned handle to the same publication.
    ///
    /// A shared source remains unchanged and keeps its guard. The returned handle holds an [`Arc`]
    /// instead of acquiring another [`Guard`].
...
/// A request handle retaining one immutable delta publication.
///
/// An [`ArcSwap`](arc_swap::ArcSwap) load selects one published [`Delta`], and later swaps do not
/// alter it. All component queries through this handle use the same world, lifetime tag and
/// revision. The handle remains readable after feed completion, runtime retirement or registry
/// closure, but its existence does not imply that the feed was healthy or caught up when
...
impl Epoch {
    /// Returns the captured world's generation identifier.
    pub(crate) fn generation(&self) -> GenerationId {
        self.delta.world.generation().id()
    }

    /// Returns the captured delta's identity and revision.
    ...
    /// Returns an independently owned epoch over the same captured delta.
    ///
    /// The returned epoch owns an [`Arc`] instead of another [`Guard`]. The source epoch remains
    /// valid with unchanged ownership. A shared source retains its guard, while an owned source
    /// retains its [`Arc`]. Forking does not block a concurrent publication.
