...
/// Builds an initial pending entry with `event` as both its event and entity seed.
...
/// Builds a completed entry for `event` that failed with an exhaustion error.
...
/// Returns slow-ticking placement options with a small retry budget and queue depth 2.
...
/// Constructs a placement task over an unconnected pool, using [`options`].
///
/// # Panics
///
...
/// Spaces nonempty projection batches by the configured minimum interval.
///
/// Projection is due with queued work when no prior projection exists or the minimum interval
...
/// Keeps projection cadence unchanged while delivering completed results.
///
/// A tick that only delivers already-completed results, with no new admission or projection,
...
/// Admits requests only up to the configured pending limit.
///
/// Admission stops at the configured queue depth, leaving the remainder on the input channel for
...
/// Caches actor lookup failure and exhausts every waiting submission.
///
/// Resolution then breaks.
#[tokio::test]
...
/// Reuses a cached unavailable-actor outcome without polling.
///
/// Queued workflow submissions still exhaust.
#[tokio::test]
...
/// Reuses a cached actor without polling.
///
/// Queued workflow submissions remain untouched.
#[tokio::test]
...
/// Returns reusable projector state while isolating per-row failures.
///
/// Projecting a batch preserves request order in the result and reports a per-row projection
/// error for a non-finite embedding without failing its neighbors. It also returns a projector
/// usable for a later batch.
#[tokio::test]
async fn projection_owned_batch() {
    ...
    assert_matches!(error.current_context(), PlacementError::Projection);
    assert_eq!(
...
/// Runs projection on the worker pool rather than the calling task.
///
/// With the pool's only thread occupied, a first poll returns pending, and the batch still
/// completes once the worker runs.
#[tokio::test]
