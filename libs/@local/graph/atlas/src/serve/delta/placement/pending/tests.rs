...
use crate::{
    ...
    serve::delta::{feed::EventId, placement::PlacementError},
};

/// Builds an entity id from a fixed web and `value`, for compact literal test identities.
...
/// Builds an embedding with every component set to `fill`.
fn embedding(fill: f32) -> BoxedVecN<PROJECTOR_DIMENSIONS> {
    BoxedVecN::from([fill; PROJECTOR_DIMENSIONS])
}

/// Builds a completed entry that failed with an exhaustion error.
...
/// Enqueues a fresh request with its full database-poll budget.
#[test]
fn enqueue_budget() {
...
/// Moves found database embeddings into projection without cloning them.
///
/// An embedding found in the batch moves that entry to projection by relocating the map's
/// allocation rather than cloning it, while an entry missing from the batch remains polling.
#[test]
fn database_hit() {
    let mut pending = Pending::default();
...
/// Promotes a database hit on the last allowed try.
#[test]
...
/// Promotes a database hit even when no tries remain.
#[test]
...
/// Moves a database miss to workflow submission only when its budget ends.
///
/// Misses decrement the budget, and the terminal one promotes the entry exactly once.
#[test]
fn database_miss() {
    let mut pending = Pending::default();
...
/// Consumes found workflow embeddings exactly once.
///
/// An embedding found in the batch moves that entry to projection and consumes it from the
/// batch's source map so it is not read twice.
#[test]
fn workflow_hit() {
    let mut pending = Pending::default();
    let hit_entity = entity(1);
...
/// Promotes a workflow hit on the last allowed try.
#[test]
...
/// Promotes a workflow hit even when no tries remain.
#[test]
...
/// Completes a workflow miss only when its budget ends.
///
/// Misses decrement the budget, and the terminal one emits one exhaustion result.
#[test]
fn workflow_exhaustion() {
    ...
    assert_matches!(report.current_context(), PlacementError::Exhaustion);
...
/// Completes queued workflow submissions with exhaustion when submission is unavailable.
///
/// Completion preserves queue order.
#[test]
fn submit_exhaustion() {
    ...
    assert_matches!(first_report.current_context(), PlacementError::Exhaustion);

    assert_eq!(pending.completed[1].entity, second);
    let Err(second_report) = &pending.completed[1].phase.0 else {
        panic!("should complete with an error")
    };
    assert_matches!(second_report.current_context(), PlacementError::Exhaustion);
}

/// Counts entries across every pending phase, including completed results.
#[test]
fn len_phases() {
    let mut pending = Pending::default();
...
/// Keeps undelivered completions queued in their original order.
///
/// Flushing more completions than the channel currently admits sends what fits and leaves the
/// rest queued, oldest first.
#[test]
fn flush_full() {
...
/// Delivers one queued completion per reopened channel slot, in order.
#[test]
fn flush_resume() {
    let (tx, mut rx) = mpsc::channel(1);
    let mut pending = Pending::default();
    ...
    assert_matches!(pending.flush(&tx), ControlFlow::Continue(()));
...
/// Discards the popped completion when its channel has closed.
#[test]
fn flush_closed() {
    ...
    assert_matches!(result, ControlFlow::Break(()));
    assert!(pending.completed.is_empty());
}
