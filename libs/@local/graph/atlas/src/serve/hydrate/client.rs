...
/// Resolves each requested node's direct-type URLs from the store, filling `nodes` in place.
///
/// # Errors
///
...
/// # Panics
///
/// Panics if the store returns a row outside the requested identities, since the slot lookup
/// indexes by that identity, and if a column fails to decode or a direct-type count is negative.
async fn read_types(
...
/// Resolves `source`'s actor-masked, `cap`-limited scalar properties.
///
/// # Errors
///
/// Returns [`HydrateError`] when a store read fails or when the store answers more than one row
/// for `source`.
///
/// # Panics
///
...
impl GraphDatabaseClient {
    /// Builds a client hydrating detail through `pool`, with resolver calls run on `runtime`.
    #[must_use]
    ...
    /// # Errors
    ///
    /// Returns [`HydrateError::Connect`] where the pool cannot supply a connection.
    ...
    /// # Panics
    ///
    /// Panics if a column fails to decode or a stored URL does not parse as its domain type.
...
mod tests {
    ...
    /// Builds the masking actor over the user `actor` names.
    ...
    /// Both resolvers answer an empty request with no results, inside a blocking worker.
    ///
    /// [`TypeUrlResolver::resolve`] and [`LocateResolver::resolve`] each run from inside a
    /// `spawn_blocking` worker without the runtime treating their nested blocking call as a
    /// stalled task.
    #[tokio::test]
