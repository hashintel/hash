...
/// Reports whether the combined filter admits every row.
///
/// That holds for no caller filter and a policy filter that is an empty `All` conjunction.
...
/// Resolves `actor`'s visible-row mask against `world`'s epoch.
///
/// The query carries `filter` and the deployment's property protection.
///
/// # Errors
///
/// Returns [`VisibilityProofError`] from the first failing stage of policy assembly, filter
/// conversion or compilation, and visibility querying.
...
mod tests {
    ...
    fn unconstrained_permit() {
        ...
        assert_matches!(&scoped_with_forbid, Filter::All(conjuncts) if !conjuncts.is_empty(), "the fixture builds the non-empty conjunction it is here to reject: \
             {scoped_with_forbid:?}");
