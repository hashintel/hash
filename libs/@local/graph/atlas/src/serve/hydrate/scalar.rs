...
/// One entity's scalar properties, ascending by base URL.
#[derive(Debug, Clone, PartialEq)]
pub(crate) struct ScalarProperties(Vec<(BaseUrl, ScalarValue)>);

impl ScalarProperties {
    /// An empty property list, usable in a `const` context.
    pub(crate) const EMPTY: Self = Self::empty();

    /// Builds an empty property list.
    pub(crate) const fn empty() -> Self {
        Self(vec![])
    }

    /// Returns the number of scalar entries.
    ...
    pub(crate) fn new(
        ...
        if maximum > 0 {
            // swap the label property into the last retained slot, dropping whichever entry
            // would otherwise have held it
...
mod tests {
    ...
    /// A subscriber layer that records every `WARN`-level event's fields.
    ///
    /// A test reads the recorded fields once the traced code under test has run.
    struct Warnings(Arc<Mutex<Vec<Fields>>>);

    impl<S: Subscriber> Layer<S> for Warnings {
        /// Asserts the event's level and appends its captured [`Fields`] to the warning list.
        ///
        /// `WARN` is the only level this fixture expects [`ScalarProperties::new`] to emit.
        ///
        /// # Panics
        ///
        /// Panics for non-`WARN` events or a poisoned warning-list mutex.
