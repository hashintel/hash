...
impl Fixture {
    /// Publishes a synthetic generation under `name` and opens its world.
    ///
    /// # Panics
    ///
...
/// Returns a generation identity no fixture publishes.
///
/// # Panics
///
/// Panics if the fixed hexadecimal string does not parse as a generation identity.
...
/// Builds an observer over `world` with a delta identity drawn from `seed`.
...
/// Returns the delta lifetime published by `observer`.
fn lifetime(observer: &Observer) -> DeltaReference {
    observer.delta.load().reference()
}

/// Promotes `observer` at `now`, asserting that the registry is still open.
///
/// # Panics
///
/// Panics if the registry has closed admission.
#[track_caller]
...
/// Builds a registry after a second generation displaces the fixture's generation.
///
/// The returned instant is the displacement time.
///
/// # Panics
///
/// Panics if the displacement instant exceeds the range of [`Instant`] or the registry closes
/// during promotion.
...
/// # Panics
///
/// Panics if the selection does not share `world` or its epoch names another generation. It also
/// panics if the first node has no visible position.
#[track_caller]
...
/// # Panics
///
/// Panics if either side fails [`assert_universe`] or carries a delta lifetime other than
/// `published`.
#[track_caller]
...
/// Rejects requests from an empty registry with [`ObserveError::Empty`].
///
/// This error precedes availability checking.
#[test]
fn observe_empty() {
    let registry = UniverseRegistry::new(HARD);
...
/// Rejects every request from a closed registry with [`ObserveError::Closed`].
#[test]
fn observe_closed() {
...
/// Rejects an unpromoted generation with [`ObserveError::Unavailable`] and names it.
#[test]
fn observe_unknown_generation() {
    ...
    assert_matches!(error, ObserveError::Unavailable(generation) if generation == unpromoted, "the refusal should name the requested generation");
}

/// Selects the active generation when a request names no generation.
#[test]
...
/// Selects the active publication when a request names that generation.
#[test]
...
/// Bounds the public admission timestamp by instants sampled around the call.
#[test]
...
/// Pairs a displaced requested generation with the new active generation.
#[test]
...
/// Admits a displaced generation one nanosecond before its deadline without a cleanup pass.
#[test]
...
/// Rejects a displaced generation at its deadline without a cleanup pass.
#[test]
fn observe_retained_at_expiry() {
    ...
    assert_matches!(error, ObserveError::Unavailable(generation) if generation == displaced, "the refusal should name the expired generation");
}

/// Rejects a displaced generation at retirement when retention is zero.
#[test]
fn observe_zero_retention() {
    ...
    assert_matches!(error, ObserveError::Unavailable(generation) if generation == displaced, "the refusal should name the displaced generation");
...
/// Appends an expired generation to caller output and removes its registry entry.
#[test]
...
/// Drops the retained entry on reactivation and retires the displaced generation.
#[test]
...
/// Replaces the active generation's previous delta lifetime with a fresh one.
#[test]
...
/// Refuses promotion after closure and returns the supplied observer.
#[test]
fn promote_closed() {
...
/// Keeps a held observation readable after retirement, cleanup and closure.
#[test]
