...
//! present feed retries with a new [delta lifetime](crate::serve::delta::DeltaId), while an ended
//! retained feed preserves its final publication until expiry or reactivation. Each restart
//! samples a new tag without obtaining a uniqueness guarantee.
...
pub(crate) struct ManagerOptions {
    ...
    /// Controls removal of expired directories after feed joining.
    ///
    /// The default retains them.
...
impl GenerationManager {
    ...
    /// Borrows the registry requests take publication handles from.
    pub(crate) const fn registry(&self) -> &Arc<UniverseRegistry> {
        &self.registry
    }

    /// Reads the current-pointer lookup's result when it is already available.
    ///
    /// This never blocks on the pointer read and leaves a running lookup alone.
    ...
    /// Polls the current-pointer lookup to completion, ready at once when none is outstanding.
    ...
    /// Records a finished pointer read, keeping the previous selection on failure.
    ...
    /// Admits `generation`'s runtime to the registry and makes it the present one.
    ///
    /// Promotion is refused where the slot holds no runtime, and a closed registry stops the
    /// manager instead, because a manager that cannot publish has nothing left to maintain.
    ...
    /// Runs one maintenance pass over the pointer, the slots and the registry.
    ///
    /// The pass reads the current pointer and advances every slot. It expires admissions that
    /// have outlived their retention, promotes the selected generation where one is ready, and
    /// then reconciles the slot set against what is wanted. Slots advance both before and after
    /// reconciliation so a slot opened during the pass makes progress within it.
    ...
    /// Returns whether `generation` holds an initialized runtime not yet promoted.
    ...
    /// Returns whether `generation` holds an initialized runtime, ready or already running.
    ///
    ...
    /// Brings the slot set in line with the desired and present generations.
    ///
    /// A wanted generation is opened or reopened, an unwanted stopped one leaves unless it is
    /// still published, and an expired one is removed where the operator enabled unlinking.
    ...
    /// Closes admission and asks every slot to stop, without waiting for any of them.
    fn stop(&mut self) {
        self.stopping = true;
        self.registry.close();
    ...
    /// Resolving `shutdown` closes admission and drains owned work. Dropping this borrowing future
    /// before completion instead leaves pending handles and results in the manager for a later
    /// call.
    ///
    /// # Panics
    ///
    ...
    /// shutdown starts no new removal or recovery. Dropping this borrowing future before completion
    /// retains every pending handle and result in the manager.
...
impl Drop for GenerationManager {
    /// Signals every slot to stop when the manager is dropped.
    ///
    /// This signals and does not wait: a destructor cannot await. Therefore, a caller that needs
    /// the feeds joined calls [`Self::shutdown`] before dropping. Dropping without it leaves
    /// runtime tasks stopping in the background. Current-pointer, opening and removal work already
