...
impl fmt::Debug for Execution {
    /// Names the execution's variant and nothing inside it.
    ///
    /// The contents are runtimes, join handles and offload tasks, none of them printable.
    /// The variant is what the pattern assertions here report on a mismatch. Each variant with
    /// a payload prints as non-exhaustive.
...
impl Fixture {
    /// Publishes one synthetic generation under `name` and opens its world.
    ///
    /// # Panics
    ///
    /// Panics if publishing the synthetic generation fails or its world does not open.
    ...
    /// # Panics
    ///
    /// Panics on generation tampering, republication, or world opening failure, or if `marker`
    /// leaves the generation identity unchanged.
    #[track_caller]
    ...
    /// Returns the generation root used by the manager.
    ///
    /// # Panics
    ///
    /// Panics if the fixture generation has no parent directory or root opening fails.
...
/// Runs `test` on a paused current-thread runtime with a one-second virtual timeout.
///
/// # Panics
///
/// Panics if Tokio runtime construction fails, if `test` panics, or if `test` exceeds the virtual
/// timeout.
#[track_caller]
...
/// A waker target that records notifications without rescheduling the future.
...
/// Checks that `complete` wakes a suspended `task` and makes it ready.
///
/// # Panics
///
/// Panics when the task is ready before `complete` runs, when the completion does not wake it, or
/// when it is not ready after being woken. Propagates panics from polling `task` or invoking
/// `complete`.
#[track_caller]
...
/// # Panics
///
/// Panics if `pass` panics, the capture lock has become poisoned, or formatted output is not UTF-8.
#[track_caller]
fn captured_warnings(pass: impl FnOnce()) -> String {
    /// A shared byte buffer that captures one pass's formatted log events.
    struct Capture(Arc<Mutex<Vec<u8>>>);

    impl std::io::Write for Capture {
        /// Appends every offered byte, returns `buf.len()` and never writes short.
        ///
        /// # Panics
        ///
        /// Panics when the buffer's lock is poisoned, which needs a panic inside another
        /// writer.
...
/// Builds an empty store pool for the configured test socket path.
///
/// # Panics
///
...
/// A controlled source task's completion channel, carrying its slot result or panic payload.
type SourceReply<T> = oneshot::Sender<Result<Result<T, Report<ManagerError>>, Box<dyn Any + Send>>>;

/// Builds a source handle whose sender controls completion.
...
/// Wraps `result` in an already-settled source handle.
///
/// # Panics
///
/// Panics when the task dropped its receiving end before the result was sent.
#[track_caller]
...
/// Returns the instant `offset` after `base`.
///
/// # Panics
///
/// Panics if adding `offset` exceeds the range of [`Instant`].
#[track_caller]
...
/// Builds a feed that ends after observing cancellation.
///
/// # Panics
///
/// Panics outside a Tokio runtime. The spawned task also panics if the caller drops its observation
...
/// Builds a feed that waits for release after observing cancellation.
///
/// # Panics
///
/// Panics outside a Tokio runtime. The spawned task also panics if the caller drops an observation
/// or completion receiver, or drops the release sender before release.
...
/// Builds a feed that ends on release without awaiting cancellation.
///
/// # Panics
///
/// Panics outside a Tokio runtime. The spawned task also panics if the caller drops its release
/// sender or completion receiver before the corresponding event.
...
/// Builds a runtime over `world` with its delta identity seeded by `seed`.
...
/// Builds a candidate slot containing an initialized runtime.
...
/// Builds a candidate slot containing an active opening.
...
/// Returns the world and delta lifetime visible to a fresh request for `generation`.
///
/// The active world and captured epoch must both name `generation`, whose first node must have a
/// visible position.
///
/// # Panics
///
/// Panics if the registry does not admit its active generation, the captured world or epoch names
/// another generation, or the first node has no visible position.
#[track_caller]
fn published(
...
/// # Panics
///
/// Panics if the registry admits `generation` or refuses it for another reason.
#[track_caller]
...
/// # Panics
///
/// Panics if the registry admits the request or refuses it for another reason.
#[track_caller]
...
/// # Panics
///
/// Panics if `generation` has no slot or its unfinished execution is not an active opening.
...
/// # Panics
///
/// Panics if `generation` has no slot or its unfinished execution is not an active removal.
...
/// # Panics
///
/// Panics if a stopped feed drops its observation sender without reporting cancellation.
async fn drain(
...
/// Fails a controlled case whose task stalls.
#[test]
...
/// Advances completed openings and drains pointer reads at the polling cadence.
#[test]
...
/// Runs maintenance once a second and keeps expired directories.
#[test]
fn options_default() {
...
/// Rejects a zero manager polling interval with [`ManagerError::InvalidInterval`].
#[tokio::test]
...
/// Publishes no selected generation until its opening succeeds.
#[tokio::test]
...
/// Publishes a successfully opened selection without a warning.
#[tokio::test]
...
/// Keeps the previous generation published while its candidate opening remains pending.
#[test]
...
/// Keeps the previous publication while reopening a failed selected candidate.
#[tokio::test]
...
/// Reuses a retained world and lifetime when reselected before expiry.
#[test]
...
/// Reopens an expired generation with a fresh delta lifetime.
///
/// Its previous feed joins before reopening begins.
#[tokio::test]
...
/// Stops an expired retained feed without removing its directory.
///
/// Admission closes with the feed.
#[test]
...
/// Lets another expired feed observe shutdown while one join stalls.
#[test]
...
/// Recovers an ended present feed with a fresh lifetime over its retained world.
#[tokio::test]
...
/// Recovers an ended retained feed only at its next selection.
#[tokio::test]
...
/// Stops a feedless runtime while retaining its world.
///
/// The next maintenance pass reaches the stable stopped state regardless of prior promotion.
#[test]
fn joining_feedless() {
...
/// Keeps a runtime joining until its feed worker ends.
///
/// The next maintenance pass then retains the world and reaches the stopped state.
#[test]
...
/// Preserves a feedless generation's lifetime across maintenance passes.
#[test]
...
/// Wakes shutdown only when the stopping feed completes, not when it observes the stop request.
#[test]
...
/// Suspends shutdown on a current-pointer read and wakes it when the read completes.
#[tokio::test]
...
/// Suspends shutdown during generation opening and wakes it on completion.
#[tokio::test]
...
/// Suspends shutdown on generation removal and wakes it when removal completes.
#[tokio::test]
...
/// Stops every initialized feed and closes admission before any join returns.
#[test]
...
/// Retains an unjoined feed result after cancellation interrupts shutdown waiting.
#[test]
...
/// Stops and joins a superseded initialized candidate instead of publishing it.
#[test]
...
/// Stops and joins an opening that completes unselected during shutdown.
#[test]
...
/// Reopens a selected expired generation instead of unlinking its directory.
#[tokio::test]
...
/// Finishes a started removal before reopening the generation.
#[tokio::test]
...
/// Removes an expired directory only after joining, while held requests remain readable.
#[tokio::test]
...
/// Reopens from disk instead of reusing the world after generation removal fails.
#[tokio::test]
...
/// # Panics
///
/// Panics if `generation` has no slot, its execution is neither opening nor ready, or the active
/// opening fails.
...
/// Retains an ended feed's publication until a later recovery attempt succeeds.
#[tokio::test]
...
/// Resumes shutdown after cancellation interrupts its wait and the opening finishes.
#[test]
