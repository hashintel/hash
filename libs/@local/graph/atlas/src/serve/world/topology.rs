...
///
/// Queries require CSR adjacency lists whose runs agree with the endpoint column. Opening compares
/// counts without establishing that correspondence.
#[derive(Debug)]
pub(crate) struct Topology {
    /// The entity key and display payload of every fitted edge row.
    pub identity: IdentityTableArchive<ArchivedEntityId, EdgeRowId>,

    /// The incoming and outgoing edge lists of every fitted node row.
    adjacency: AdjacencyArchive,
    /// The `[source, target]` node rows of every fitted edge row.
...
impl Topology {
    ...
    ///
    /// Returns [`None`] for a withdrawn edge and for an edge whose endpoint node has no visible
    /// placement at that revision.
    ///
    /// # Panics
    ///
...
impl TopologyProvider for Topology {
    #[expect(
        ...
        reason = "the archive's validation converted its row count to `usize` before deriving the \
                  node count from it, and an archive that opened has a representable count"
    )]
    fn provide_node_count(&self) -> usize {
