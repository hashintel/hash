//! Wire-id encoding for one row domain of an opened world.
//!
//! A world derives one [`RowCodec`] per row domain when it opens. Every row the fitted generation
//! holds is encoded once at that point. Rows added later are encoded on demand by the codec.
...
pub(crate) struct Encoding<I> {
    /// The keyed mapping between the domain's rows and their wire ids.
    codec: RowCodec<I>,
    /// The wire id of every row in the opening domain, in row order.
...
impl<I> Encoding<I> {
    ...
    /// # Complexity
    ///
    /// The table lookup and codec fallback take O(1) time in the opening row count and use constant
    /// additional space. A miss applies eight keyed Feistel rounds through [`RowCodec::encode`].
    ///
    /// # Panics
    ///
    /// Panics when the table lookup misses and [`RowCodec::encode`] rejects `row` at or beyond
    /// [`WIRE_ROW_BOUND`](crate::serve::codec::WIRE_ROW_BOUND).
    ///
    /// # Warning
    ///
    /// A lossy [`Id::as_usize`] conversion can select a different cached row before the codec
    /// checks the original ID. For [`NodeRowId`](crate::identity::NodeRowId) on a 32-bit target,
    /// row 2³² selects index zero in a nonempty lookup.
