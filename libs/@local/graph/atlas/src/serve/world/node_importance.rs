...
// The derived `Ord` compares the variant before its contents: the declaration order is the
// priority order, and every rank precedes every identity.
#[derive(Debug, Copy, Clone, PartialEq, Eq, PartialOrd, Ord)]
pub(crate) enum NodePriority {
    /// A fitted node's importance rank.
    Rank(ImportanceRank),
    /// An unranked node's entity key, ordering it after every ranked node.
    Identity(ArchivedEntityId),
}

/// Node ordering over allocated rows, independent of visibility.
pub(crate) trait ImportanceProvider {
    /// Returns the node's priority, or [`None`] if no priority is available for the row.
...
pub(crate) struct NodeImportance {
    /// The rank of every base position.
    lookup: Column<BasePosition, ImportanceRank>,
    /// The base position of every rank, the inverse of `lookup`.
...
impl NodeImportance {
    ...
    /// Returns the number of ranked positions, the length of both columns.
...
mod tests {
    ...
    /// Builds an entity key from `u128` web and entity UUID values.
    ...
    /// Open refuses a rank-of-position column short of the position-of-rank column.
    ///
    /// Returns [`WorldError::NodeImportanceCountMismatch`].
    #[test]
    ...
    /// Open refuses a position-of-rank column short of the rank-of-position column.
    ///
    /// Returns [`WorldError::NodeImportanceCountMismatch`].
    #[test]
