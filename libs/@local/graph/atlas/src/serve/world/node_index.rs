...
pub(crate) struct NodeIndex {
    /// The entity key and display payload of every fitted row.
    pub identity: IdentityTableArchive<ArchivedEntityId, NodeRowId>,
    /// The wire-id codec of the node row domain, every fitted row precomputed.
    encoding: Encoding<NodeRowId>,

    /// The row of every base position.
    lookup: Column<BasePosition, NodeRowId>,
    /// The base position of every row, the inverse of `lookup`.
...
impl NodeIndex {
    ...
    /// and its `usize` conversion misses the precomputed table.
    ///
    /// # Warning
    ///
    /// On a 32-bit target, truncating a large row for the cache lookup can return another fitted
    /// row's wire id, as [`Encoding::encode`] documents.
    ...
    ///
    /// Returns [`None`] when the row the wire value decodes to lies outside the allocated row
    /// domain, and for a row whose identity is not live at the captured revision. The check is on
    /// the decoded row, not the wire value: the permutation can carry a large wire value to a
    /// small allocated row.
    ///
    /// # Panics
    ///
    ...
    /// Returns the number of fitted positions, the length of the row-of-position column.
    pub(crate) fn len(&self) -> usize {
        self.lookup.len()
    }

    /// Returns the exclusive bound of the fitted row domain: the first row no base position holds.
