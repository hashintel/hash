...
//! rows under their [`NodePriority`](crate::serve::world::node_importance::NodePriority) order.
...
pub(crate) struct ScopeSchedule {
    /// The captured rows in natural-bucket order, with the indexes delivery reads them through.
    column: BucketColumn,
}
...
impl ScopeSchedule {
    ...
    /// Builds the complete base cascade [saturated scopes](super::ViewSchedule::of) share.
    ///
    /// # Panics
    ///
    /// Panics if a base node has no position or priority.
    ...
    /// Borrows the assigned bucket column delivery reads from.
    pub(super) const fn column(&self) -> &BucketColumn {
        &self.column
    }

    /// Assigns the cascade over `rows`, without an external key set to rank against.
