//! Cases covering the visible cascade: bucket assignment, delivery order and the density offset.
...
/// Builds a recorded schedule over zooms `0..=zoom` whose tiles have `2^span` cells per axis.
///
/// `span` is the base-2 exponent of the cell count per tile axis, the schedule's [`Log2`] span.
///
/// # Panics
///
/// Panics if `span` reaches the shift width, `zoom` exceeds the key width, or `span + zoom` does.
...
/// Captured rows at `keys`, alternating the two priority forms.
///
/// Alternating rank and identity priorities keeps the cases from passing under an implementation
/// that orders one form only, and the identity form's three webs exercise its tie-breaking.
#[expect(
...
/// Returns the leading `depth` levels of `key`, two bits per level.
...
/// Computes the natural bucket of each row: the first depth without a better-ranked occupant.
///
/// Rows whose complete keys are equal never separate, and take the deepest level, 32.
...
/// Computes the delivery the cases expect over `interval` from the rows rather than the column.
fn delivery(
    rows: &[ScheduleNode],
    natural: &[u8],
    deepest: u8,
...
/// An empty scope delivers nothing at every query, and still reports one run per bucket.
#[test]
fn delivery_empty() {
...
/// A density offset is accepted exactly when `span + zoom + offset` fits the key width.
///
/// An accepted cut reports the deepened schedule.
#[test]
...
/// Asking for a cut beyond the schedule's deepest served zoom is refused.
#[test]
