//! Cases covering delivery at one cut: what each zoom adds, and what the catch-all absorbs.
...
/// Reads the generation's recorded assignment straight from its artifacts.
///
/// The cases compare delivery against this independent reading rather than against the schedule
/// that produced it.
///
/// # Panics
///
/// Panics if the Morton or row-of-position artifact fails to open, if a Morton position does not
/// fit [`BasePosition`], or if the row-of-position column is shorter than the Morton domain: every
/// position in `0..morton.count()` converts and then indexes it. The generation is any published
/// one rather than one [`World::open`] accepted, and the Morton file's own validation checks its
/// fenceposts' structure and the file length, not the position range.
...
/// Asserts that one cell's delivery at `zoom` matches the recorded assignment.
///
/// Asserts cumulative delivery through the cut, the increment at this zoom and the child mask for
/// cells with deeper rows.
///
/// # Panics
///
/// Panics when `zoom` lies beyond the schedule's deepest served zoom, or when the cumulative
/// delivery through the cut, the incremental delivery at `zoom`, or the child mask of `cell`
...
/// Corpus delivery reproduces the recorded assignment at every served zoom.
///
/// The case checks every recorded key's own cell.
#[test]
...
/// Corpus delivery reads the recorded keys after zeroing the coordinate artifact.
///
/// The generation records the assignment rather than recomputing it from placements.
#[test]
