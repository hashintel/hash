...
/// the extent the entry records is not a range within `buffer`. An unwritten slot's `(0, 0)` entry
...
/// Builds a complete delivery of zero edges at minimal detail.
...
/// Builds the `HEAD` map of a document with `count` edges.
///
/// # Panics
///
/// Panics on a `digest` above 255 bytes, or a `variant` or `count` above 255: each is a one-byte
/// CBOR argument here.
fn expected_head(
    digest: &[u8],
    variant: u64,
