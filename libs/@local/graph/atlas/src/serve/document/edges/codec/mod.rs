...
impl EdgesResponse<'_> {
    ...
    /// Encodes the edges trailer map.
    ///
    /// The map holds the interned representative type URLs, the per-edge labels, and each edge's
    /// representative-type index into that interned list.
