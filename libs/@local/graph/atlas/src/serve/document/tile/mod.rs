...
use crate::{
    ...
    serve::{codec::EncodedRowId, membership::OntologySelection, scene::Scene, walk::Walk},
};

hashql_core::id::newtype! {
    /// A reference to one delivered point by its slot in this document's tile order.
    pub(crate) struct TileSlot(u32)
}

/// Caps on one tile request: the requested types it may carry.
...
/// Whether a tile document carries only its columns or also the auxiliary trailer.
#[derive(Debug, Copy, Clone, PartialEq, Eq)]
pub(crate) enum TileDocumentDetailLevel {
    /// Columns alone: position, row id and optional type mask per point.
    Minimal,
    /// Columns plus the trailer: label and icon per point.
    Auxiliary,
}

/// Delivery mode, detail level, requested types and limits for one tile request.
...
/// A failure to assemble a tile document.
#[derive(Debug)]
...
/// The auxiliary detail for one tile document: each delivered point's label and icon.
...
impl<'details> TileTrailer<'details> {
    /// Builds an empty trailer with room for `capacity` points.
    ...
    /// Appends `row`'s label and icon from the captured scene.
    ///
    /// # Errors
    ///
    /// Returns [`TileDocumentError::Display`] when `row` has no display payload in the captured
    /// scene.
    fn push(
        &mut self,
