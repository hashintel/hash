...
impl Fixture {
    ...
    /// Builds a synthetic identity for node `seed`, derived from the seed byte alone.
    ...
    /// Returns the [`MortonKey`] of node row `row`'s captured position.
    ///
    /// # Panics
    ///
    /// Panics where the fixture's epoch captured no position for `row`.
    ...
    /// Returns `row`'s captured display label.
    ///
    /// # Panics
    ///
    /// Panics where the fixture captured no display payload for `row`.
    ...
    /// # Panics
    ///
    /// Panics where either published artifact does not open: the Morton key file or the
    /// row-of-position column. Both open on every call, after the world itself opened.
    ...
    /// # Panics
    ///
    /// Panics under `records`'s conditions, which it takes by opening the stored artifacts on
    /// every call.
    fn delivered_in(
        &self,
        first: Depth,
...
fn trailer_auxiliary_slot_alignment() {
    ...
    for (index, &row) in rows.iter().enumerate() {
        ...
        if row_index == 0 {
            ...
            assert_eq!(
                icon, "",
                "row {row_index}'s representative should have no icon"
            );
            saw_empty_icon = true;
        }
    }
...
/// A delivered row missing its position or display payload logs an internal problem.
///
/// [`Problem`]'s [`TileDocumentError`] conversion records the whole report in
/// [`Debug`](core::fmt::Debug) form, including the underlying source error's text and an attachment
/// placed on top of it. Outside debug builds the response carries the fixed detail instead of
/// either.
#[test]
fn missing_row_data_internal() {
    for context in [
        TileDocumentError::Position {
            row: NodeRowId::new(0),
        },
        TileDocumentError::Display {
            row: NodeRowId::new(0),
        },
    ] {
        let report = Report::new(io::Error::other("private-store-message"))
