...
impl TileResponse<'_> {
    ...
    pub(crate) fn encode_into<A: Allocator>(&self, buffer: &mut Vec<u8, A>) -> Envelope {
        ...
        // Slot 4 holds the reserved `MASS` column. A wireVersion-1 server marks it absent, which
        // the directory's initial (0, 0) entry already records.
        envelope.skip();
    ...
    ///
    /// The map runs from key 0 through key 10. It reserves key 5, and carries key 8 only for a
    ...
    /// Encodes the global-view head: visible count, optional bounds and minimum resolution.
    ...
    /// Encodes the tile trailer map: per-point labels and icons.
