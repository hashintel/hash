...
impl CurrentDocument {
    /// Captures `generation` as the active-generation document.
...
mod tests {
    ...
    /// Encoding a [`CurrentDocument`] replaces the buffer's bytes.
    ///
    /// What remains is the generation's lowercase-hexadecimal JSON encoding alone, not an appended
    /// copy of it.
    #[test]
    fn json_generation() {
