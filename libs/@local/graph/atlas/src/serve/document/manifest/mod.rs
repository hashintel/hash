...
/// A bucket-schedule cut, rendered as `z+<n>` for its `n`-level span.
...
/// The corpus bucket schedule's span, cut and maximum served zoom.
...
/// The resolved delivery scope's offset, cut and maximum served zoom for this request.
#[derive(serde::Serialize, schemars::JsonSchema)]
#[serde(rename_all = "camelCase")]
...
/// The document limits alongside the request's resolved authority windows.
///
/// The windows are the refresh threshold and the hard expiry, in whole seconds.
#[derive(serde::Serialize, schemars::JsonSchema)]
#[serde(rename_all = "camelCase")]
...
impl<'limits> ManifestDocument<'limits> {
    /// Builds the manifest for `scene`'s bound world and delivery schedule.
    ///
    /// The published limits are `limits` together with the request's resolved authority windows.
    pub(crate) fn new(
        Scene {
            world, delivery, ..
        }: Scene<'_>,
