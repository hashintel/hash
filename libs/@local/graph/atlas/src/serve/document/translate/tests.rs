...
impl Fixture {
    /// Publishes and opens the named synthetic generation, with a fresh delta identity.
    ///
    /// The mask grants one fixed test actor full visibility.
    ///
    /// # Panics
    ///
    /// Panics if publishing fails or [`World::open`] cannot open or validate the serving artifacts.
    ...
    /// Returns the [`EntityId`] the fixture generation assigned to node `row`.
    ///
    /// # Panics
    ///
    /// Panics where that generation holds no node row `row`.
    ...
    /// Returns the [`EntityId`] the fixture generation assigned to edge `row`.
    ///
    /// # Panics
    ///
    /// Panics where that generation holds no edge row `row`.
...
/// A mixed request keeps only the plain node and edge among its ids.
///
/// The request carries a duplicated node id, an edge id, a draft node id, a draft edge id and an
/// id with an unknown UUID. The duplicate collapses, and the document drops every id it cannot
/// resolve.
#[test]
...
/// Hiding a row drops it from the document, along with the edge touching it.
///
/// An untouched node or edge keeps the value it had in the unrestricted baseline.
#[test]
