...
use crate::{identity::NodeRowId, postgres::id::ArchivedEntityId, serve::codec::EncodedRowId};
...
impl<'doc> TranslateResponse<'doc> {
    /// Borrows `document`'s node and edge maps for serialization.
...
impl schemars::JsonSchema for TranslatedNode {
    ...
    /// Describes the object [`Self::serialize`] actually writes.
    ///
    /// The schema exposes `id`, `x` and `y` as top-level fields.
...
impl Serialize for TranslatedNode {
    /// Writes `id`, `x` and `y` as a flat three-field struct.
    fn serialize<S: Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        // a default derive would nest `position` instead of splitting it into its two coordinate
        // fields
