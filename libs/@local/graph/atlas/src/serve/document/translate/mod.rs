...
/// Caps on one translate request: the input identities it may name.
...
/// A failure to assemble a translate document.
#[derive(Debug)]
...
/// One resolved node's encoded row id and position.
...
/// One resolved edge's encoded source and target row ids.
#[derive(Debug, Copy, Clone, PartialEq, Eq, serde::Serialize, schemars::JsonSchema)]
