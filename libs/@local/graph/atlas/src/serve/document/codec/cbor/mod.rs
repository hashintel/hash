...
/// A [`fmt::Write`] adapter that appends UTF-8 bytes directly onto the wrapped buffer.
///
/// It lets [`CborWriter::collect_text`] format a value without an intermediate [`String`].
...
impl<'bytes, A: Allocator> CborWriter<'bytes, A> {
    ...
    pub(crate) fn int(&mut self, value: i64) {
        match u64::try_from(value) {
            ...
            // Major type 1 encodes the argument -1 - value. Complementing the two's-complement
            // bits computes that argument without negating. For value in i64::MIN..=-1 the
            // complement is in 0..=i64::MAX. Arithmetic negation overflows at i64::MIN instead.
