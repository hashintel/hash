...
use crate::{math::Vec2, postgres::id::ArchivedEntityId, serve::codec::EncodedRowId};
...
impl<'bytes, A: Allocator> ColumnWriter<'bytes, A> {
    ...
    /// Emits each row id as a little-endian column, in slot order.
    ...
    /// Emits each position as an interleaved little-endian x/y column pair, in slot order.
    ...
    /// Emits each entity identity as a fixed-width byte column, in slot order.
