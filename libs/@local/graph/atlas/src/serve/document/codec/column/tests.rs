...
///
/// Coordinates in `set` must lie inside the `rows` by `cols` grid. The byte index is `row *
/// ceil(cols / 8) + (col >> 3)` and the bit index is `col & 7`. An out-of-grid coordinate can still
/// address an allocated byte, setting a bit above the column count or in another row.
///
/// # Panics
///
/// Panics when the computed byte index is outside the allocated buffer. In overflow-checked builds,
/// also panics when the buffer-length or byte-index arithmetic overflows.
...
/// # Panics
///
/// Panics when the encoded length or bytes differ from what `set` implies. Also propagates the
/// [`expected_mask_bytes`] panic conditions for the supplied `set`, independently of the matrix
/// contents.
#[track_caller]
