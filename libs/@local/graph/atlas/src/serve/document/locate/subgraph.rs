...
/// A resolved source's row, identity, position and first delivery cell.
...
impl PartialEq for Keyed {
    /// Compares `key` alone. Edges sharing a key are interchangeable for this cap's ranking.
...
mod tests {
    ...
    /// Builds a synthetic identity distinguished by `value` alone, sharing one fixed web id.
    ...
    /// Parses `value` as a [`Zoom`] within the domain.
    ///
    /// # Panics
    ///
    /// Panics if `value` does not fit the domain.
    #[track_caller]
    ...
    impl Incident {
        /// Builds an empty incident set with no recorded edges or keys.
        fn new() -> Self {
            Self {
                edges: IdVec::new(),
                keys: IdVec::new(),
        ...
        /// # Panics
        ///
        /// Panics where [`zoom()`] cannot parse `zoom` into the [`Zoom`] domain.
        ...
        /// # Panics
        ///
        /// The returned function panics where an offered edge's row lies outside this set's key
        /// table. In overflow-checked builds it panics where `calls` already holds `usize::MAX`,
        /// which the count it records exceeds by one.
        fn rank<'this>(
            &'this self,
        ...
        /// # Panics
        ///
        /// Panics where the selection retains more than `capacity` after an offer, and for the
        /// conditions [`Self::rank`]'s returned function states. That function runs only after the
        /// offers exceed a nonzero `capacity`.
        #[track_caller]
        ...
        /// Selects this set's `capacity` smallest keys by a full sort, in identity order.
        ///
        /// # Panics
        ///
        /// Panics for the conditions [`reference()`] states, over this set's edges and keys.
        fn reference(&self, capacity: usize) -> EdgeSet {
            reference(&self.edges, &self.keys, capacity)
        }
    }

    /// Selects the `capacity` smallest-keyed edges by full sort and returns them in identity order.
    ///
    /// # Panics
    ///
    /// Panics where the sort evaluates the key function for an edge whose row lies outside
    /// `keys`. The sort evaluates that function only for the edges it compares.
    fn reference(
    ...
    /// Counts the edges a selection currently holds.
    ...
    fn nearest_zero_capacity() {
        ...
        let set = nearest.into_set();
        assert!(!set.complete);
        assert_eq!(set.edges, [] as [serve::neighbourhood::DeliveredEdge; 0]);
