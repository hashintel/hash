...
impl RecordedRequest {
    /// Copies the parts of `request` a [`FakeResolver`] needs to compare against an expectation.
...
/// The resolved detail a [`FakeResolver`] answers a request with.
struct Details {
...
/// A configured response from [`FakeResolver`].
///
/// Successful responses supply [`Details`] or report no resolution. Failure is the other configured
/// outcome.
enum Answer {
    Response(Details),
    Unresolved,
...
impl FakeResolver {
    /// Builds a resolver answering its one call with `response`.
    ...
    /// Builds a resolver failing its one call.
    ...
    /// Constructs a resolver whose `resolve` method must never run.
    ...
    /// Builds a deterministic fixture type URL for `name`.
    ///
    /// # Panics
    ///
    ...
    /// Builds a deterministic fixture property [`BaseUrl`] for `name`.
    ///
    /// # Panics
    ///
    /// Panics where `name` leaves the interpolated URL unparseable.
    ...
    /// Builds the [`ScalarProperties`] a JSON `value` reduces to.
    ///
    /// The reduction runs with no value cap and no label property.
    fn scalars(value: serde_json::Value) -> ScalarProperties {
        let (properties, _truncated) = ScalarProperties::new(value, None, usize::MAX);
        properties
    }

    /// Builds [`Details`] where every one of `nodes` nodes and `edges` links resolves.
    ///
    /// Each resolved row carries empty properties and no types.
    ...
    /// Marks node `slot` unresolved in `response`.
    ///
    /// Accepts a slot that is already unresolved. Marking the source node also clears the source
    /// properties.
    ///
    /// # Panics
    ///
    /// Panics where `slot` is not one of `response`'s node slots, and where `slot` lies outside
    /// the [`NodeSlot`] domain.
    ...
    /// Marks link `slot` unresolved in `response`.
    ///
    /// Marking an already unresolved slot changes nothing.
    ///
    /// # Panics
    ///
    /// Panics where `slot` is not one of `response`'s link slots, and where `slot` lies outside
    /// the [`EdgeSlot`] domain.
    fn unresolve_link(response: &mut Details, slot: usize) {
        response.links[EdgeSlot::from_usize(slot)] = None;
    }

    /// Builds [`Details`] over three nodes and two links that share one type URL.
    ///
    /// The node types and link 0's types share the `alpha` URL, and link 1 is left unresolved.
    ...
    /// Asserts that this resolver was called exactly once, with exactly the expected request.
    ///
    /// # Panics
    ///
    /// Panics if the recorded calls differ from that expectation or the request log is borrowed
    /// mutably.
    #[track_caller]
    ...
    /// Asserts that this resolver was never called.
    ///
    /// # Panics
    ///
    /// Panics if a request was recorded or the request log is borrowed mutably.
    #[track_caller]
...
impl LocateResolver for FakeResolver {
    /// Records the request and answers according to the configured [`Answer`].
    ///
    /// The request must arrive with every slot's details unset. The configured answer fills
    /// [`Details`] slot by slot or reports no resolution on success. It can instead fail.
    ///
    /// # Panics
    ///
    /// Panics if the request has resolved slots, its lengths differ from the configured response,
    /// no answer remains or either fixture cell is already borrowed.
    #[expect(
...
impl Fixture {
    /// Builds a user [`VisibilityActor`] distinguished by `id`, with the given instance-admin flag.
    ...
    ///
    /// The mask grants `actor` full visibility.
    ///
    /// # Panics
    ///
    ...
    ///
    /// The fixture's actor is a fixed non-admin test user.
    ///
    /// # Panics
    ///
    ...
    /// Builds a synthetic identity from a repeated `seed` byte.
    ...
    /// Builds an empty type selection, admitting no ontology type.
    ...
    /// Returns the identity the fixture generation assigned to `node`.
    ///
    /// # Panics
    ///
    /// Panics where that generation holds no identity for `node`.
    ...
    /// Builds a [`LocateSource::Key`] naming `node`'s [`EntityId`].
    ///
    /// # Panics
    ///
    /// Panics where the fixture generation holds no identity for `node`, the condition
    /// [`Self::node_identity`] states.
    fn entity_source(&self, node: NodeRowId) -> LocateSource {
        LocateSource::Key(EntityId::from(self.node_identity(node)))
    }

    /// Builds a [`LocateSource::Row`] naming `node`'s encoded row.
    fn row_source(&self, node: NodeRowId) -> LocateSource {
        LocateSource::Row(self.encode(node))
    }

    /// Returns `node`'s captured wire position.
    ///
    /// # Panics
    ///
    ...
    /// Returns the tile of the node's wire position at its first delivery zoom.
    ///
    /// # Panics
    ///
    /// Panics where the zero-offset delivery schedules no zoom for `node`, and on the captured
    /// position `position_of` requires.
    ...
    /// Returns `node`'s captured display label.
    ///
    /// # Panics
    ///
    /// Panics where the fixture captured no display payload for `node`.
    ...
    /// Selects the edge rows of [`ENDPOINTS`] touching `source`, in identity order.
    ...
    /// Lists the source followed by the distinct partners of `edges`, in encoded-row order.
    ///
    /// # Panics
    ///
    /// Panics where an edge in `edges` lies outside the fixture's endpoint table.
    ...
    /// Resolves each of `indices` against `trailer`'s interned type-URL table, in order.
    ///
    /// # Panics
    ///
    /// Panics where an index in `indices` lies outside that table.
    fn decode_type_urls(
    ...
    /// Resolves `properties`' interned indices against `trailer`'s property-URL table.
    ///
    /// The result uses the resolved [`BaseUrl`] as its key.
    ///
    /// # Panics
    ///
    /// Panics where an index in `properties` lies outside that table.
    ...
    /// # Panics
    ///
    /// Panics where an edge in `edges` lies outside the fixture's endpoint table, and for the
    /// conditions [`Self::node_identity`] and [`Self::fly_to`] state.
    #[track_caller]
    fn assert_geometry(
        &self,
    ...
    /// Asserts that `result` failed with [`LocateDocumentError::UnknownEntity`].
    #[track_caller]
...
/// One named input to [`trailer_completeness`].
///
/// The case supplies the source's direct types and requested type selection, with expected
/// completeness for both types and properties.
