...
/// # Panics
///
/// Panics outside `-256..=255`. The head carries one byte of argument, and a negative value
/// encodes `-1 - value` into it.
...
/// # Panics
///
/// Panics where a bit selects a word past the allocated storage, which holds `domain` rounded up
/// to whole 64-bit words. A bit between `domain` and that rounding sets a padding bit instead.
...
/// # Panics
///
/// Panics where the last slot's directory entry is unreadable, and where the padded end that
/// entry records lies past `buffer`. Rounding the recorded `u32` end to a multiple of eight uses
/// `usize`. On a 32-bit target with overflow checking, an end greater than `u32::MAX - 7` panics
/// during rounding.
///
/// # Warning
///
...
/// Builds an encoded row id from a raw `value`, bypassing the usual encoding path.
fn row(value: u32) -> EncodedRowId<NodeRowId> {
    EncodedRowId::new_unchecked(value)
}

/// Builds a synthetic identity from repeated `web` and `entity` bytes.
...
/// Builds the 32 identity bytes `HEAD` key 7 carries for [`identity`].
...
/// Builds a synthetic property [`BaseUrl`] distinguished by `name`.
///
/// # Panics
///
...
/// Builds a trailer with every array present and carrying no node and no link.
...
/// Builds the wire bytes of [`empty_trailer`]: ten keys over empty collections and a null map.
...
/// Builds a nonroot source alone in its ego graph, with an empty trailer.
...
/// Builds the ten-key `HEAD` map of [`minimal_document`], over the given variant and flags.
///
/// # Panics
///
/// Panics on a `digest` above 255 bytes, or a `variant` above 255: each is a one-byte CBOR
/// argument here.
