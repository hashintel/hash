...
impl PropertyMap {
    /// Interns `properties`'s base URLs into `table`, keying each value by its table index.
...
/// Interned URLs and per-source and per-link detail for one locate document.
///
/// The auxiliary data includes interned type and property URLs alongside the source's and every
/// link's labels, representative types and properties.
