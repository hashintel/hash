...
/// The hard-expiry threshold and expired-entry age.
...
impl Fixture {
    /// Builds a visibility-cache fixture at a fixed logical time.
    ///
    /// Its backing entries never expire. The fixture includes an active lifetime, an unrelated
    /// retired generation, one actor and serving artifacts published under `name`.
    ///
    /// # Panics
    ///
    /// Panics when the synthetic generation cannot be published or opened.
    ...
    /// Returns this fixture's cache key with `generation` substituted.
    ...
    /// Asserts the backing cache holds no entry under `key`.
    ///
    /// # Panics
    ///
    /// Panics when an entry is present.
    ...
    /// Inserts an entry under `key`, backdated by `age`, and returns it.
    ///
    /// # Panics
    ///
    /// Panics when `age` predates the instant range the fixture's clock reading allows.
...
fn pending(world: &Arc<World>, epoch: &Epoch, actor: ActorId) -> PendingCacheEntry {
    let mask = VisibilityMask::full(VisibilityActor {
        id: actor,
...
/// A resolver over the fixture's world and actor for any supplied lifetime.
///
/// A macro rather than a function because the closure captures the fixture's borrows by
/// value and each caller needs its own.
...
/// Guards cache-only paths with a resolver that must never run.
///
/// # Panics
///
/// Always, because the cache alone must answer these lookups.
///
/// # Errors
///
/// Never: the resolution this stands in for is one the lookup must not reach.
async fn forbidden(_epoch: &Epoch) -> Result<PendingCacheEntry, ()> {
    panic!("this lookup should not invoke the resolver")
}

/// Fails every resolution attempt with a fixed error.
///
/// # Errors
///
/// Always, with the message the case asserts on.
async fn refusing(_epoch: &Epoch) -> Result<PendingCacheEntry, &'static str> {
    Err("permission resolution failed")
}

/// Panics instead of returning a resolution.
///
/// # Panics
///
/// Always, when invoked.
...
/// # Errors
///
/// Never: it unwinds instead of returning.
#[expect(
...
/// Builds a delta lifetime over a tampered generation distinct from the fixture's.
///
/// # Panics
///
/// Panics when the placeholder cannot be rewritten or the tampered generation cannot
/// be opened.
...
/// Reopens the fixture's generation as another world and delta lifetime.
///
/// # Panics
///
/// Panics when the generation cannot be reopened.
...
/// Separates in-flight resolutions by their full lifetime keys.
///
/// A resolution admitted before a same-generation reopen remains under its original key.
...
/// Keeps a previous delta lifetime retired after its generation reopens.
///
/// Soft-stale, hard-expired and missing entries under the previous key do not resolve again.
#[tokio::test]
...
/// Reuses an unexpired retired entry without refreshing it.
///
/// The current lifetime can neither invoke the resolver nor take a refresh claim for the retired
/// generation.
#[tokio::test]
...
/// Refreshes a stale entry only after its lifetime becomes active again.
///
/// A stale entry remains reusable without refresh under a foreign lifetime. Returning to the
/// entry's own unchanged delta lifetime returns the entry and starts a refresh. A failed refresh
/// preserves the immediate answer, reports the error to the callback and releases the claim.
#[tokio::test(start_paused = true)]
...
/// Retains stale data after a detached refresh fails.
///
/// The error callback runs after releasing the refresh claim and under the scheduling span.
#[tokio::test(start_paused = true)]
...
/// Releases a refresh claim after resolver panic and admits the next attempt.
#[tokio::test]
...
/// Removes a retired entry at `HARD` age and returns no value.
#[tokio::test]
...
/// Reuses a soft-stale entry one nanosecond before expiration.
#[tokio::test]
...
/// Reuses an unexpired entry under a different generation's epoch.
#[tokio::test]
...
/// Resolves an active-generation miss and retains the supplied timestamp.
#[tokio::test]
...
/// Skips resolution and returns no entry for a retired-generation miss.
#[tokio::test]
...
/// Propagates a foreground resolution error instead of returning an expired fallback.
#[tokio::test]
...
/// Replaces an expired active-generation entry with a new resolution.
#[tokio::test]
...
/// Removes an expired retired-generation entry without resolving a replacement.
#[tokio::test]
