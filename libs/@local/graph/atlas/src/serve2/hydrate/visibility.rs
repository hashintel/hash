...
// ) -> Result<(VisibilityProof, VisibilityActor), Report<VisibilityProofError>>
// where
...
//     // protection and the actor is not an instance admin. The same condition governs here, since
// the     // same filter reaches the same compiler.
//     let protected;
//     let filter = match filter {
...
//     ///
//     /// The bug class is a short-circuit that reads a shape the policy compiler does not reserve
// for     /// unconstrained permits. The expectation therefore comes from
//     /// [`Filter::for_policies`](hash_graph_store::filter::Filter::for_policies) itself, not from
// a     /// hand-built filter, so a change in what that constructor emits fails here rather than
...
//         // A blank forbid denies everything, and no permit at all denies everything: neither is
// the         // tautology, and reading either as one would invert the decision.
//         let forbidden =
...
//     /// generation. An operator asking for a narrowed view would receive every row instead, which
// is     /// a wrong answer rather than a leak.
//     #[test]
