...
pub(crate) struct DeltaLayoutProvider<'delta, B: ?Sized> {
    data: &'delta LayoutDelta,
    base: &'delta B,
}

impl<'delta, B: ?Sized> DeltaLayoutProvider<'delta, B> {
    pub(crate) const fn from_parts(data: &'delta LayoutDelta, base: &'delta B) -> Self {
        Self { data, base }
    }
}

impl<B: VersionedLayoutProvider + ?Sized> LayoutProvider for DeltaLayoutProvider<'_, B> {
    fn provide_node_count(&self) -> usize {
...
impl<B: VersionedLayoutProvider + ?Sized> VersionedLayoutProvider for DeltaLayoutProvider<'_, B> {
    fn provide_node_universe(&self) -> Universe<NodeRowId> {
        Universe::from_length(self.provide_node_count())
    }
