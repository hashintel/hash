...
The wire version the binary envelopes speak, the served variant names, the bucket schedule the \
     ...
     without a token succeeds.";

/// What the optional filter document does.
const FILTER: &str =
    "An optional body carries a filter document, which is why this read is a `POST`: the document \
     is part of the view's identity. Its digest - taken over the bytes exactly as presented - \
     seals into the token, and the visibility proof compiles over the document itself. Presented \
     beside a token, it re-binds what the view selects while the session keeps its delivery \
     depth, so changing a filter does not change the detail a tile carries at a fixed zoom; a new \
     view that resolves coarser clamps that depth down to it. A request with no body is an \
     unfiltered bootstrap, or a renewal that re-mints the sealed scope verbatim.";
...
///
/// The manifest is also where a filter binds. The body holds the filter document; its digest seals
/// into the token as the view's identity and the resolution compiles the document into the
/// visibility proof. `k` is the session's visual scale rather than the filter's property: a carried
/// token keeps its offset through every re-mint, so re-binding a filter changes what the view
/// selects without changing how deep its tiles reach. Only a fresh bootstrap - a request presenting
/// no token - resolves `k`, over the occupancy of the view it seals for. A request with no body
/// re-mints the sealed scope verbatim and resolves nothing.
pub(super) async fn handler(
    ...
    let scope = match (carried, filter) {
        // A verbatim renewal: authority re-mints, the view is untouched, nothing resolves.
        (Some(scope), None) => scope,
        // The filter re-binds at the mint boundary: the presented document is resolved and its
        // digest names the view from here on, while the carried offset keeps the session's
        // delivery depth across the re-bind.
        (Some(scope), Some((digest, document))) => {
            visibility::resolve(&state, actor, Some(digest), Some(document)).await?;
        ...
        // A fresh bootstrap resolves `k` over the occupancy of the view it seals for.
        (None, filter) => {
    ...
    operation
        .response_with::<200, Json<Manifest>, _>(|mut response| {
            ...
            response
                .inner()
                .headers
                .insert("Atlas-Authority".to_owned(), headers::authority());
...
mod tests {
    ...
    fn the_response_declares_the_minted_authority_header() {
        ...
        assert!(
            headers["Atlas-Authority"].is_object(),
