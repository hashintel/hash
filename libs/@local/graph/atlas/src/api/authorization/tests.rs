...
/// The wall-clock timeout bounding every `advance` wait.
const BUDGET: Duration = Duration::from_secs(5);
/// The manager's polling interval in these fixtures.
const POLL_INTERVAL: Duration = Duration::from_millis(5);
/// The manager's generation retention window in these fixtures.
const RETENTION: Duration = Duration::from_secs(30);
/// The fixed sealing secret shared by every token issued in this module.
const AUTH_SECRET: &[u8] = b"atlas-admission-test-authority-secret-material";
/// The token window used by cases that do not exercise expiry.
const LONG_EXPIRATION: Duration = Duration::from_secs(3600);

/// Opens the parent [`GenerationRoot`] of a published fixture.
///
/// # Panics
///
/// Panics if the fixture has no parent directory or opening its root fails.
#[track_caller]
...
/// Republishes a fixture with a different representations placeholder.
///
/// `marker` replaces the placeholder to distinguish generations without constructing another fitted
/// representation.
///
/// # Panics
///
/// Panics if copying, modifying or republishing the fixture fails.
#[track_caller]
...
/// Builds a store pool for extractor-only requests.
///
/// The fixture routes complete without visibility resolution or a database connection.
///
/// # Panics
///
...
/// Publishes a synthetic generation and prepares its maintenance manager.
///
/// The returned [`GenerationManager`] retains replaced generations for `retention`.
///
/// # Panics
///
...
/// Drives maintenance until `probe` observes the requested state.
///
/// # Panics
///
/// Panics if polling `running` completes it or `probe` does not succeed within [`BUDGET`].
async fn advance<T>(
...
/// Creates a generation root owned by a scratch-directory handle.
///
/// # Panics
///
/// Panics if the temporary path is not UTF-8, removing prior scratch data fails or creating the
/// root fails.
#[track_caller]
...
/// Builds request state with fixed test limits and supplied admission resources.
///
/// # Panics
///
...
/// Creates a user [`ActorId`] from the UUID integer `id`.
fn user_actor(id: u128) -> ActorId {
    ActorId::new(Uuid::from_u128(id), ActorType::User)
}

/// Issues a token for a fixture's current publication.
///
/// The token seals `actor`, `filter` and `zoom` over the requested `generation`.
///
/// # Panics
///
/// Panics if generation observation or nonce generation fails.
#[track_caller]
fn issue_token(
...
/// Returns the publication reference expected for a fixture generation.
///
/// # Panics
///
/// Panics if observing `generation` fails.
#[track_caller]
...
/// The generation and delta reference expected from [`AuthorityScope`] extraction.
#[derive(Clone, Copy)]
...
/// The continuity expected from [`Renewal`] extraction, if any.
#[derive(Clone, Copy)]
struct ExpectedContinuity(Option<(Option<FilterDigest>, Zoom)>);

/// Attaches `value` to `req`'s extensions for extraction through [`Extension`].
...
/// Checks the publication retained by an extracted [`AuthorityScope`].
///
/// # Panics
///
/// Panics if the captured generation or delta reference differs from `expected`.
...
/// Checks the continuity retained by an extracted [`Renewal`].
///
/// # Panics
///
/// Panics if the carried filter or offset differs from `expected`.
...
/// Builds fixture routes for admission and renewal probes.
...
/// Builds a pre-authenticated `GET` request for an extractor probe.
///
/// The path is `/{prefix}/{generation}`, with `/plain` appended for `"admission"`. A supplied token
/// becomes the `Atlas-Authority` header.
///
/// # Panics
///
/// Panics if the path or token is not valid HTTP request data.
fn request(
    prefix: &str,
    generation: &str,
    actor: ActorId,
...
/// Builds a pre-authenticated JSON `POST` request to an API route.
///
/// A supplied token becomes the `Atlas-Authority` header.
///
/// # Panics
///
/// Panics if the path or token is invalid HTTP request data, or body serialization fails.
#[track_caller]
fn route_request(
    path: &str,
    actor: ActorId,
...
/// Routes `req` through `app` and returns the status and parsed JSON response.
///
/// # Panics
///
/// Panics if routing, body collection or JSON parsing fails.
...
/// Checks a response's status and problem-type URI.
///
/// # Panics
///
/// Panics if `status` differs from `expected` or `document` does not name `kind`.
#[track_caller]
fn assert_problem(
    status: StatusCode,
...
/// Checks missing-token admission and optional-token renewal.
///
/// Renewal without a token carries no continuity. A malformed token must refuse rather than
/// bootstrap.
///
/// # Panics
///
/// Panics if request construction or execution fails, or a response differs from these
/// expectations.
...
/// Checks a data token's actor binding and captured publication.
///
/// The issuing actor's request succeeds with the expected generation and delta reference. The same
/// token from `other_actor` fails admission.
///
/// # Panics
///
/// Panics if fixture observation or request execution fails, or admission differs from these
/// expectations.
...
/// Checks generation-path rejection on admission and renewal routes.
///
/// Malformed generations answer `400 invalid-generation`. Well-formed but unknown generations
/// answer `404 unknown-generation`.
///
/// # Panics
///
/// Panics if request execution fails or a response differs from these expectations.
...
/// Checks expired-token admission and renewal under a zero expiration window.
///
/// Data admission refuses every token. Renewal retains the filter and offset.
///
/// # Panics
///
/// Panics if fixture setup or request execution fails, or a response differs from these
/// expectations.
...
/// Exercises admission and renewal over one running generation.
///
/// The case covers missing tokens, actor binding, path rejection and expiration, then joins manager
/// shutdown.
#[tokio::test]
...
/// Checks cross-generation rejection and variant-free manifest parsing.
///
/// Every data route and manifest renewal refuse the other generation's token. A manifest with its
/// own token or no token reaches filter parsing and answers `invalid-body` for the supplied invalid
/// filter.
///
/// # Panics
///
...
/// Checks token binding while a generation and its retained predecessor coexist.
///
/// Each generation admits its own token and refuses the other's. [`generation_routes`] also checks
...
/// Checks `503 visibility-unavailable` for renewal against an empty registry.
///
/// The request supplies a resolved actor and no token.
#[tokio::test]
