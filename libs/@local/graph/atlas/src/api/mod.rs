...
const API_DESCRIPTION: &str =
    "The Atlas read API serves zoomable maps of the HASH graph. Geometry responses use binary \
     `SALTILE` envelopes.
...
- Binary responses use `Cache-Control: private, no-store`: the client's application-layer cache is \
     the cache, keyed on (authorization context, generation, route, canonical body). Geometry, \
...
- Atlas handlers and extractors report failures as RFC 9457 problem documents \
     (`application/problem+json`) whose `type` member is a stable root-relative URI \
     (`/problems/atlas/<slug>`). Required JSON-body extraction answers `missing-body` when \
     Content-Type is absent, without inspecting body bytes. JSON extraction failures answer \
     `invalid-body`, including an empty body with a JSON content type. An unparsable tile address \
     answers `invalid-coordinate`, and a malformed generation id answers `invalid-generation`. \
     The manifest reads raw bytes without checking Content-Type. Its body-buffering failures \
     remain plain-text 400 or 413 responses. The router also answers unmatched routes and wrong \
     methods with plain responses. An `unknown-generation` problem always means: re-read \
     `current` and retry.
...
- The binary envelope's normative contract is the `Atlas wire format` section below - this \
     document is self-contained, and a decoder implements against it.";
...
/// The shared axum state every atlas route handler and extractor receives.
///
/// It carries the request registry, the resolved limits, and the store-backed capabilities they
/// call against.
struct AppState<R> {
...
impl<R> Clone for AppState<R> {
    /// Clones shared state without requiring `R: Clone`.
    fn clone(&self) -> Self {
        // a derived impl would require that bound even though `R` lives behind an `Arc`
        Self {
...
/// # Panics
///
/// Panics outside a Tokio runtime, if `visibility.hard` exceeds 1,000 times 365 days, or if the
/// static OpenAPI route document cannot serialize.
pub(crate) fn router(
    registry: Arc<UniverseRegistry>,
