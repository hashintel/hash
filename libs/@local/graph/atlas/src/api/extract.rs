...
//!
//! [`Body`] answers `missing-body` when required JSON extraction finds no `Content-Type` header,
//! and `invalid-body` when JSON extraction rejects the request. [`Coordinates`] answers
//! `invalid-coordinate` for an unparsable `z/x/y` segment.
//!
...
///
/// Without a `Content-Type` header, required extraction answers `missing-body` and
/// `Option<Body<T>>` yields `None`, without inspecting the body bytes. JSON extraction parses even
/// an empty body when the content type is JSON. An unsupported content type, malformed JSON, shape
/// mismatch or oversized body answers `invalid-body` with the framework's rejection detail and
/// status.
#[derive(Debug)]
    ...
    async fn from_request(req: Request, state: &S) -> Result<Self, Self::Rejection> {
        match <Json<T> as OptionalFromRequest<S>>::from_request(req, state).await {
            ...
            Ok(None) => Err(Problem::new(
                ...
                "the operation requires a JSON Content-Type header for its request body",
            )),
...
/// Builds the `invalid-body` problem for one JSON rejection.
///
...
/// Builds the `invalid-generation` problem for one generation-path rejection.
///
/// Always answers 400 and uses the framework's rejection message as detail.
...
/// The `{generation}` path segment [`Generation`] extracts.
...
/// The `{variant}` path segment [`Variant`] extracts.
#[derive(serde::Deserialize, schemars::JsonSchema)]
struct VariantData {
...
mod tests {
    ...
    /// Builds a `POST` request carrying `body` as an `application/json` request body.
    ...
    /// Builds a `POST` request with no body and no content type.
    ...
    /// Renders `problem` as an HTTP response and parses its body back as JSON.
    ///
    /// The parsed document is what a test asserts a rejection's wire shape against.
    ...
    /// Well-formed JSON of the wrong shape answers `invalid-body` at `422`.
    ///
    /// The status separates a data error from the syntax error's `400`.
    #[tokio::test]
    ...
    /// Routes `uri` through a minimal router extracting [`Coordinates`].
    ///
    /// The answer is the response status with its body parsed as JSON where possible, and as a
    /// raw string otherwise.
    ///
    /// # Panics
    ///
    /// Panics where `uri` does not parse as a URI, which the request builder reports at its `body`
    /// call rather than at the `uri` call that took it. The routed answer and its body read cannot
    /// fail for this in-memory router.
    ...
    /// An unparsable tile-address segment answers `invalid-coordinate` at `400`.
    ///
    ...
    /// The extractor refuses a body with the wrong content type.
    ///
