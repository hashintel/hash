...
pub(super) struct LocateRequest {
    ...
    /// Versioned type URLs conditioning the `TYPE_MASK` column, defaulting to empty.
    ///
    /// An absent or empty list omits the column. Also the `typeIdsComplete` reference set: the
    /// flag reads `true` exactly when these ids cover the source's direct types. Entries parse
    /// at the transport boundary: a malformed URL rejects the body, while a well-formed URL
    /// this generation never ingested is legal and reads zero bits.
    #[serde(default)]
    #[schemars(with = "Vec<String>")]
    pub colored_type_ids: Vec<VersionedUrl>,
}

/// Assembles the source entity's ego-graph as `SALTILEL` bytes.
///
/// Serves `POST /v1/atlas/locate/{generation}/{variant}`. The ego-graph is the source's edges and
/// the partners they connect. The required body names exactly one source, by entity id or row id.
///
/// # Errors
///
/// Answers `invalid-source` where the body names both source fields or neither, `unknown-entity`
/// where the `entityId` does not parse, `unauthorized` where the request's authority offset cannot
/// bind its delivery schedule, the refusals [`LocateDocument::new`] reports (`too-many-types`, and
/// `unknown-entity` for a source that names no visible node, an out-of-range `row` included), and
/// `internal` where a delivered row has no captured data, where hydration fails, or where the
/// assembly worker panics. Encoding itself reports nothing, since the locate document declares an
...
pub(super) async fn handler<R>(
    ...
    // encoding all three. `LocateDocument` borrows the scene `visibility.scene()` resolves. None of
    // them can outlive this closure. `AssertUnwindSafe` covers the resolver's connection pool
    // handle.
