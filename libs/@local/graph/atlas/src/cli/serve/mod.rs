...
impl ServeCommand {
    ...
    /// window, a visibility cache entry's expiry, and retained-generation admission after
    /// replacement promotion. Each check runs on its own, and a token issued now can still read an
    /// entry already past [`VisibilityLimits::soft`].
    ///
    /// The host must run and retain the maintenance future and any download future from
    /// [`Serve::into_parts`] through listener failure and shutdown. Downloading verifies files and
    /// updates local current. Maintenance observes that pointer to open and promote generations.
    ///
    /// # Errors
    ///
    ...
    /// # Panics
    ///
    /// Panics outside a Tokio runtime or if `visibility.hard` exceeds 1,000 times 365 days. The
    /// rate limiters start their eviction task on that runtime. Before any of that,
    /// [`verify_cpu_baseline`](crate::math::kernel::verify_cpu_baseline) rejects a CPU below the
    /// compiled baseline, on the conditions it documents.
    pub fn run<P>(
        self,
        ServeOptions {
            provider,
