    ...
    T: ColumnScalar + FromBytes + KnownLayout,
{
    ...
    pub(crate) fn view(&self) -> &IdSlice<I, T> {
        // SAFETY: `Self::new` proved `file.column::<I, T>()` succeeds for this file: its element
        // stamp is `T`'s variant and trailing shape, and the open validated the data length. The
        // file is immutable after open. Therefore the same view exists for the value's lifetime.
