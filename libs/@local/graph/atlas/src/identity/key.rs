...
hashql_core::id::newtype! {
    ...
    /// generation's points.
    ///
    /// The key order and the [`BasePosition`] order are different permutations of the same points.
    /// A bucket-major cut reads the base order, while a prefix scan over spatial keys reads this one.
    /// Converting between them goes through the generation's key-order columns, never by reinterpreting the integer.
    ///
