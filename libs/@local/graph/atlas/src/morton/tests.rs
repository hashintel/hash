...
/// The first sixteen keys trace the Z-order curve over the 4 x 4 grid, with `x` on the even bits
/// and `y` on the odd bits, in both encoding directions.
#[test]
fn curve_start() {
...
/// A key's tile at depth `d` carries the leading `d` bits of each axis, from the root tile at
/// depth zero to the full coordinates at maximum depth.
#[test]
...
/// A key's tile at any depth addresses the same cell the key's `cell` names at that depth.
#[property_test]
