...
hashql_core::id::newtype! {
    ...
    ///
    /// Depth d cells are the squares of a 2ᵈ × 2ᵈ grid over the axis domain. [`Depth::MIN`] is the whole domain. [`Depth::MAX`] fixes all 32 bits of both axes and identifies one key.
    ///
    /// Serializes as a byte-valued integer. Deserialization rejects values above 32.
    #[id(unaligned, const, derive(Step))]
    pub struct Depth(u8 is 0..=32)
}

impl Depth {
    /// Validates a subdivision count.
    ///
    ...
    /// Returns the subdivisions left below this depth, `32 - depth`, as a zoom offset.
    ///
    /// A schedule that cuts at depth `s + k` for an offset `k` can raise `k` this far before the
    /// cut leaves the key width.
    #[inline]
    #[must_use]
    ...
    /// Returns the first zoom whose cut reaches this bucket depth.
    ///
    /// For a cut at z + s, where z is the zoom and s is `span`, a bucket at depth b first satisfies
    /// b ≤ z + s at z = max(b − s, 0).
    #[inline]
    #[must_use]
    ...
    /// Converts a cut depth to a zoom, clamping depths below `span` to the root.
    ///
    /// Returns max(depth − span, 0), as [`Self::first_zoom`] does.
    #[inline]
    #[must_use]
    ...
    /// Adds `steps` subdivisions, [`None`] past [`Depth::MAX`].
    #[inline]
    #[must_use]
...
///
/// Zoom z addresses the tiles of the 2ᶻ × 2ᶻ grid, the cells of [`Depth`] z. Distinct types keep a
/// tile's level and a key's subdivision depth out of each other's arithmetic.
///
/// Serializes as a byte-valued integer. Deserialization rejects values above 32.
#[derive(
    Debug,
...
impl Zoom {
    ...
    /// Returns the cut depth `zoom + span`, [`None`] past [`Depth::MAX`].
    pub(crate) const fn depth(self, span: Log2) -> Option<Depth> {
        Depth::new(self.get()).checked_add(span)
    }

    /// Returns the cut depth `zoom + span`, saturating at [`Depth::MAX`].
...
impl Step for Zoom {
    ...
    #[expect(
        ...
        reason = "the wrap folds the step count onto the `MAX + 1` cycle"
    )]
...
/// A depth and cell coordinates on that depth's grid.
///
/// The fields alone do not enforce coordinate bounds. [`MortonCell::from_tile`] validates that x
/// and y each lie below 2ᶻ.
...
pub(crate) struct MortonTile {
    ...
    pub z: Depth,
    /// The cell's x index on the 2ᶻ × 2ᶻ grid.
    pub x: u32,
    /// The cell's y index on the 2ᶻ × 2ᶻ grid.
    pub y: u32,
}
...
impl MortonKey {
    ...
    /// Returns the tile address of the cell containing this key at `depth`.
    ///
    /// The address is the key's coordinates truncated to their leading `depth` bits.
    #[inline]
    #[must_use]
...
impl MortonCell {
    ...
    /// Validates the coordinates of a tile address and selects its cell.
    ///
    /// Returns [`None`] when the address lies outside its zoom's grid, as [`Self::new`] does.
    #[inline]
    #[must_use]
