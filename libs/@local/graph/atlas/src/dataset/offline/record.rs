...
//!
//! Every stream file is one rkyv archive, and this module names each file's root beside the record
//! types inside it. The writer and the reader share one vocabulary and cannot drift. Structural
//! validation happens whole at the reader's `rkyv::access` call per stream: every length, offset,
//! and text encoding, and every domain invariant a record type carries (a confidence is a canonical
//! unit fraction), checks before any record is served. The invariants that reach across fields - a
//! row column and its embedding column agreeing on the row count, an embedding index landing inside
//! its column - are stated here per root and checked once at open, because a byte-level check sees
//! one value at a time.
//!
//! # Roots
//!
...
///
/// The type is its own archived form. An archived column of embeddings is readable in place:
...
/// Validation accepts every bit pattern, matching the source contract that components are finite at
/// the source and spot-checked statistically downstream.
///
/// The declared alignment is what makes the in-place borrow an [`AlignedVecN`]: the serializer
/// aligns every archived position to the type's alignment and the open validates it. Every mapped
/// value satisfies the wrapper's address invariant by construction. The layout stays padding-free
/// only while `4 · N` is a multiple of the alignment, and [`aligned`](Self::aligned) asserts that
/// at compile time. A dimension count that would introduce padding fails the build instead of
/// undermining [`NoUndef`](rkyv::traits::NoUndef).
///
...
impl<const N: usize> Embedding<N> {
    /// The compile-time proof that the declared alignment introduces no padding.
    ///
    /// The same proof establishes the SIMD wrapper's address invariant.
    ///
    /// Every constructor and accessor evaluates it at compile time. An `N` that violates it fails
    /// the build. `Portable` and [`NoUndef`](rkyv::traits::NoUndef) below rely on the padding-free
    /// half.
    const LAYOUT: () = {
        assert!(
            size_of::<Self>() == N * 4,
    ...
    pub(super) const fn aligned(&self) -> &AlignedVecN<N> {
        ...
        // SAFETY: `self` is `repr(C, align(32))`. Every reference to it is 32-aligned, and `LAYOUT`
        // pins 32 at or above the wrapper's required alignment. The component array is the struct's
        // first field at offset 0. It shares the address.
...
// alignment 32, no padding (`LAYOUT`, compile-time) - on every target, and the type has no interior
// mutability. The components are the writer's native `f32`s. A reader on the other byte order would
// compute different values from the same bytes: the dump manifest stamps its writer's byte order
// and the open refuses the other order before any archived value is reached.
unsafe impl<const N: usize> rkyv::Portable for Embedding<N> {}

// SAFETY: one `[f32; N]` field and no padding (`LAYOUT`, compile-time). Every byte of a value is
// initialized.
...
// SAFETY: every bit pattern of `[f32; N]` is a valid component array, which leaves the check
// vacuous.
...
///
/// Entry `n` of both columns belongs to node row `n`, and the two columns are equal in length. The
/// open checks that equality once. A consumer zips them without a per-row bound. The embedding
/// column comes ahead of the record column in the declaration, because the writer streams that
/// column to disk first and validation claims the columns in declaration order.
#[cfg_attr(
    not(test),
    expect(
        dead_code,
...
/// The edge stream's root, one record per edge row.
///
/// The root packs the present embeddings apart.
///
/// An edge that carries an embedding names its entry in the packed column by position. The column
/// holds exactly the present embeddings and an embedding-free edge costs no space. The open checks
/// every named position against the column's length once. The embedding column is declared ahead of
/// the record column because the writer streams it to disk first and validation claims the columns
...
pub(crate) struct EdgeRecord {
    ...
    pub ontology: Vec<OntologyRowId>,
    /// The position of this edge's embedding in the root's packed column.
    ///
    /// It is `None` when the store holds no embedding for the edge.
...
/// payload type's own byte-level parse validates them at materialization. The payload contract
/// stays with the payload type instead of being restated here.
...
/// Builds one edge from its archived record.
///
/// The edge borrows its entry of the packed embedding column in place.
///
/// # Panics
///
/// This panics when the record names an embedding position outside the column. The open checks
/// every position against the column's length. A validated dump never reaches the panic.
...
/// # Panics
///
/// Panics when the stored token count exceeds this target's `usize`. A dump written on a wider
/// machine carries counts a narrower one cannot hold.
