//! Pluggable token counting and sentence splitting for card budgets.
...
/// Deterministic offline counter: `ceil(utf8_bytes / 4)`.
#[derive(Debug, Copy, Clone, Default)]
...
impl SentenceSplitter for NaiveSentenceSplitter {
    #[expect(
        clippy::string_slice,
        reason = "boundaries come from char_indices plus len_utf8, so slicing stays on character \
                  boundaries"
    )]
