...
the first form of the optimizer streaming its evaluations back. A study run in
the browser streams its trials into this view as Optuna tells them, and its
ring follows the record's `navigation` rather than component state, so the
drawer's Parameters band and the plot move together
