...
eight times), runs one batch of `cells × 8` runs, and combines the per-run
metric values back into per-cell means. One batch setup per chunk replaces
one per cell, and batch setup dominates a cell's wall time. Cells do not wait
for the whole chunk either: CPU
...
awaits `session.whenSelectionStreamed()` — a check that passes once the
...
reduces runs to histograms on-device.
...
A restart — any slice or axis change — clears the sampled values. The plot
...
following is off, so the drawer's Parameters band and the plot move together.
