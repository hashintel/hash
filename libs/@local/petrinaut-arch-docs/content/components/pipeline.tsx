  ...
  <figure className="pnd">
    ...
    <div className="pnd-pipeline">
      {steps.map((step, index) => (
        ...
        <Step key={index} index={index} numbered={numbered} step={step} />
      ))}
    </div>
  </figure>
);
...
const Step = ({
  ...
  // The arrow is grouped with the step that follows it rather than emitted as a
  // sibling. As separate flex items they wrapped independently, which left an
  // arrow stranded at the end of a row pointing at a box on the next one.
  <div className="pnd-step-group">
    {index === 0 ? null : (
