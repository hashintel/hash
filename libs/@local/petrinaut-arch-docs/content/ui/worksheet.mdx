...
keeping the form layer out of its imports ships with the form. The token
spreadsheet (place initial state and the scenario forms) is its first
consumer — a stop table with a gutter lane, wrapped in a vertical stack
where a scenario form mounts one grid per place; the ad-hoc scenario form
is the expected next. Stories in `worksheet.stories.tsx` demonstrate the configurations:
