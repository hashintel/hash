...
4. `contain` fences a stack: arrows never carry focus out of it, in any
   direction.
...
property access ("Cannot access refs during render"); destructured
bindings compile without the diagnostic.

## Boundaries

The layer must not import form or scenario state; the dependency rule
fencing it from the form layer ships with the form. The ad-hoc scenario form is
