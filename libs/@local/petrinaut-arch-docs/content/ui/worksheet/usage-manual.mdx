...
## Keeping the selection clear of faded edges

Hosts fade the edges of their scroll areas, and a focus scroll that parks
a cell flush against an edge leaves the selection under the fade.
`useFocusClearance` keeps every focused element at least 25px
(`FOCUS_CLEARANCE_PX`) clear of each scrollable ancestor's edges — spread
its two capture handlers onto the element that hosts the tables, and pair
it with `scroll-margin: 25px` on the focusable triggers so the scrolls the
browser performs itself land clear too.
...
A focus caused by a fresh pointer press on the element itself never
nudges: the pointer proves the user sees the element, and moving it
between the two clicks of a double-click would make the second click miss
(the selection grammar's click-count activation).
