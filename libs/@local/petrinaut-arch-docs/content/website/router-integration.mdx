...
rather than merging into the existing one. Params an older link may still
carry (`view=optimizations`, `overlay=create-optimization`, `present`) fall
under the same rule: `validateSharedExampleSearch` normalises them to the
baseline location, and the first navigation rewrites the URL. `/brunch` is the exception: its
...
Every page that mounts Petrinaut does: the editable demo at `/`, the Brunch
stream at `/brunch`, and the example pages at `/examples/$slug` and
`/embed/examples/$slug`.

They differ in whether the URL also names the model. The editable demo holds
its nets in local storage, so a link there names a location inside whatever
net the visitor has open. `/brunch` names its model with the stream keys it
