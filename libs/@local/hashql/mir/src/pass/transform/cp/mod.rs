...
impl<A: BumpAllocator> CopyPropagation<A> {
    ...
    fn propagate_block_params<'heap>(
        args: &mut Vec<Option<Constant<'heap>>, &A>,
        visitor: &mut CopyPropagationVisitor<'_, 'heap, &A>,
        body: &Body<'heap>,
        id: BasicBlockId,
    ) {
        let pred = body.basic_blocks.predecessors(id);
        ...
        args.extend(first.args.iter().map(|arg| arg.as_constant().copied()));

        for target in targets {
            debug_assert_eq!(args.len(), target.args.len());

            for (lhs, &rhs) in args.iter_mut().zip(target.args.iter()) {
                let rhs = rhs.as_constant().copied();
                if *lhs != rhs {
                    *lhs = None;
                }
            }
