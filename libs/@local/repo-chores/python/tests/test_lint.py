...
class TestUpperBounds:
    def test_bounded_ranges_pass(self) -> None:
...
class TestAllowlist:
    def test_allowlisted_dependency_is_exempt(self) -> None:
        workspace = make_workspace(dev_dependencies=("ruff>=0.15.21",), allowlist=("ruff",))

        assert lint_workspace(workspace) == []

    def test_allowlist_names_are_canonicalized(self) -> None:
    ...
    def test_allowlist_is_scoped_to_its_manifest(self) -> None:
...
class TestUrlRequirements:
    def test_url_requirements_fail_even_when_allowlisted(self) -> None:
        workspace = make_workspace(
    ...
    def test_invalid_requirement_is_reported(self) -> None:
