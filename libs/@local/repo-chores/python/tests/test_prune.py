...
def test_parse_scopes_splits_on_spaces_and_newlines() -> None:
...
def test_parse_scopes_ignores_blank_lines_and_extra_whitespace() -> None:
...
def test_expand_scopes_no_rule_matches() -> None:
    assert prune.expand_scopes({"@apps/hash-frontend"}) == frozenset()


def test_expand_scopes_prefix_trigger() -> None:
...
def test_fixpoint_expands_rules_triggered_by_dependencies() -> None:
...
def test_expansion_reaches_fixpoint_through_added_scopes() -> None:
...
def test_fixpoint_expand_terminates_on_cycles() -> None:
    dependencies = {
...
def test_resolves_cargo_globs_and_literal_members(tmp_path: Path) -> None:
...
def _make_cargo_workspace(root: Path) -> None:
...
def test_stubs_only_missing_cargo_members(tmp_path: Path) -> None:
    _make_cargo_workspace(tmp_path)
    out = tmp_path / "out"
...
def test_skips_manifests_without_a_package_name(tmp_path: Path) -> None:
    (tmp_path / "Cargo.toml").write_text('[workspace]\nmembers = ["virtual"]\n')
...
def test_cargo_dry_run_writes_nothing(tmp_path: Path) -> None:
    _make_cargo_workspace(tmp_path)
    out = tmp_path / "out"
...
def test_missing_root_pyproject(tmp_path: Path) -> None:
    assert prune.resolve_uv_members(tmp_path) == []


def test_pyproject_without_workspace(tmp_path: Path) -> None:
    (tmp_path / "pyproject.toml").write_text('[project]\nname = "root"\n')
    assert prune.resolve_uv_members(tmp_path) == []


def test_resolves_uv_member_globs(tmp_path: Path) -> None:
...
def test_respects_workspace_excludes(tmp_path: Path) -> None:
    (tmp_path / "pyproject.toml").write_text(
    ...
    assert [path.relative_to(tmp_path).as_posix() for path in members] == ["tools/a/pyproject.toml"]
...
def test_copies_manifests_of_missing_members(tmp_path: Path) -> None:
...
def test_uv_dry_run_writes_nothing(tmp_path: Path) -> None:
    _make_uv_workspace(tmp_path)
    out = tmp_path / "out"
