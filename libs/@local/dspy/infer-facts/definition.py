...
class Fact(pydantic.BaseModel):
    text: str = pydantic.Field(description="""
      ...
      - must have a single object, especially where an objectEntityLocalId is available (it only refers to a single object of the fact)
    ...
    prepositionalPhrases: List[str] = pydantic.Field(description="""
      ...
      - "with a combination of cash and stock"
            """)
    objectEntityLocalId: Optional[str] = pydantic.Field(description="""
        The localId of the entity that is the object if the fact, if any. Facts don't need to have entities as their object.
        If a
        """, default = None)

# This is not used in the Signature but is here to represent the schema of potential_object_entities as they should appear in the training/testing dataset JSON
...
class FactInferrerSignature(dspy.Signature):
    ...
    The user will provide you with:
        - context: the text from which you should extract facts.
        - subject_entity: the subject entities of facts that the user is looking for
        - potential_object_entities: a list of other entities mentioned in the text, which may be the object of facts.

    You must provide an exhaustive list of facts about the provided subject entity based on the information provided in the text.
    ...
    potential_object_entities: list = dspy.InputField(description="The entities that could be the object of the facts", required=False, format=list)
