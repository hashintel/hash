 ...
 */
export const getStepDefinitionFromFlowDefinition = <T extends FlowStep>(params: {
  step: T;
  flowDefinition: FlowDefinition<FlowActionDefinitionId>;
}): T extends ActionStep ? ActionStepWithParallelInput : ParallelGroupStepDefinition => {
  const { step, flowDefinition } = params;

  const allStepDefinitions = getAllStepDefinitionsInFlowDefinition(flowDefinition);

  const [stepIdWithoutIndex] = step.stepId.split("~");

  const flowDefinitionNode = allStepDefinitions.find(({ stepId }) => stepId === stepIdWithoutIndex);

  if (!flowDefinitionNode) {
    throw new Error(`No flow definition step found for step with id ${step.stepId}`);
  }
