...
fn details_typed_extensions() {
    ...
    let details: ProblemDetails<'_, '_, InvalidParameters> = serde_json::from_str(&encoded)
...
fn details_empty_extensions() {
    ...
    let encoded = serde_json::to_string(&body).expect("the response should serialize");
    let details: ProblemDetails<'_, '_, NoExtensions> =
...
fn details_owned_deserialize() {
    ...
    let encoded = serde_json::to_string(&body).expect("the response should serialize");
    let from_string: ProblemDetails<'static, 'static, NoExtensions> =
        serde_json::from_str(&encoded).expect("the response should deserialize from a string");
    let from_value: ProblemDetails<'static, 'static, NoExtensions> =
        serde_json::from_value(body.clone()).expect("the response should deserialize from a value");
    let from_reader: ProblemDetails<'static, 'static, NoExtensions> =
...
fn details_extensions_missing() {
    ...
    let error = serde_json::from_value::<ProblemDetails<'_, '_, InvalidParameters>>(body)
        .err()
...
fn details_extensions_non_object_deserialize() {
    let body = json!({"type": "about:blank", "title": "Bad Request", "status": 400});
    let error = serde_json::from_value::<ProblemDetails<'_, '_, u32>>(body)
        .err()
...
mod schema {
    ...
    #[test]
    fn schema_base() {
        assert_schema_snapshot!(ProblemDetails<'_, '_, NoExtensions>);
    }

    #[test]
    fn schema_typed_extensions() {
        assert_schema_snapshot!(ProblemDetails<'_, '_, InvalidParameters>);
    }

    #[test]
    fn schema_occurrence_members() {
        let schema = schemars::schema_for!(ProblemDetails<'_, '_, NoExtensions>).to_value();
        let minimal =
