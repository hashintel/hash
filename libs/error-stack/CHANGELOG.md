...
- Introduced `Report<dyn Error>`: `From<Report<C>> for Report<dyn Error>` to use dynamically. It should be said, it's discouraged to be used as return type and is mainly interesting for locations where we don't need the context type, e.g. in error-handling. It's expected to be seen more in argument position in general ([#7274](https://github.com/hashintel/hash/pull/7274))
