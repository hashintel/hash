...
pub trait PostgresContext {
    ...
    async fn read_all_current_links(&self) -> Result<links::RecordStream, QueryError>;

    async fn read_current_links_by_source(
        &self,
        entity_id: EntityId,
    ) -> Result<links::RecordStream, QueryError>;

    async fn read_current_links_by_target(
        &self,
        entity_id: EntityId,
    ) -> Result<links::RecordStream, QueryError>;
}
...
impl<C: AsClient> PostgresContext for PostgresStore<C> {
    ...
    async fn read_all_current_links(&self) -> Result<links::RecordStream, QueryError> {
        links::read_all_current_links(&self.client)
            .await
    ...
    async fn read_current_links_by_source(
        ...
        links::read_current_links_by_source(&self.client, entity_id)
            .await
    ...
    async fn read_current_links_by_target(
        ...
        links::read_current_links_by_target(&self.client, entity_id)
            .await
