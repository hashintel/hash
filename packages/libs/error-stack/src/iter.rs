...
impl<'r> FramesMut<'r> {
    pub(crate) fn new<C>(report: &'r mut Report<C>) -> Self {
        Self {
            current: Some(NonNull::from(report.frame_mut())),
            _marker: PhantomData,
        }
    }
}
...
impl<'r> Iterator for FramesMut<'r> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        unsafe {
            if let Some(source) = current.as_mut().source_mut() {
                self.current = Some(NonNull::from(source));
            }
            Some(current.as_mut())
        }
    }
