...
export const wrapCommand =
  ...
  (state, dispatch, view) => {
    ...
    const wrappers: [number, ProsemirrorNode<Schema>[]][] = [];
    ...
    state.doc.descendants((node, pos) => {
      ...
      if (node.isTextblock) {
        ...
        for (let depth = $originalStart.depth; depth > 0; depth--) {
          wrapperNodes.push($originalStart.node(depth));
        }
     ...
     * order to apply the desired prosemirror command, we need an instance of
     * the current state at the point of which each of the blocks have been
     ...
     * that next state. This will allow us to use it to generate the desired
     * end state.
     *
     ...
     * prosemirror and merge them into our existing transaction. This allows us
     * to apply all the changes together in one fell swoop, ensuring we don't
     * have awkward intermediary history breakpoints
     *
    ...
    (() => {
      // const newState = nextState2;
      // const tr = newState.tr;
      //
      // const mapping = new Mapping();
      // let stepCount = tr.steps.length;

      for (const [pos, wrapperNodes] of wrappers) {
        const $start = tr.doc.resolve(tr.mapping.map(pos));
        // console.log($pos);
        const node = $start.nodeAfter;
        if (!node) {
          throw new Error("Missing node to wrap");
        }
