...
(async () => {
  ...
  const createEntities = async (elmnts: Map<string, CreateEntityMutationVariables>) => {
    for (const [name, vars] of elmnts.entries()) {
    //   console.log(name);
      const res = await client.request<CreateEntityMutation>(createEntity, vars);
...
//   ]));
  ...
  console.log(JSON.stringify(Object.fromEntries(results as any), null, 2));
})();
