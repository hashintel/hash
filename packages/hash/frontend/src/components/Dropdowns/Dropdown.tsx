  ...
  return (
    <Listbox value={selected} onChange={(option) => onChange(option?.value)}>
      <div className={tw`relative mt-1`}>
        <Listbox.Button className={tw`relative w-40 h-6 py-2 pl-3 pr-10 text-left bg-white rounded-lg shadow-md cursor-default focus:outline-none focus-visible:ring-2 focus-visible:ring-opacity-75 focus-visible:ring-white focus-visible:ring-offset-orange-300 focus-visible:ring-offset-2 focus-visible:border-indigo-500 sm:text-sm`}>
          <span className={tw`block truncate`}>{selected?.label}</span>
          <span className={tw`absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none`}>
            <IconDropdown />
          </span>
        </Listbox.Button>
        <Transition
        ...
        >
          <Listbox.Options className={tw`absolute w-60 py-1 mt-1 overflow-auto text-base bg-white rounded-md shadow-lg max-h-60 ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm`}>
