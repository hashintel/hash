...
impl Engine {
    ...
    fn handle_messages(&mut self, state: &mut State, message_map: &MessageMap) -> Result<()> {
        ...
        // it is necessary to drop `message_proxies` here, because it contains a strong reference to
        // the MessageBatch; if this strong reference exists, then it is not possible to call
        // `state.read()` later (because `state.read()` will fail it there are multiple references
        // to the batch).
        drop(message_proxies);
