...
impl AgentBatch {
    ...
    pub fn from_segment(
        ...
        let (schema, static_meta) = if let Some(s) = schema {
            ...
            let static_meta = Arc::new(Static::from_schema(schema.clone()));
            (schema, static_meta)
        };
        ...
        let dynamic_meta = Dynamic::from_record_batch(&batch_message, buffers.data().len())?;
