...
// Task is passed to a worker (Worker) comprising of a triplet of language runners (Python + JS + Rust).
//
...
//
// A TargetedPackageTaskMsg is then returned with a possible target of Python, JavaScript, Rust, Custom
// or Main. The first three targets are obvious, so we'll touch more on Custom and on
// Main later.
// This TargetedPackageTaskMsg is passed around until a TaskResult instance is returned (which must be
// defined (by adding a custom enum-variant to TaskResult)) per each Task type.
// This TaskResult is then passed back up to the worker pool (which in the distributed case waits for
// and combines all returned 'TaskResult's into a single TaskResult (using custom logic)).
//
...
// a TargetedPackageTaskMsg with a new (and different) target based on custom logic. Hence this target
// for such a message can be either Python, JavaScript, Rust or Main.
//
